  ...
  function signOutUser() {
    // ✅ NEW: Tej ফিড থেকে প্রোফাইলে গিয়ে লগ আউট করলে, পরের লগইনে আবার Tej ফিডেই ফিরে আসার জন্য ফ্ল্যাগ সেভ করো
    if (window._cameFromTejFeed) {
      localStorage.setItem('pv_reopen_tej_feed_after_login', '1');
    } else {
      localStorage.removeItem('pv_reopen_tej_feed_after_login');
    }
    auth.signOut().then(() => {
      currentUser = null;
      currentUserRole = null;
      // ✅ FIX: _prevAuthUid রিসেট না করলে, একই ইউজার লগ আউট করে আবার লগইন করলে
      // (uid অপরিবর্তিত থাকায়) নিচের "Welcome back" ব্লক স্কিপ হয়ে যেত —
      // ফলে Tej ফিডে ফিরে আসার লজিকও কখনো রান হতো না। তাই সাইন-আউটেই uid ক্লিয়ার করো।
      _prevAuthUid = null;
      closeProfile();
    });
  }
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
      ...
      if (_prevAuthUid !== user.uid) {
        ...
        // ✅ NEW: Tej ফিডে থাকা অবস্থায় লগ আউট করা হলে, লগইন করার পর আবার Tej ফিডেই ফিরিয়ে আনো
        if (localStorage.getItem('pv_reopen_tej_feed_after_login') === '1') {
          localStorage.removeItem('pv_reopen_tej_feed_after_login');
          setTimeout(function() {
            if (typeof closeProfile === 'function') closeProfile();
            if (typeof openTejFeed === 'function') openTejFeed();
          }, 500);
        }
      }
      ...
      // ✅ NEW (feature-98): মিসড কল ব্যাজ + অ্যালার্ট লিসেনার
      if (!window._missedCallBadgeListenerStarted) {
        window._missedCallBadgeListenerStarted = true;
        initMissedCallBadgeListener(user.uid);
      }
      ...
      // ✅ NEW (feature-67): লাইফ পার্টনার — নিজে পাঠানো প্রপোজ কেউ accept করলে রিয়েল-টাইমে ম্যাচ সেলিব্রেশন দেখানোর লিসেনার
      if (!window._lifePartnerMatchListenerStarted) {
        window._lifePartnerMatchListenerStarted = true;
        initLifePartnerMatchListener();
      }
      // ✅ NEW: X/Y (বয়ফ্রেন্ড-গার্লফ্রেন্ড প্রপোজ) ফিচার — প্রোফাইল ক্যাশ + ম্যাচ/ব্যাজ লিসেনার
      if (!window._tejXYModuleStarted) {
        window._tejXYModuleStarted = true;
        initTejXYModule();
      }
      ...
      // ✅ NEW (feature-70): লগ আউট করলে সব ইউজার-স্কোপড Firestore লিসেনার বন্ধ করো —
      // নাহলে auth টোকেন চলে যাওয়ার পরেও এগুলো চালু থেকে যায় এবং কনসোলে বারবার
      // "permission-denied" এরর দেখাতে থাকে (Firestore Rules লগইন ছাড়া রিড ব্লক করে)।
      if (typeof _lifePartnerMatchListenerUnsub === 'function') { _lifePartnerMatchListenerUnsub(); _lifePartnerMatchListenerUnsub = null; }
      if (typeof _tejXYMatchListenerUnsub === 'function') { _tejXYMatchListenerUnsub(); _tejXYMatchListenerUnsub = null; }
      if (typeof _tejXYReceivedUnsub === 'function') { _tejXYReceivedUnsub(); _tejXYReceivedUnsub = null; }
      if (typeof _friendReqUnsub === 'function') { _friendReqUnsub(); _friendReqUnsub = null; }
      if (typeof _myFriendCountUnsubA === 'function') { _myFriendCountUnsubA(); _myFriendCountUnsubA = null; }
      if (typeof _myFriendCountUnsubB === 'function') { _myFriendCountUnsubB(); _myFriendCountUnsubB = null; }
      if (typeof _myFollowerCountUnsub === 'function') { _myFollowerCountUnsub(); _myFollowerCountUnsub = null; }
      if (typeof _incomingCallUnsub === 'function') { _incomingCallUnsub(); _incomingCallUnsub = null; }
      if (typeof _missedCallBadgeUnsub === 'function') { _missedCallBadgeUnsub(); _missedCallBadgeUnsub = null; }
      if (typeof _customerOrderStatusUnsub === 'function') { _customerOrderStatusUnsub(); _customerOrderStatusUnsub = null; }
      if (typeof _chatUnreadUnsub === 'function') { _chatUnreadUnsub(); _chatUnreadUnsub = null; }
      if (typeof _adminNotifUnsub === 'function') { _adminNotifUnsub(); _adminNotifUnsub = null; }

      // ✅ পরের লগইনে (একই বা অন্য ইউজার) সব লিসেনার নতুন করে চালু হওয়ার জন্য ফ্ল্যাগ রিসেট করো
      window._chatBadgeListenerStarted = false;
      window._friendReqListenerStarted = false;
      window._myFriendCountListenerStarted = false;
      window._myFollowerCountListenerStarted = false;
      window._incomingCallListenerStarted = false;
      window._missedCallBadgeListenerStarted = false;
      window._orderStatusListenerStarted = false;
      window._lifePartnerMatchListenerStarted = false;
      window._tejXYModuleStarted = false;
      window._adminNotifListenerStarted = false;
    }
  ...
  let _customerOrderStatusUnsub = null; // ✅ NEW (feature-70): লগ আউটে বন্ধ করার জন্য রেফারেন্স রাখা হলো
  function initCustomerOrderStatusListener(uid) {
    _myOrderStatusFirstLoad = true;
    if (_customerOrderStatusUnsub) { _customerOrderStatusUnsub(); _customerOrderStatusUnsub = null; }
    _customerOrderStatusUnsub = firestore.collection('orders').where('customerUid', '==', uid).limit(30)
      .onSnapshot(snap => {
  ...
  let _chatUnreadUnsub = null; // ✅ NEW (feature-70): লগ আউটে বন্ধ করার জন্য রেফারেন্স রাখা হলো
  function initChatUnreadListener(uid) {
    if (_chatUnreadUnsub) { _chatUnreadUnsub(); _chatUnreadUnsub = null; }
    _chatUnreadUnsub = firestore.collection('direct_chats').doc(uid).onSnapshot(doc => {
  ...
  async function loadUserRole(user) {
    try {
      ...
      if (doc.exists) {
        ...
        userVoucherBalance = doc.data().voucherBalance || 0; // ✅ NEW (feature-90)
        // ✅ NEW: কাস্টম রিংটোন URL — অ্যাকাউন্ট-লেভেলে সেভ থাকে বলে নতুন ডিভাইসেও লগইন করলে পাওয়া যাবে
        userCustomRingtoneUrl = doc.data().customRingtoneUrl || null;
        _cacheCustomRingtoneUrl(userCustomRingtoneUrl);
        ...
        userVoucherBalance = 0; // ✅ NEW (feature-90)
        userCustomRingtoneUrl = null; // ✅ NEW
        _cacheCustomRingtoneUrl(null);
        await userRef.set({
          uid: user.uid, name: user.displayName || '', email: user.email || '',
          photo: user.photoURL || '', role: currentUserRole,
          loyaltyPoints: 0, referralCode: userReferralCode, referralCount: 0,
          balance: 0, voucherBalance: 0,
  ...
  // ============================================================
  // ✅ NEW (feature-90): কেনা পণ্যের জন্য ইউনিক টোকেন জেনারেট — লাইভ যাচাইয়ের কাজে ব্যবহৃত হবে
  // ============================================================
  ...
  // কপি-টু-ক্লিপবোর্ড — অর্ডার হিস্টরিতে টোকেন কপি বাটনে ব্যবহৃত
  function copyProductToken(token, btnEl) {
    if (!token) return;
    const doToast = () => {
      if (btnEl) {
        const original = btnEl.innerHTML;
        btnEl.innerHTML = `<i class="fas fa-check"></i> কপি হয়েছে`;
        setTimeout(() => { btnEl.innerHTML = original; }, 1500);
      } else {
        alert('✅ টোকেন কপি হয়েছে: ' + token);
      }
    };
    if (navigator.clipboard) {
      navigator.clipboard.writeText(token).then(doToast).catch(() => alert('টোকেন: ' + token));
    } else {
      alert('টোকেন: ' + token);
    }
  }
  ...
  // ✅ [FULL HD আপডেট] sampleRate 48kHz করে দেওয়া হলো — পরিষ্কার, উচ্চমানের ভয়েসের জন্য
  ...
  // ✅ [FULL HD আপডেট] ভিডিও কলে ফুল HD (1920x1080) রেজোলিউশন — নেটওয়ার্ক ভালো না থাকলে
  // ব্রাউজার নিজে থেকেই "ideal" মান অনুযায়ী কমিয়ে দেবে (তাই ব্লক হবে না, শুধু চেষ্টা করবে সর্বোচ্চ মানে)
  ...
  // ✅ [FULL HD আপডেট] WebRTC ডিফল্ট বিটরেট সাধারণত রেজোলিউশনের তুলনায় কম রাখে (ব্যান্ডউইথ বাঁচাতে)।
  // এখানে sender-এর encoding parameters বদলে ভিডিও/অডিও বিটরেটের সর্বোচ্চ সীমা বাড়ানো হলো,
  // যাতে ভালো নেটওয়ার্কে সত্যিকারের ফুল HD মানের স্ট্রিম যায়। এটা একটা "সিলিং" মাত্র —
  // নেটওয়ার্ক খারাপ হলে ব্রাউজারের নিজস্ব adaptive bitrate (bandwidth estimation) স্বয়ংক্রিয়ভাবে
  // এর নিচে নেমে আসবে, তাই কল ড্রপ বা ল্যাগের ঝুঁকি বাড়ে না।
  async function applyHDEncodingParams(pc) {
    try {
      const videoSender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
      if (videoSender) {
        const params = videoSender.getParameters();
        if (!params.encodings || !params.encodings.length) params.encodings = [{}];
        params.encodings[0].maxBitrate   = 2500000; // ~2.5 Mbps — 1080p এর জন্য যথেষ্ট
        params.encodings[0].maxFramerate = 30;
        await videoSender.setParameters(params);
      }
    } catch (e) { /* setParameters সাপোর্ট না করলে ডিফল্ট বিটরেটেই কাজ চলবে */ }
    try {
      const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
      if (audioSender) {
        const aparams = audioSender.getParameters();
        if (!aparams.encodings || !aparams.encodings.length) aparams.encodings = [{}];
        aparams.encodings[0].maxBitrate = 64000; // ৬৪ kbps — স্পষ্ট, পরিষ্কার ভয়েসের জন্য যথেষ্ট
        await audioSender.setParameters(aparams);
      }
    } catch (e) { /* সাপোর্ট না থাকলে স্কিপ */ }
  }
  ...
  let _vibrateInterval     = null; // ✅ [রিংটোন ফিক্স] ভাইব্রেশন ও বিপ-লুপের জন্য আলাদা ইন্টারভাল — আগে দুটোই _ringtoneOsc শেয়ার করত বলে বিপ লুপ কখনো শুরুই হতো না
  // ✅ NEW (feature-98): কল হিস্টরি (ইনকামিং/আউটগোয়িং/মিসড) + মিসড কল অ্যালার্ট
  let _missedCallBadgeUnsub = null; // 'call_logs' রিয়েল-টাইম আনরিড মিসড কল ব্যাজ লিসেনার
  let _missedCallFirstLoad  = true; // প্রথম স্ন্যাপশট লোডে পুশ নোটিফিকেশন স্প্যাম আটকানোর জন্য
  let _tejCallLogData       = [];   // কল হিস্টরি লিস্টের ক্যাশ (মেসেজ ইনবক্স মডালের "কল" ট্যাব)
  let _tejInboxActiveTab    = 'chat'; // মেসেজ ইনবক্স মডালে বর্তমান সক্রিয় ট্যাব: 'chat' | 'calls'
  ...
  // ============================================================
  let _tejUnsub = null;
  let _tejFeedVideoObserver = null; // ✅ NEW (feature-88): মেইন ফিডের ইনলাইন ভিডিও অটো-প্লে/পজ পর্যবেক্ষক
  ...
  function closeTejFeed() {
    ...
    document.body.style.overflow = 'auto';
    // ✅ NEW (feature-87): ফিড বন্ধ করার সময় ইনলাইন প্রিভিউতে চলতে থাকা ভিডিও থাকলে পজ করে দাও
    document.querySelectorAll('#tej-feed-list video, #tej-feed-scroll-wrap video').forEach(v => { v.pause(); });
  ...
  async function openTejMessageInbox() {
    ...
    switchTejInboxTab('chat'); // ✅ NEW (feature-98): সবসময় "চ্যাট" ট্যাব দিয়ে শুরু হবে
  ...
  // ✅ NEW (feature-84): ক্যাটাগরি ফিল্টার ('' মানে সব ক্যাটাগরি)
  let _appStoreCategoryFilter = '';
  // ✅ NEW (feature-84): অ্যাপ আপলোডে স্ক্রিনশট (সর্বোচ্চ ৪টি)
  const APP_SCREENSHOT_MAX    = 5;
  let _appUploadScreenshotFiles = [];
  // ✅ NEW (feature-84): রিপোর্ট ফর্মের জন্য টেম্পোরারি স্টেট
  let _appReportTargetAppId    = null;
  let _appReportSelectedReason = null;

  // ✅ NEW (feature-84): Play Store-স্টাইল কমপ্যাক্ট সংখ্যা ফরম্যাট (1K+, 1.2M+ ইত্যাদি)
  ...
  async function loadAppStoreApps() {
    ...
    renderFeaturedBanner(); // ✅ NEW (feature-84)
  }

  // ✅ NEW (feature-84): ক্যাটাগরি ফিল্টার — চিপে ট্যাপ করলে
  function filterAppStoreCategory(cat) {
    _appStoreCategoryFilter = cat || '';
    document.querySelectorAll('.app-store-cat-chip').forEach(btn => {
      const active = (btn.getAttribute('data-cat') || '') === _appStoreCategoryFilter;
  ...
  // ✅ NEW (feature-84): এডিটরস চয়েস ব্যানার — isFeatured===true ও verified অ্যাপ থেকে
  function renderFeaturedBanner() {
    const wrap = document.getElementById('app-store-featured-wrap');
    const banner = document.getElementById('app-store-featured-banner');
    if (!wrap || !banner) return;
    const featured = _appStoreApps.filter(a => a.isFeatured && a.status === 'verified');
    if (featured.length === 0) { wrap.classList.add('hidden'); banner.innerHTML = ''; return; }
    wrap.classList.remove('hidden');
    banner.innerHTML = featured.map(a => `
      <div onclick='openAppDetail("${a.id}")' class='shrink-0 w-32 bg-slate-50 rounded-2xl p-2.5 flex flex-col items-center text-center cursor-pointer active:scale-95 transition'>
        <div class='w-14 h-14 rounded-2xl bg-white flex items-center justify-center text-2xl overflow-hidden shadow-sm mb-1.5'>
          ${a.iconBase64 ? `<img loading="lazy" src="${a.iconBase64}" class="w-full h-full object-cover">` : '📦'}
        </div>
        <p class='text-[10px] font-bold text-slate-900 line-clamp-1 w-full'>${escapeHtml(a.name || '')}</p>
        <span class='text-[8px] font-bold text-amber-500 mt-0.5'><i class="fas fa-star"></i> এডিটরস চয়েস</span>
      </div>`).join('');
  }

  function _visibleAppStoreApps() {
    return _appStoreApps.filter(a => {
      // ✅ NEW (feature-84): ক্যাটাগরি ফিল্টার — অন্য যেকোনো ট্যাবের সাথেও একসাথে কাজ করে
      if (_appStoreCategoryFilter && a.category !== _appStoreCategoryFilter) return false;
  ...
  // ✅ UPDATED (feature-83): Play Store-স্টাইল ফিল্টার চিপ ট্যাব — 'সব' / 'আমার আপলোড' / 'আমার ডাউনলোড'
  // একসাথে সিঙ্ক থাকে; active চিপ = কালো ব্যাকগ্রাউন্ড সাদা টেক্সট, বাকিগুলো হালকা ধূসর।
  function _syncAppStoreTabStyles() {
    const allBtn = document.getElementById('app-store-tab-all');
    const mineBtn = document.getElementById('app-store-mine-btn');
    const dlBtn = document.getElementById('app-store-dl-btn');
    const isAll = !_appStoreOnlyMine && !_appStoreShowDownloads;
    [[allBtn, isAll], [mineBtn, _appStoreOnlyMine], [dlBtn, _appStoreShowDownloads]].forEach(([btn, active]) => {
      if (!btn) return;
      btn.classList.toggle('bg-slate-900', active);
      btn.classList.toggle('text-white', active);
      btn.classList.toggle('bg-slate-100', !active);
  ...
  // ✅ NEW (feature-83): 'সব' ট্যাবে ফিরে আসার বাটন
  function setAppStoreTabAll() {
    _appStoreOnlyMine = false;
    _appStoreShowDownloads = false;
    const searchInput = document.getElementById('app-store-search');
    if (searchInput) searchInput.value = '';
    _syncAppStoreTabStyles();
    _renderVisibleAppStoreGrid();
  }
  ...
  function openAppStore() {
    ...
    _syncAppStoreTabStyles(); // ✅ NEW (feature-83): মডাল খোলার সময় ট্যাব-চিপের ভিজ্যুয়াল স্টেট সিঙ্ক
    loadAppStoreApps();
  }
  ...
  // ✅ UPDATED (feature-83): Play Store-স্টাইল মিনিমাল ব্যাজ (emerald/amber)
  function _appStoreStatusBadge(status) {
    if (status === 'verified') return `<span class="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-1 rounded-full bg-emerald-50 text-emerald-700"><i class="fas fa-shield-halved text-[8px]"></i> যাচাইকৃত</span>`;
    if (status === 'pending') return `<span class="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-1 rounded-full bg-amber-50 text-amber-700"><i class="fas fa-clock text-[8px]"></i> রিভিউ চলছে</span>`;
    return '';
  }

  // ✅ UPDATED (feature-83): grid tiles → Google Play Store-স্টাইল vertical list rows
  // (আইকন বামে, নাম/ক্যাটাগরি/রেটিং মাঝে, ইন্সটল বাটন ডানে — ঠিক Play Store-এর অ্যাপ লিস্টের মতো)
  function renderAppStoreGrid(apps) {
    const grid = document.getElementById('app-store-grid');
    if (!grid) return;
    if (apps.length === 0) {
      grid.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-box-open text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">কোনো অ্যাপ পাওয়া যায়নি</p></div>`;
      return;
    }
    grid.innerHTML = apps.map(a => {
      ...
      const ratingLabel = avgRating > 0
        ? `<span class="inline-flex items-center gap-0.5 text-[10px] text-slate-500 font-semibold">${avgRating.toFixed(1)} <i class="fas fa-star text-[8px] text-slate-400"></i></span>`
        : `<span class="text-[10px] text-slate-300">নতুন</span>`;
      const pendingChip = a.status === 'pending'
        ? `<span class="ml-1 text-[8px] font-black px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 align-middle"><i class="fas fa-clock"></i></span>`
        : '';
      return `
      <div onclick='openAppDetail("${a.id}")' class='flex items-center gap-3 px-4 py-3 active:bg-slate-50 transition cursor-pointer'>
        <div class='w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center text-2xl overflow-hidden shrink-0'>
          ${a.iconBase64 ? `<img loading="lazy" src="${a.iconBase64}" class="w-full h-full object-cover">` : '📦'}
        </div>
        <div class='flex-1 min-w-0'>
          <p class='text-[13px] font-bold text-slate-900 truncate'>${escapeHtml(a.name || '')}${pendingChip}</p>
          <p class='text-[10px] text-slate-400 truncate mt-0.5'>${escapeHtml(a.category || '')}</p>
          <div class='flex items-center gap-2 mt-1'>
            ${ratingLabel}
            <span class="text-[9px] text-slate-300">•</span>
            <span class="text-[9px] text-slate-400">${formatCompactCount(a.downloads)} ইন্সটল</span>
            ${isDownloaded ? `<span class="text-[9px] text-emerald-600 font-bold"><i class="fas fa-check-circle"></i> ইন্সটল করা</span>` : ''}
          </div>
        </div>
        <button onclick='event.stopPropagation(); openAppDetail("${a.id}")'
          class='shrink-0 text-[11px] font-black px-4 py-1.5 rounded-full ${isDownloaded ? 'bg-slate-100 text-slate-500' : 'bg-emerald-600 text-white'} active:scale-95 transition'>
          ${isDownloaded ? 'খোলা হয়েছে' : 'ইন্সটল'}
        </button>
      </div>`;
    }).join('');
  }
  ...
  // ✅ UPDATED (feature-83): Google Play Store-স্টাইল অ্যাপ প্রোডাক্ট পেজ
  // (আইকন+নাম+ডেভেলপার হেডার, rating/downloads/size/version স্ট্যাট বার, সবুজ ইন্সটল বাটন,
  //  "এই অ্যাপ সম্পর্কে" সেকশন, "রেটিং ও রিভিউ" সেকশন — Play Store-এর অ্যাপ ডিটেইল পেজের রেফারেন্সে)
  // ✅ UPDATED (feature-84): হেডার অ্যাকশন (শেয়ার/রিপোর্ট/অ্যাডমিন-ফিচার্ড) + স্ক্রিনশট গ্যালারি +
  // "নতুন কী আছে" + সিমিলার অ্যাপ + রিভিউ লিস্ট (ডেভেলপার রিপ্লাই সহ) — Play Store প্রোডাক্ট পেজের ধাঁচে
  function openAppDetail(appId) {
    ...
    const avgRating = (app.ratingCount && app.ratingCount > 0) ? (app.ratingSum / app.ratingCount) : 0;

    // হেডার অ্যাকশন বাটন (শেয়ার সবার জন্য, রিপোর্ট নিজের আপলোড না হলে, ফিচার্ড টগল অ্যাডমিনের জন্য)
    const headerActions = document.getElementById('app-detail-header-actions');
    if (headerActions) {
      headerActions.innerHTML = `
        ${isAdminUser ? `<button onclick='adminToggleFeatured("${app.id}")' class='w-9 h-9 rounded-full hover:bg-slate-100 flex items-center justify-center active:scale-90 transition' title='এডিটরস চয়েস টগল'>
          <i class='fas fa-star ${app.isFeatured ? 'text-amber-400' : 'text-slate-300'} text-sm'></i>
        </button>` : ''}
        <button onclick='shareApp("${app.id}")' class='w-9 h-9 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-600 active:scale-90 transition' title='শেয়ার করুন'>
          <i class='fas fa-share-nodes text-sm'></i>
        </button>
        ${!isOwner ? `<button onclick='openAppReportForm("${app.id}")' class='w-9 h-9 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-600 active:scale-90 transition' title='রিপোর্ট করুন'>
          <i class='fas fa-flag text-sm'></i>
        </button>` : ''}
      `;
    }

    // ✅ NEW (feature-84): স্ক্রিনশট গ্যালারি (হরাইজন্টাল স্ক্রল, ট্যাপে লাইটবক্স জুম)
    const screenshots = Array.isArray(app.screenshots) ? app.screenshots : [];
    const screenshotsHtml = screenshots.length ? `
      <div class='flex gap-2 overflow-x-auto mb-4 -mx-5 px-5'>
        ${screenshots.map(url => `
          <img loading="lazy" src="${url}" onclick='openAppScreenshotLightbox("${url}")'
            class='h-44 rounded-xl object-cover shrink-0 cursor-pointer active:scale-95 transition' style='width:auto;max-width:80vw'>
        `).join('')}
      </div>` : '';

    // ✅ NEW (feature-84): "নতুন কী আছে" সেকশন
    const whatsNewHtml = app.whatsNew ? `
      <div class='mb-5'>
        <h4 class='text-[13px] font-black text-slate-900 mb-1.5'><i class="fas fa-sparkles text-emerald-500 mr-1"></i> নতুন কী আছে</h4>
        <p class='text-xs text-slate-600 leading-relaxed'>${escapeHtml(app.whatsNew)}</p>
      </div>` : '';

    // ✅ NEW (feature-84): সিমিলার/রিলেটেড অ্যাপ (একই ক্যাটাগরি, verified, নিজে ছাড়া)
    const similarApps = _appStoreApps.filter(a => a.id !== app.id && a.status === 'verified' && a.category === app.category).slice(0, 8);
    const similarHtml = similarApps.length ? `
      <div class='mb-5 border-t border-slate-100 pt-4'>
        <h4 class='text-[13px] font-black text-slate-900 mb-2'>একই ক্যাটাগরির আরও অ্যাপ</h4>
        <div class='flex gap-3 overflow-x-auto -mx-5 px-5 pb-1'>
          ${similarApps.map(a => {
            const r = (a.ratingCount && a.ratingCount > 0) ? (a.ratingSum / a.ratingCount) : 0;
            return `
            <div onclick='openAppDetail("${a.id}")' class='shrink-0 w-24 text-center cursor-pointer active:scale-95 transition'>
              <div class='w-16 h-16 mx-auto rounded-2xl bg-slate-100 flex items-center justify-center text-xl overflow-hidden mb-1'>
                ${a.iconBase64 ? `<img loading="lazy" src="${a.iconBase64}" class="w-full h-full object-cover">` : '📦'}
              </div>
              <p class='text-[10px] font-bold text-slate-800 line-clamp-1'>${escapeHtml(a.name || '')}</p>
              <p class='text-[9px] text-slate-400'>${r > 0 ? r.toFixed(1) + ' ★' : 'নতুন'}</p>
            </div>`;
          }).join('')}
        </div>
      </div>` : '';

    // ✅ NEW (feature-84): মালিকের জন্য "পোস্ট আপডেট" মিনি ফর্ম (পুনরায় APK আপলোড ছাড়াই ভার্সন নোট)
    const ownerUpdateFormHtml = isOwner ? `
      <div class='mb-4'>
        <button onclick='toggleAppUpdateForm("${app.id}")' id='app-update-toggle-btn-${app.id}'
          class='w-full text-[11px] font-bold text-slate-600 border border-slate-200 rounded-full py-2 active:scale-95 transition'>
          <i class="fas fa-pen mr-1"></i> "নতুন কী আছে" আপডেট করুন
        </button>
        <div class='hidden mt-2 space-y-2' id='app-update-form-${app.id}'>
          <input id='app-update-version-${app.id}' type='text' maxlength='20' placeholder='ভার্সন (যেমন: 1.1.0)' value='${escapeHtml(app.version || '')}'
            class='w-full px-3 py-2 bg-slate-100 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500'>
          <textarea id='app-update-whatsnew-${app.id}' rows='2' maxlength='500' placeholder='এই সংস্করণে কী পরিবর্তন হয়েছে'
            class='w-full px-3 py-2 bg-slate-100 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-none'>${escapeHtml(app.whatsNew || '')}</textarea>
          <button onclick='submitAppUpdate("${app.id}")' class='w-full bg-emerald-600 text-white text-xs font-black py-2.5 rounded-full active:scale-95 transition'>সেভ করুন</button>
        </div>
      </div>` : '';
    ...
    body.innerHTML = `
      <div class='flex items-start gap-3.5 mb-4'>
        <div class='w-20 h-20 rounded-3xl bg-slate-100 flex items-center justify-center text-4xl shrink-0 overflow-hidden shadow-sm'>
          ${app.iconBase64 ? `<img loading="lazy" src="${app.iconBase64}" class="w-full h-full object-cover">` : '📦'}
        </div>
        <div class='flex-1 min-w-0 pt-1'>
          <h3 class='font-black text-lg text-slate-900 leading-tight line-clamp-2'>${escapeHtml(app.name || '')}</h3>
          <p class='text-[11px] text-blue-600 font-semibold mt-0.5 truncate'>${escapeHtml(app.uploaderName || 'অজানা ডেভেলপার')}</p>
          <p class='text-[10px] text-slate-400 mt-0.5'>${escapeHtml(app.category || '')}</p>
        </div>
      </div>

      ${screenshotsHtml}

      <!-- Play Store-স্টাইল স্ট্যাট বার -->
      <div class='flex items-stretch justify-between mb-4 px-1'>
        <div class='flex-1 text-center'>
          <p class='text-sm font-black text-slate-900'>${avgRating > 0 ? avgRating.toFixed(1) : '—'}</p>
          <p class='text-[9px] text-slate-400 mt-0.5'><i class="fas fa-star text-slate-300"></i> ${formatCompactCount(app.ratingCount)} রিভিউ</p>
        </div>
        <div class='w-px bg-slate-100'></div>
        <div class='flex-1 text-center'>
          <p class='text-sm font-black text-slate-900'>${formatCompactCount(app.downloads)}</p>
          <p class='text-[9px] text-slate-400 mt-0.5'><i class="fas fa-download text-slate-300"></i> ডাউনলোড</p>
        </div>
        <div class='w-px bg-slate-100'></div>
        <div class='flex-1 text-center'>
          <p class='text-sm font-black text-slate-900'>${escapeHtml(app.sizeLabel || '-')}</p>
          <p class='text-[9px] text-slate-400 mt-0.5'><i class="fas fa-hard-drive text-slate-300"></i> সাইজ</p>
        </div>
        <div class='w-px bg-slate-100'></div>
        <div class='flex-1 text-center'>
          <p class='text-sm font-black text-slate-900'>${escapeHtml(app.version || '-')}</p>
          <p class='text-[9px] text-slate-400 mt-0.5'><i class="fas fa-code-branch text-slate-300"></i> ভার্সন</p>
        </div>
      </div>
      ${_appStoreStatusBadge(app.status) ? `<div class="mb-3">${_appStoreStatusBadge(app.status)}</div>` : ''}
      ${app.isFeatured ? `<div class="mb-3"><span class="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-1 rounded-full bg-amber-50 text-amber-600"><i class="fas fa-star"></i> এডিটরস চয়েস</span></div>` : ''}

      ${!canDownload ? `<div class="bg-slate-100 rounded-xl p-3 text-center text-[11px] text-slate-500 mb-3">⏳ এই অ্যাপটি এখনো অ্যাডমিন রিভিউয়ের অপেক্ষায়, তাই পাবলিকভাবে ইন্সটল করা যাবে না</div>` : ''}
      <button onclick='downloadApp("${app.id}")' ${canDownload ? '' : 'disabled'}
        class='w-full ${canDownload ? "bg-emerald-600 active:bg-emerald-700" : "bg-slate-300 cursor-not-allowed"} text-white font-black text-sm py-3 rounded-full transition active:scale-95 mb-5'>
        ${isOwner && app.status === 'pending' ? 'টেস্ট ইন্সটল (এখনো পাবলিক নয়)' : 'ইন্সটল করুন'}
      </button>

      <div class='mb-5'>
        <h4 class='text-[13px] font-black text-slate-900 mb-1.5'>এই অ্যাপ সম্পর্কে</h4>
        <p class='text-xs text-slate-600 leading-relaxed'>${escapeHtml(app.longDesc || app.shortDesc || '')}</p>
      </div>

      ${whatsNewHtml}
      ${ownerUpdateFormHtml}

      <div class='bg-amber-50 rounded-xl p-3 flex items-start gap-2 mb-5'>
      ...
      <div class='border-t border-slate-100 pt-4' id='app-review-section-${app.id}'>
        <h4 class='text-[13px] font-black text-slate-900 mb-2'>রেটিং ও রিভিউ</h4>
        ...
        <textarea id='app-review-comment-${app.id}' placeholder='মন্তব্য লিখুন (ঐচ্ছিক, সর্বোচ্চ ৩০০ অক্ষর)'
          class='w-full text-xs border border-slate-200 rounded-xl px-3 py-2 resize-none focus:outline-none focus:ring-1 focus:ring-emerald-500' rows='2' maxlength='300'></textarea>
        <button onclick='submitAppReview("${app.id}")'
          class='mt-2 w-full bg-slate-900 text-white text-xs font-black py-2.5 rounded-full active:scale-95 transition'>
          রিভিউ জমা দিন
        </button>
      </div>` : ''}

      <!-- ✅ NEW (feature-84): রিভিউ লিস্ট (প্রথমবার দেখানো হচ্ছে) — ডেভেলপার রিপ্লাই সহ -->
      <div class='border-t border-slate-100 pt-4 mt-4'>
        <h4 class='text-[13px] font-black text-slate-900 mb-2'>সবার রিভিউ</h4>
        <div id='app-reviews-list-${app.id}'>
          <p class='text-[11px] text-slate-400 text-center py-3'><i class="fas fa-spinner fa-spin"></i> রিভিউ লোড হচ্ছে...</p>
        </div>
      </div>

      ${similarHtml}
      ${isOwner ? `<button onclick='deleteMyAppUpload("${app.id}")' class='w-full mt-3 text-red-500 text-[11px] font-bold py-2 rounded-full border border-red-200 active:scale-95 transition'><i class="fas fa-trash mr-1"></i> আমার এই আপলোড ডিলিট করুন</button>` : ''}
    `;
    document.getElementById('app-detail-modal').classList.remove('hidden');
    loadAndRenderAppReviews(app.id, isOwner);
  }
  ...
  // ✅ NEW (feature-84): স্ক্রিনশট লাইটবক্স
  function openAppScreenshotLightbox(url) {
    document.getElementById('app-screenshot-lightbox-img').src = url;
    document.getElementById('app-screenshot-lightbox').classList.remove('hidden');
  }
  function closeAppScreenshotLightbox() {
    document.getElementById('app-screenshot-lightbox').classList.add('hidden');
    document.getElementById('app-screenshot-lightbox-img').src = '';
  }

  // ✅ NEW (feature-84): রিভিউ লিস্ট লোড + রেন্ডার (রিভিউয়ারের নাম, স্টার, মন্তব্য, ডেভেলপার রিপ্লাই)
  async function loadAndRenderAppReviews(appId, isOwner) {
    const listEl = document.getElementById(`app-reviews-list-${appId}`);
    if (!listEl) return;
    try {
      const snap = await firestore.collection('apps').doc(appId).collection('reviews')
        .orderBy('createdAt', 'desc').limit(20).get();
      if (snap.empty) {
        listEl.innerHTML = `<p class='text-[11px] text-slate-400 text-center py-3'>এখনো কোনো রিভিউ নেই</p>`;
        return;
      }
      listEl.innerHTML = snap.docs.map(d => {
        ...
        <div class='py-2.5 border-b border-slate-50 last:border-0'>
          <div class='flex items-center justify-between'>
            <p class='text-[11px] font-bold text-slate-800'>${escapeHtml(r.reviewerName || 'অজানা ইউজার')}</p>
            <span class='text-amber-400 text-[10px]'>${stars}</span>
          </div>
          ${r.comment ? `<p class='text-[11px] text-slate-600 mt-1 leading-relaxed'>${escapeHtml(r.comment)}</p>` : ''}
          ${r.devReply ? `
            <div class='mt-2 bg-slate-50 rounded-xl p-2.5'>
              <p class='text-[9px] font-black text-emerald-600 mb-0.5'><i class="fas fa-reply mr-1"></i>ডেভেলপারের জবাব</p>
              <p class='text-[10px] text-slate-600 leading-relaxed'>${escapeHtml(r.devReply)}</p>
            </div>` : (isOwner ? `
            <button onclick='toggleDevReplyBox("${appId}","${d.id}")' class='text-[10px] font-bold text-emerald-600 mt-1.5' id='dev-reply-toggle-${d.id}'>জবাব দিন</button>
            <div class='hidden mt-1.5 flex gap-1.5' id='dev-reply-box-${d.id}'>
              <input type='text' maxlength='300' placeholder='আপনার জবাব লিখুন...' id='dev-reply-input-${d.id}'
                class='flex-1 text-[11px] border border-slate-200 rounded-full px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-emerald-500'>
              <button onclick='submitDevReply("${appId}","${d.id}")' class='shrink-0 text-[10px] font-black text-white bg-slate-900 px-3 rounded-full'>পাঠান</button>
            </div>` : '')}
        </div>`;
      }).join('');
    } catch (e) {
      listEl.innerHTML = `<p class='text-[11px] text-red-400 text-center py-3'>রিভিউ লোড করা যায়নি</p>`;
    }
  }

  // ✅ NEW (feature-84): ডেভেলপার রিপ্লাই বক্স টগল
  function toggleDevReplyBox(appId, reviewId) {
    const box = document.getElementById(`dev-reply-box-${reviewId}`);
    const toggleBtn = document.getElementById(`dev-reply-toggle-${reviewId}`);
    if (box) box.classList.remove('hidden');
    if (toggleBtn) toggleBtn.classList.add('hidden');
  }

  // ✅ NEW (feature-84): ডেভেলপার রিপ্লাই সাবমিট — শুধু app owner-ই পারবে (Rules-এ ভ্যালিডেট হয়)
  async function submitDevReply(appId, reviewId) {
    const input = document.getElementById(`dev-reply-input-${reviewId}`);
    const reply = (input ? input.value : '').trim().slice(0, 300);
    if (!reply) { showCartToast('⚠️ জবাব লিখুন'); return; }
    try {
      await firestore.collection('apps').doc(appId).collection('reviews').doc(reviewId).update({
        devReply: reply,
        devReplyAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      showCartToast('✅ জবাব পোস্ট হয়েছে');
      loadAndRenderAppReviews(appId, true);
    } catch (e) {
      showCartToast('❌ জবাব পোস্ট হয়নি: ' + e.message);
    }
  }

  // ✅ NEW (feature-84): শেয়ার — Web Share API, না থাকলে ক্লিপবোর্ড কপি
  function shareApp(appId) {
    const app = _appStoreApps.find(a => a.id === appId);
    if (!app) return;
    const shareText = `"${app.name}" অ্যাপটি দেখুন — পারভেজ স্টোর অ্যাপ স্টোরে: ${window.location.origin}`;
    if (navigator.share) {
      navigator.share({ title: app.name, text: shareText }).catch(() => {});
    } else if (navigator.clipboard) {
      navigator.clipboard.writeText(shareText).then(() => showCartToast('🔗 লিংক কপি হয়েছে'));
    } else {
      showCartToast('⚠️ শেয়ার সাপোর্ট নেই এই ব্রাউজারে');
    }
  }

  // ✅ NEW (feature-84): রিপোর্ট ফর্ম
  function openAppReportForm(appId) {
    if (!currentUser) { showCartToast('⚠️ রিপোর্ট করতে লগইন করুন'); return; }
    _appReportTargetAppId = appId;
    _appReportSelectedReason = null;
    document.querySelectorAll('.app-report-reason-btn').forEach(b => { b.classList.remove('bg-red-500', 'text-white'); b.classList.add('bg-slate-100', 'text-slate-700'); });
    document.getElementById('app-report-detail').value = '';
    document.getElementById('app-report-modal').classList.remove('hidden');
  }
  function closeAppReportForm() {
    document.getElementById('app-report-modal').classList.add('hidden');
  }
  function selectAppReportReason(reason) {
    _appReportSelectedReason = reason;
    document.querySelectorAll('.app-report-reason-btn').forEach(b => {
      const active = b.getAttribute('data-reason') === reason;
  ...
  async function submitAppReportForm() {
    if (!currentUser || !_appReportTargetAppId) { showCartToast('⚠️ লগইন করুন'); return; }
    if (!_appReportSelectedReason) { showCartToast('⚠️ একটি কারণ বেছে নিন'); return; }
    const detail = document.getElementById('app-report-detail').value.trim().slice(0, 200);
    const reason = detail ? `${_appReportSelectedReason} — ${detail}` : _appReportSelectedReason;
    const btn = document.getElementById('app-report-submit-btn');
    if (btn) { btn.disabled = true; btn.classList.add('opacity-50'); }
    try {
      await firestore.collection('app_reports').add({
        uid: currentUser.uid,
        appId: _appReportTargetAppId,
        reason: reason.slice(0, 200),
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      showCartToast('✅ রিপোর্ট জমা হয়েছে, ধন্যবাদ');
      closeAppReportForm();
    } catch (e) {
      showCartToast('❌ রিপোর্ট জমা হয়নি: ' + e.message);
    } finally {
      if (btn) { btn.disabled = false; btn.classList.remove('opacity-50'); }
    }
  }

  // ✅ NEW (feature-84): অ্যাডমিন — এডিটরস চয়েস টগল
  async function adminToggleFeatured(appId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    const app = _appStoreApps.find(a => a.id === appId);
    if (!app) return;
    const newVal = !app.isFeatured;
    try {
      await firestore.collection('apps').doc(appId).update({ isFeatured: newVal });
      app.isFeatured = newVal;
      showCartToast(newVal ? '⭐ এডিটরস চয়েসে যুক্ত হলো' : 'এডিটরস চয়েস থেকে বাদ দেওয়া হলো');
      openAppDetail(appId);
  ...
  // ✅ NEW (feature-84): মালিকের "পোস্ট আপডেট" মিনি ফর্ম টগল + সাবমিট
  function toggleAppUpdateForm(appId) {
    const form = document.getElementById(`app-update-form-${appId}`);
    const btn = document.getElementById(`app-update-toggle-btn-${appId}`);
    if (form) form.classList.toggle('hidden');
    if (btn) btn.classList.toggle('hidden');
  }
  async function submitAppUpdate(appId) {
    const versionEl = document.getElementById(`app-update-version-${appId}`);
    const whatsNewEl = document.getElementById(`app-update-whatsnew-${appId}`);
    const version = (versionEl ? versionEl.value : '').trim().slice(0, 20) || '1.0.0';
    const whatsNew = (whatsNewEl ? whatsNewEl.value : '').trim().slice(0, 500);
    try {
      await firestore.collection('apps').doc(appId).update({ version, whatsNew });
      const cached = _appStoreApps.find(a => a.id === appId);
      if (cached) { cached.version = version; cached.whatsNew = whatsNew; }
      showCartToast('✅ আপডেট নোট সেভ হয়েছে');
      openAppDetail(appId);
    } catch (e) {
      showCartToast('❌ সেভ হয়নি: ' + e.message);
    }
  }
  ...
  async function submitAppReview(appId) {
    ...
    try {
      ...
      batch.set(reviewRef, {
        ...
        reviewerName: currentUser.displayName || 'অজানা ইউজার', // ✅ NEW (feature-84): রিভিউ লিস্টে দেখানোর জন্য
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      ...
      loadAndRenderAppReviews(appId, false); // ✅ NEW (feature-84): নতুন রিভিউ সহ লিস্ট রিফ্রেশ
    } catch (e) {
  ...
  // ✅ NEW (feature-84): স্ক্রিনশট সিলেক্ট হ্যান্ডলার (সর্বোচ্চ APP_SCREENSHOT_MAX টি, Firebase Storage-এ আপলোড হবে)
  function processAppScreenshotUpload(input) {
    if (!input.files || !input.files.length) return;
    const incoming = Array.from(input.files);
    const room = APP_SCREENSHOT_MAX - _appUploadScreenshotFiles.length;
    if (room <= 0) {
      showCartToast(`⚠️ সর্বোচ্চ ${APP_SCREENSHOT_MAX}টি স্ক্রিনশট আপলোড করা যাবে`);
      input.value = '';
      return;
    }
    _appUploadScreenshotFiles = _appUploadScreenshotFiles.concat(incoming.slice(0, room));
    if (incoming.length > room) showCartToast(`⚠️ সর্বোচ্চ ${APP_SCREENSHOT_MAX}টি স্ক্রিনশট রাখা হয়েছে`);
    input.value = '';
    renderAppUploadScreenshotPreviews();
  }

  // ✅ NEW (feature-84): স্ক্রিনশট প্রিভিউ গ্রিড রেন্ডার
  function renderAppUploadScreenshotPreviews() {
    const grid = document.getElementById('app-upload-screenshots-grid');
    if (!grid) return;
    let html = _appUploadScreenshotFiles.map((file, i) => {
      const objUrl = URL.createObjectURL(file);
      return `
        <div class='relative aspect-[9/16]'>
          <img src="${objUrl}" class='w-full h-full object-cover rounded-lg border border-slate-200'>
          <button type='button' onclick='removeAppUploadScreenshot(${i})' class='absolute -top-1 -right-1 w-4 h-4 rounded-full bg-black/60 text-white text-[8px] flex items-center justify-center'>
            <i class='fas fa-times'></i>
          </button>
        </div>`;
    }).join('');
    if (_appUploadScreenshotFiles.length < APP_SCREENSHOT_MAX) {
      html += `
        <label class='flex items-center justify-center aspect-[9/16] border-2 border-slate-300 border-dashed rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition'>
          <i class='fas fa-plus text-slate-400 text-sm'></i>
          <input class='hidden' onchange='processAppScreenshotUpload(this)' type='file' accept='image/*' multiple>
        </label>`;
    }
    grid.innerHTML = html;
  }

  // ✅ NEW (feature-84): নির্দিষ্ট ইনডেক্সের স্ক্রিনশট বাদ দাও
  function removeAppUploadScreenshot(index) {
    _appUploadScreenshotFiles.splice(index, 1);
    renderAppUploadScreenshotPreviews();
  }
  ...
  function _resetAppUploadForm() {
    ...
    _appUploadFileHash = null;
    // ✅ NEW (feature-84): স্ক্রিনশট ও "নতুন কী আছে" ফিল্ড রিসেট
    _appUploadScreenshotFiles = [];
    renderAppUploadScreenshotPreviews();
    const whatsNewEl = document.getElementById('app-upload-whats-new');
    if (whatsNewEl) whatsNewEl.value = '';
  }
  ...
  async function submitAppUpload() {
    ...
    // ✅ NEW (feature-84): "নতুন কী আছে" ঐচ্ছিক ফিল্ড
    const whatsNew = (document.getElementById('app-upload-whats-new')?.value || '').trim().slice(0, 500);
    ...
    try {
      ...
      // ✅ NEW (feature-84): স্ক্রিনশট থাকলে Firebase Storage-এ আপলোড (base64 নয় — Firestore doc সাইজ লিমিট এড়াতে)
      const screenshotUrls = [];
      for (let i = 0; i < _appUploadScreenshotFiles.length; i++) {
        const file = _appUploadScreenshotFiles[i];
        progressLabel.innerText = `স্ক্রিনশট আপলোড হচ্ছে (${i + 1}/${_appUploadScreenshotFiles.length})...`;
        const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
        const shotPath = `app_store/${currentUser.uid}/${appRef.id}/screenshots/${i}_${safeName}`;
        const shotTask = storage.ref(shotPath).put(file, { contentType: file.type });
        await new Promise((res, rej) => shotTask.on('state_changed', null, rej, res));
        screenshotUrls.push(await shotTask.snapshot.ref.getDownloadURL());
      }
      ...
      await appRef.set({
        ...
        ratingSum: 0,
        ratingCount: 0,
        // ✅ NEW (feature-84): স্ক্রিনশট গ্যালারি + "নতুন কী আছে"
      ...
      closeAppUploadForm();
      _resetAppUploadForm();
      // ✅ UPDATED (feature-96): অ্যাডমিন রিভিউ ছাড়াই এখন আপলোডের সাথে সাথে সবাই দেখতে/ডাউনলোড করতে পারবে
      showCartToast('✅ আপলোড সম্পন্ন! অ্যাপটি এখনই সবাই দেখতে ও ডাউনলোড করতে পারবে');
      loadAppStoreApps();
    } catch (e) {
  ...
  // ============================================================
  // ✅ NEW (feature-92): ই-বুক লাইব্রেরি — সবাই পড়তে/আপলোড/ডাউনলোড করতে পারবে
  // (PDF ফাইল Firebase Storage-এ, মেটাডেটা 'ebooks' কালেকশনে — কোনো অ্যাডমিন রিভিউ নেই,
  // আপলোডের সাথে সাথেই পাবলিক হয়ে যায়, ঠিক app store-এর মতো uid/hash-স্কোপড স্টোরেজ পাথ)
  // ============================================================
  let _ebooks              = [];
  let _ebookOnlyMine        = false;
  let _ebookShowDownloads   = false;
  let _ebookDownloadedIds   = new Set();
  let _ebookCategoryFilter  = '';

  async function loadEbooks() {
    const grid = document.getElementById('ebook-store-grid');
    if (grid) grid.innerHTML = `<div class="col-span-2 text-center py-14 text-slate-400"><i class="fas fa-spinner fa-spin text-xl text-slate-300 mb-2 block"></i><p class="text-xs">বই লোড হচ্ছে...</p></div>`;
    try {
      const snap = await firestore.collection('ebooks').orderBy('createdAt', 'desc').limit(200).get();
      _ebooks = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      if (currentUser) {
        try {
          const dlSnap = await firestore.collection('ebook_downloads')
            .where('uid', '==', currentUser.uid).limit(300).get();
          _ebookDownloadedIds = new Set(dlSnap.docs.map(d => d.data().ebookId));
        } catch (_) { /* permission denied হলে নীরবে এড়িয়ে যাই */ }
      }
    } catch (e) {
      if (grid) grid.innerHTML = `<div class="col-span-2 text-center py-10 text-red-400 text-xs">লোড করা যায়নি: ${e.message}</div>`;
    ...
    _renderVisibleEbookGrid();
  }

  function filterEbookCategory(cat) {
    _ebookCategoryFilter = cat || '';
    document.querySelectorAll('.ebook-cat-chip').forEach(btn => {
      const active = (btn.getAttribute('data-cat') || '') === _ebookCategoryFilter;
      btn.classList.toggle('bg-slate-900', active);
      btn.classList.toggle('text-white', active);
      btn.classList.toggle('bg-slate-100', !active);
      btn.classList.toggle('text-slate-600', !active);
  ...
  function _visibleEbooks() {
    return _ebooks.filter(b => {
      if (_ebookCategoryFilter && b.category !== _ebookCategoryFilter) return false;
      if (_ebookShowDownloads) return _ebookDownloadedIds.has(b.id);
      if (_ebookOnlyMine) return currentUser && b.uid === currentUser.uid;
      return true;
    });
  }

  function _renderVisibleEbookGrid() {
    renderEbookGrid(_visibleEbooks());
  }

  function _syncEbookTabStyles() {
    const allBtn = document.getElementById('ebook-tab-all');
    const mineBtn = document.getElementById('ebook-mine-btn');
    const dlBtn = document.getElementById('ebook-dl-btn');
    const isAll = !_ebookOnlyMine && !_ebookShowDownloads;
    [[allBtn, isAll], [mineBtn, _ebookOnlyMine], [dlBtn, _ebookShowDownloads]].forEach(([btn, active]) => {
      if (!btn) return;
      btn.classList.toggle('bg-slate-900', active);
      btn.classList.toggle('text-white', active);
      btn.classList.toggle('bg-slate-100', !active);
  ...
  function setEbookTabAll() {
    _ebookOnlyMine = false;
    _ebookShowDownloads = false;
    const searchInput = document.getElementById('ebook-store-search');
    if (searchInput) searchInput.value = '';
    _syncEbookTabStyles();
    _renderVisibleEbookGrid();
  }

  function toggleEbookOnlyMine() {
    if (!currentUser) { showCartToast('⚠️ লগইন করুন'); return; }
    _ebookOnlyMine = !_ebookOnlyMine;
    if (_ebookOnlyMine) _ebookShowDownloads = false;
    _syncEbookTabStyles();
    document.getElementById('ebook-store-search').value = '';
    _renderVisibleEbookGrid();
  }

  function toggleEbookShowDownloads() {
    if (!currentUser) { showCartToast('⚠️ লগইন করুন'); return; }
    _ebookShowDownloads = !_ebookShowDownloads;
    if (_ebookShowDownloads) _ebookOnlyMine = false;
    _syncEbookTabStyles();
    document.getElementById('ebook-store-search').value = '';
    _renderVisibleEbookGrid();
  }

  function openEbookStore() {
    document.getElementById('ebook-store-modal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    _syncEbookTabStyles();
    loadEbooks();
  }

  function closeEbookStore() {
    document.getElementById('ebook-store-modal').classList.add('hidden');
    document.body.style.overflow = 'auto';
  }

  function renderEbookGrid(books) {
    const grid = document.getElementById('ebook-store-grid');
    if (!grid) return;
    if (books.length === 0) {
      grid.innerHTML = `<div class="col-span-2 text-center py-14 text-slate-400"><i class="fas fa-book text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">কোনো বই পাওয়া যায়নি</p></div>`;
      return;
    }
    grid.innerHTML = books.map(b => {
      const isDownloaded = _ebookDownloadedIds.has(b.id);
      return `
      <div onclick='openEbookDetail("${b.id}")' class='bg-white border border-slate-100 rounded-2xl p-2.5 active:scale-95 transition cursor-pointer shadow-sm'>
        <div class='w-full aspect-[3/4] rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden mb-2'>
          ${b.coverBase64 ? `<img loading="lazy" src="${b.coverBase64}" class="w-full h-full object-cover">` : `<i class='fas fa-book text-3xl text-slate-300'></i>`}
        </div>
        <p class='text-[12px] font-bold text-slate-900 line-clamp-1'>${escapeHtml(b.title || '')}</p>
        <p class='text-[10px] text-slate-400 line-clamp-1 mt-0.5'>${escapeHtml(b.author || 'অজানা লেখক')}</p>
        <div class='flex items-center justify-between mt-1.5'>
          <span class='text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-600'>${escapeHtml(b.category || '')}</span>
          ${isDownloaded ? `<i class="fas fa-check-circle text-emerald-500 text-[10px]"></i>` : `<span class="text-[9px] text-slate-400"><i class="fas fa-download text-[8px]"></i> ${formatCompactCount(b.downloads)}</span>`}
        </div>
      </div>`;
    }).join('');
  }

  function filterEbookGrid(query) {
    const base = _visibleEbooks();
    if (!query.trim()) { renderEbookGrid(base); return; }
    const q = query.toLowerCase();
    renderEbookGrid(base.filter(b => (b.title || '').toLowerCase().includes(q) || (b.author || '').toLowerCase().includes(q) || (b.category || '').toLowerCase().includes(q)));
  }

  let _currentEbookDetailId = null;

  function openEbookDetail(ebookId) {
    const b = _ebooks.find(x => x.id === ebookId);
    if (!b) return;
    _currentEbookDetailId = ebookId;
    const isOwner = currentUser && b.uid === currentUser.uid;
    const body = document.getElementById('ebook-detail-body');
    body.innerHTML = `
      <div class='flex flex-col items-center text-center pt-2 pb-4'>
        <div class='w-32 aspect-[3/4] rounded-2xl bg-slate-100 flex items-center justify-center overflow-hidden mb-3 shadow'>
          ${b.coverBase64 ? `<img src="${b.coverBase64}" class="w-full h-full object-cover">` : `<i class='fas fa-book text-4xl text-slate-300'></i>`}
        </div>
        <h2 class='text-slate-900 font-black text-lg leading-snug'>${escapeHtml(b.title || '')}</h2>
        <p class='text-slate-500 text-xs mt-1'>${escapeHtml(b.author || 'অজানা লেখক')}</p>
        <div class='flex items-center gap-2 mt-2'>
          <span class='text-[10px] font-bold px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600'>${escapeHtml(b.category || '')}</span>
          <span class='text-[10px] text-slate-400'><i class="fas fa-download text-[9px]"></i> ${formatCompactCount(b.downloads)} ডাউনলোড</span>
        </div>
        <p class='text-[10px] text-slate-400 mt-1'>আপলোডার: ${escapeHtml(b.uploaderName || 'অজানা')}</p>
      </div>
      <div class='grid grid-cols-2 gap-2.5 mb-4'>
        <button onclick='readEbook("${b.id}")' class='bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-black text-xs py-3 rounded-full active:scale-95 transition'>
          <i class='fas fa-book-open mr-1'></i> পড়ুন
        </button>
        <button onclick='downloadEbook("${b.id}")' class='bg-slate-100 text-slate-700 font-black text-xs py-3 rounded-full active:scale-95 transition'>
          <i class='fas fa-download mr-1'></i> ডাউনলোড
        </button>
      </div>
      ${b.description ? `
      <div class='border-t border-slate-100 pt-3'>
        <p class='text-slate-900 font-bold text-xs mb-1.5'>বইয়ের সম্পর্কে</p>
        <p class='text-slate-600 text-[12px] leading-relaxed whitespace-pre-wrap'>${escapeHtml(b.description)}</p>
      </div>` : ''}
      ${isOwner ? `
      <button onclick='deleteMyEbookUpload("${b.id}")' class='w-full mt-5 text-red-500 font-bold text-xs py-2.5 rounded-full border border-red-200 active:scale-95 transition'>
        <i class='fas fa-trash mr-1'></i> ডিলিট করুন
      </button>` : ''}
    `;
    document.getElementById('ebook-detail-modal').classList.remove('hidden');
  }

  function closeEbookDetail() {
    document.getElementById('ebook-detail-modal').classList.add('hidden');
  }

  function readEbook(ebookId) {
    const b = _ebooks.find(x => x.id === ebookId);
    if (!b || !b.pdfUrl) { showCartToast('⚠️ ফাইল পাওয়া যায়নি'); return; }
    document.getElementById('ebook-reader-title').innerText = b.title || '';
    document.getElementById('ebook-reader-iframe').src = b.pdfUrl;
    document.getElementById('ebook-reader-download-link').href = b.pdfUrl;
  ...
  function closeEbookReader() {
    ...
    document.getElementById('ebook-reader-iframe').src = '';
  }
  ...
  function downloadEbook(ebookId) {
    ...
    if (currentUser) {
      _ebookDownloadedIds.add(ebookId);
      const dlDocId = currentUser.uid + '_' + ebookId;
      firestore.collection('ebook_downloads').doc(dlDocId).set({
        uid: currentUser.uid,
        ebookId: ebookId,
        downloadedAt: firebase.firestore.FieldValue.serverTimestamp()
      }, { merge: true }).catch(() => {});
  ...
  async function deleteMyEbookUpload(ebookId) {
    if (!confirm('আপনার এই বইটি স্থায়ীভাবে ডিলিট করতে চান?')) return;
    const b = _ebooks.find(x => x.id === ebookId);
    if (!b) return;
    try {
      if (b.pdfPath) await storage.ref(b.pdfPath).delete().catch(() => {});
  ...
  // ---- আপলোড ফর্ম ----
  let _ebookUploadCoverDataUrl = null;
  let _ebookUploadPdfFile      = null;

  function openEbookUploadForm() {
    if (!currentUser) { showCartToast('⚠️ আপলোড করার জন্য লগইন করুন'); return; }
    document.getElementById('ebook-upload-modal').classList.remove('hidden');
  }

  function closeEbookUploadForm() {
    document.getElementById('ebook-upload-modal').classList.add('hidden');
  }

  function handleEbookCoverPick(event) {
    const file = event.target.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) { showCartToast('⚠️ একটি ছবি ফাইল বেছে নিন'); return; }
    const reader = new FileReader();
    reader.onload = e => {
      const dataUrl = e.target.result;
      if (dataUrl.length > 900000) { showCartToast('⚠️ কভার ছবিটি বড় — ছোট/কম রেজোলিউশনের ছবি দিন'); return; }
      _ebookUploadCoverDataUrl = dataUrl;
      const img = document.getElementById('ebook-upload-cover-preview');
      const placeholder = document.getElementById('ebook-upload-cover-placeholder-icon');
      img.src = _ebookUploadCoverDataUrl;
      img.classList.remove('hidden');
      if (placeholder) placeholder.classList.add('hidden');
    };
    reader.readAsDataURL(file);
  ...
  function handleEbookPdfPick(event) {
    const file = event.target.files[0];
    if (!file) return;
    if (file.type !== 'application/pdf' && !file.name.toLowerCase().endsWith('.pdf')) {
      showCartToast('⚠️ শুধুমাত্র PDF ফাইল আপলোড করা যাবে');
      event.target.value = '';
      return;
    }
    const sizeMB = file.size / (1024 * 1024);
    if (sizeMB > 80) {
      showCartToast('⚠️ ফাইলটি অনেক বড় (৮০MB-এর বেশি) — আপলোড সম্ভব হবে না');
      event.target.value = '';
      return;
    }
    _ebookUploadPdfFile = file;
    document.getElementById('ebook-upload-pdf-filename').innerText = `${file.name} (${sizeMB.toFixed(1)} MB) ✓`;
  }

  function _resetEbookUploadForm() {
    document.getElementById('ebook-upload-title').value = '';
    document.getElementById('ebook-upload-author').value = '';
    document.getElementById('ebook-upload-desc').value = '';
    document.getElementById('ebook-upload-pdf-filename').innerText = 'PDF ফাইল বেছে নিতে ট্যাপ করুন';
    document.getElementById('ebook-upload-cover-preview').classList.add('hidden');
    const placeholderIcon = document.getElementById('ebook-upload-cover-placeholder-icon');
    if (placeholderIcon) placeholderIcon.classList.remove('hidden');
    _ebookUploadCoverDataUrl = null;
    _ebookUploadPdfFile = null;
  }

  async function submitEbookUpload() {
    const title = document.getElementById('ebook-upload-title').value.trim();
    const author = document.getElementById('ebook-upload-author').value.trim();
    const category = document.getElementById('ebook-upload-category').value;
    const description = document.getElementById('ebook-upload-desc').value.trim();

    if (!title) { showCartToast('⚠️ বইয়ের নাম লিখুন'); return; }
    if (!_ebookUploadPdfFile) { showCartToast('⚠️ একটি PDF ফাইল বেছে নিন'); return; }
    if (!currentUser) { showCartToast('⚠️ আপলোড করার জন্য লগইন করুন'); return; }

    const submitBtn = document.getElementById('ebook-upload-submit-btn');
    const progressWrap = document.getElementById('ebook-upload-progress-wrap');
    const progressBar = document.getElementById('ebook-upload-progress-bar');
    const progressLabel = document.getElementById('ebook-upload-progress-label');
    submitBtn.disabled = true;
    submitBtn.classList.add('opacity-50');
    progressWrap.classList.remove('hidden');
    progressBar.style.width = '0%';
    progressLabel.innerText = 'আপলোড হচ্ছে...';

    try {
      const bookRef = firestore.collection('ebooks').doc();
      const safeName = _ebookUploadPdfFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
      const storagePath = `ebooks/${currentUser.uid}/${bookRef.id}/${safeName}`;
      const uploadTask = storage.ref(storagePath).put(_ebookUploadPdfFile, { contentType: 'application/pdf' });
      ...
      const sizeMB = _ebookUploadPdfFile.size / (1024 * 1024);
      ...
      await bookRef.set({
        ...
        sizeBytes: _ebookUploadPdfFile.size,
        sizeLabel: sizeMB.toFixed(1) + ' MB',
        coverBase64: _ebookUploadCoverDataUrl || null,
        pdfUrl, pdfPath: storagePath,
        downloads: 0,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });

      closeEbookUploadForm();
      _resetEbookUploadForm();
      showCartToast('✅ বই আপলোড সম্পন্ন হয়েছে!');
      loadEbooks();
    } catch (e) {
      showCartToast('❌ আপলোড ব্যর্থ হয়েছে: ' + e.message);
    } finally {
  ...
  // ============================================================
  // ✅ NEW (feature-94): স্টোর হাউস — কাস্টমারের সম্পূর্ণ প্রাইভেট ব্যক্তিগত ভল্ট
  // (ছবি/ভিডিও/অডিও/কন্টাক্ট নম্বর)। ফাইল কোনো কম্প্রেশন/base64-এনকোডিং ছাড়াই সরাসরি
  // মূল অবস্থায় Firebase Storage-এ আপলোড হয় (storehouse/{uid}/...), তাই ডাউনলোড করলে
  // ঠিক আপলোড করা ফাইলটাই ফেরত পাওয়া যায়। Storage/Firestore Rules — শুধু owner (uid মিলে
  // এমন লগইন ইউজার) পড়তে/লিখতে পারবে, অ্যাডমিনসহ অন্য কারো জন্য কোনো ব্যতিক্রম নেই।
  // ============================================================
  let _storeHouseItems = { photo: [], video: [], audio: [], contact: [] };
  let _storeHouseActiveTab = 'photo';
  let _storeHouseUploadFile = null;
  ...
  async function openStoreHouse() {
    if (!currentUser) { showCartToast('⚠️ দেখার জন্য আগে লগইন করুন'); return; }
    document.getElementById('storehouse-modal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    setStoreHouseTab('photo');
    await loadStoreHouseItems();
  }

  function closeStoreHouse() {
    document.getElementById('storehouse-modal').classList.add('hidden');
    document.body.style.overflow = 'auto';
  }

  async function loadStoreHouseItems() {
    const area = document.getElementById('storehouse-content-area');
    if (area) area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-spinner fa-spin text-xl text-slate-300 mb-2 block"></i><p class="text-xs">লোড হচ্ছে...</p></div>`;
    try {
      // ✅ শুধু নিজের uid-এর আইটেম — একটামাত্র equality where, orderBy নেই তাই কম্পোজিট ইনডেক্স লাগবে না
      const snap = await firestore.collection('storehouse_items').where('uid', '==', currentUser.uid).limit(500).get();
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .sort((a, b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0));
      _storeHouseItems = { photo: [], video: [], audio: [], contact: [] };
      all.forEach(item => { if (_storeHouseItems[item.type]) _storeHouseItems[item.type].push(item); });
    } catch (e) {
      if (area) area.innerHTML = `<div class="text-center py-10 text-red-400 text-xs">লোড করা যায়নি: ${e.message}</div>`;
      return;
    }
    renderStoreHouseContent();
  }

  function setStoreHouseTab(tab) {
    _storeHouseActiveTab = tab;
    document.querySelectorAll('.storehouse-tab-btn').forEach(btn => {
      const active = btn.id === 'storehouse-tab-' + tab;
      btn.classList.toggle('bg-slate-900', active);
      btn.classList.toggle('text-white', active);
      btn.classList.toggle('bg-slate-100', !active);
      btn.classList.toggle('text-slate-600', !active);
  ...
  function renderStoreHouseContent() {
    const area = document.getElementById('storehouse-content-area');
    if (!area) return;
    const items = _storeHouseItems[_storeHouseActiveTab] || [];

    if (_storeHouseActiveTab === 'contact') {
      if (items.length === 0) {
        area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-address-book text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">এখনো কোনো কন্টাক্ট যোগ করা হয়নি</p></div>`;
        return;
      }
      area.innerHTML = `<div class='space-y-2'>` + items.map(c => `
        <div class='flex items-center gap-3 bg-white border border-slate-100 rounded-2xl p-3 shadow-sm'>
          <div class='w-10 h-10 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center shrink-0'><i class='fas fa-user text-sm'></i></div>
          <div class='flex-1 min-w-0'>
            <p class='text-xs font-bold text-slate-900 truncate'>${escapeHtml(c.contactName || '')}</p>
            <p class='text-[11px] text-slate-500 mt-0.5'>${escapeHtml(c.contactPhone || '')}</p>
            ${c.contactNote ? `<p class='text-[10px] text-slate-400 mt-0.5 truncate'>${escapeHtml(c.contactNote)}</p>` : ''}
          </div>
          <a href='tel:${escapeHtml(c.contactPhone || '')}' class='w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0'><i class='fas fa-phone text-xs'></i></a>
          <button onclick='deleteStoreHouseItem("${c.id}")' class='w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center shrink-0'><i class='fas fa-trash text-xs'></i></button>
        </div>`).join('') + `</div>`;
      return;
    }

    if (items.length === 0) {
      const emptyIcon = _storeHouseActiveTab === 'photo' ? 'fa-image' : (_storeHouseActiveTab === 'video' ? 'fa-video' : 'fa-music');
      area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas ${emptyIcon} text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">এখনো কিছু যোগ করা হয়নি</p></div>`;
      return;
    }

    if (_storeHouseActiveTab === 'photo') {
      area.innerHTML = `<div class='grid grid-cols-3 gap-2'>` + items.map(p => `
        <div class='relative aspect-square rounded-xl overflow-hidden bg-slate-100 cursor-pointer' onclick='openStoreHousePhotoLightbox("${p.id}")'>
          <img loading='lazy' src="${p.fileUrl}" class='w-full h-full object-cover'>
        </div>`).join('') + `</div>`;
      return;
    }

    // ভিডিও/অডিও — লিস্ট রো
    const icon = _storeHouseActiveTab === 'video' ? 'fa-file-video' : 'fa-file-audio';
    const iconColor = _storeHouseActiveTab === 'video' ? 'text-purple-500 bg-purple-50' : 'text-pink-500 bg-pink-50';
    area.innerHTML = `<div class='space-y-2'>` + items.map(m => `
      <div class='flex items-center gap-3 bg-white border border-slate-100 rounded-2xl p-3 shadow-sm cursor-pointer' onclick='openStoreHouseMediaPlayer("${m.id}")'>
        <div class='w-10 h-10 rounded-xl ${iconColor} flex items-center justify-center shrink-0'><i class='fas ${icon} text-sm'></i></div>
        <div class='flex-1 min-w-0'>
          <p class='text-xs font-bold text-slate-900 truncate'>${escapeHtml(m.fileName || 'ফাইল')}</p>
          <p class='text-[10px] text-slate-400 mt-0.5'>${escapeHtml(m.sizeLabel || '')}</p>
        </div>
        <button onclick='event.stopPropagation(); deleteStoreHouseItem("${m.id}")' class='w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center shrink-0'><i class='fas fa-trash text-xs'></i></button>
      </div>`).join('') + `</div>`;
  }

  function openStoreHousePhotoLightbox(itemId) {
    const p = _storeHouseItems.photo.find(x => x.id === itemId);
    if (!p) return;
    document.getElementById('storehouse-photo-lightbox-img').src = p.fileUrl;
    document.getElementById('storehouse-photo-lightbox-download').href = p.fileUrl;
    document.getElementById('storehouse-photo-lightbox').classList.remove('hidden');
  }

  function closeStoreHousePhotoLightbox() {
    document.getElementById('storehouse-photo-lightbox').classList.add('hidden');
  }

  function openStoreHouseMediaPlayer(itemId) {
    const m = _storeHouseItems[_storeHouseActiveTab].find(x => x.id === itemId);
    if (!m) return;
    const videoEl = document.getElementById('storehouse-video-player');
    const audioEl = document.getElementById('storehouse-audio-player');
    videoEl.classList.add('hidden'); videoEl.pause(); videoEl.src = '';
    audioEl.classList.add('hidden'); audioEl.pause(); audioEl.src = '';
    if (_storeHouseActiveTab === 'video') {
      videoEl.src = m.fileUrl;
      videoEl.classList.remove('hidden');
    } else {
      audioEl.src = m.fileUrl;
      audioEl.classList.remove('hidden');
    }
    document.getElementById('storehouse-media-player-title').innerText = m.fileName || '';
    document.getElementById('storehouse-media-player-download').href = m.fileUrl;
    document.getElementById('storehouse-media-player-modal').classList.remove('hidden');
  }

  function closeStoreHouseMediaPlayer() {
    document.getElementById('storehouse-media-player-modal').classList.add('hidden');
    const videoEl = document.getElementById('storehouse-video-player');
    const audioEl = document.getElementById('storehouse-audio-player');
  ...
  async function deleteStoreHouseItem(itemId) {
    if (!confirm('এটি স্থায়ীভাবে ডিলিট করতে চান?')) return;
    const item = [..._storeHouseItems.photo, ..._storeHouseItems.video, ..._storeHouseItems.audio, ..._storeHouseItems.contact].find(x => x.id === itemId);
    if (!item) return;
    try {
      if (item.filePath) await storage.ref(item.filePath).delete().catch(() => {});
      await firestore.collection('storehouse_items').doc(itemId).delete();
      showCartToast('🗑️ ডিলিট করা হয়েছে');
      await loadStoreHouseItems();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
  ...
  // ---- আপলোড ফর্ম ----
  function openStoreHouseUploadForm() {
    if (!currentUser) { showCartToast('⚠️ আগে লগইন করুন'); return; }
    _storeHouseUploadFile = null;
    document.getElementById('storehouse-upload-name').value = '';
    document.getElementById('storehouse-contact-name').value = '';
    document.getElementById('storehouse-contact-phone').value = '';
    document.getElementById('storehouse-contact-note').value = '';
    document.getElementById('storehouse-file-input').value = '';

    const fileSection = document.getElementById('storehouse-file-upload-section');
    const contactSection = document.getElementById('storehouse-contact-form-section');
    const titleEl = document.getElementById('storehouse-upload-title');
    const fileInput = document.getElementById('storehouse-file-input');
    const inputLabel = document.getElementById('storehouse-file-input-label');
    const inputIcon = document.getElementById('storehouse-file-input-icon');

    if (_storeHouseActiveTab === 'contact') {
      titleEl.innerText = 'নতুন কন্টাক্ট যোগ করুন';
      fileSection.classList.add('hidden');
      contactSection.classList.remove('hidden');
    } else {
      contactSection.classList.add('hidden');
      fileSection.classList.remove('hidden');
      inputLabel.innerText = 'ফাইল বেছে নিতে ট্যাপ করুন';
      if (_storeHouseActiveTab === 'photo') {
        titleEl.innerText = 'ছবি যোগ করুন';
        fileInput.accept = 'image/*';
        inputIcon.className = 'fas fa-image text-slate-400 text-xl';
      } else if (_storeHouseActiveTab === 'video') {
        titleEl.innerText = 'ভিডিও যোগ করুন';
        fileInput.accept = 'video/*';
        inputIcon.className = 'fas fa-video text-slate-400 text-xl';
      } else {
        titleEl.innerText = 'অডিও যোগ করুন';
        fileInput.accept = 'audio/*';
        inputIcon.className = 'fas fa-music text-slate-400 text-xl';
      }
    }
    document.getElementById('storehouse-upload-modal').classList.remove('hidden');
  }

  function closeStoreHouseUploadForm() {
    document.getElementById('storehouse-upload-modal').classList.add('hidden');
  }

  function handleStoreHouseFilePick(event) {
    const file = event.target.files[0];
    if (!file) return;
    // ✅ NEW (feature-94): মূল ফাইল যেমন আছে তেমনই রাখা হয় — কোনো রিসাইজ/কম্প্রেশন/রি-এনকোড করা হয় না
    const sizeMB = file.size / (1024 * 1024);
    const maxMB = _storeHouseActiveTab === 'video' ? 300 : (_storeHouseActiveTab === 'audio' ? 100 : 25);
    if (sizeMB > maxMB) {
      showCartToast(`⚠️ ফাইলটি অনেক বড় (সর্বোচ্চ ${maxMB}MB)`);
      event.target.value = '';
      return;
    }
    _storeHouseUploadFile = file;
    document.getElementById('storehouse-file-input-label').innerText = `${file.name} (${sizeMB.toFixed(1)} MB) ✓`;
  }

  async function submitStoreHouseUpload() {
    if (!currentUser) { showCartToast('⚠️ আগে লগইন করুন'); return; }
    const submitBtn = document.getElementById('storehouse-upload-submit-btn');

    if (_storeHouseActiveTab === 'contact') {
      const contactName = document.getElementById('storehouse-contact-name').value.trim();
      const contactPhone = document.getElementById('storehouse-contact-phone').value.trim();
      const contactNote = document.getElementById('storehouse-contact-note').value.trim();
      if (!contactName)  { showCartToast('⚠️ নাম লিখুন'); return; }
      if (!contactPhone) { showCartToast('⚠️ ফোন নম্বর দিন'); return; }
      submitBtn.disabled = true; submitBtn.classList.add('opacity-50');
      try {
        await firestore.collection('storehouse_items').add({
          uid: currentUser.uid,
          type: 'contact',
          contactName, contactPhone, contactNote,
          createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        closeStoreHouseUploadForm();
        showCartToast('✅ কন্টাক্ট সংরক্ষণ করা হয়েছে');
        await loadStoreHouseItems();
      } catch (e) {
        showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
      } finally {
    ...
    if (!_storeHouseUploadFile) { showCartToast('⚠️ একটি ফাইল বেছে নিন'); return; }
    const label = document.getElementById('storehouse-upload-name').value.trim();
    const progressWrap = document.getElementById('storehouse-upload-progress-wrap');
    const progressBar = document.getElementById('storehouse-upload-progress-bar');
    const progressLabel = document.getElementById('storehouse-upload-progress-label');
    submitBtn.disabled = true; submitBtn.classList.add('opacity-50');
    progressWrap.classList.remove('hidden');
    progressBar.style.width = '0%';
    progressLabel.innerText = 'আপলোড হচ্ছে...';
    ...
    try {
      const file = _storeHouseUploadFile;
      const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
      const storagePath = `storehouse/${currentUser.uid}/${_storeHouseActiveTab}/${Date.now()}_${safeName}`;
      // ✅ মূল contentType বজায় রেখে, কোনো রূপান্তর ছাড়াই সরাসরি আপলোড
      const uploadTask = storage.ref(storagePath).put(file, { contentType: file.type || 'application/octet-stream' });
      ...
      await firestore.collection('storehouse_items').add({
        uid: currentUser.uid,
        type: _storeHouseActiveTab,
        fileName: label || file.name,
        fileUrl, filePath: storagePath,
        mimeType: file.type || '',
        sizeBytes: file.size,
      ...
      closeStoreHouseUploadForm();
      showCartToast('✅ সংরক্ষণ করা হয়েছে');
      await loadStoreHouseItems();
    } catch (e) {
      showCartToast('❌ আপলোড ব্যর্থ হয়েছে: ' + e.message);
    } finally {
  ...
  // ============================================================
  // ✅ NEW (feature-95): পার্টনারশিপ ইনভেস্টমেন্ট — কাস্টমার পণ্যে ইনভেস্ট করতে পারবে, লাভের অংশ
  // দেখতে পারবে, যেকোনো সময় উইথড্র অনুরোধ করতে পারবে, আর সবাই লাইভ মোট ইনভেস্টমেন্ট দেখতে পারবে।
  // ⚠️ users.balance ফিল্ডে ক্লায়েন্ট থেকে increment করা Rules-এ নিষিদ্ধ (শুধু seller/admin পারে —
  // ইচ্ছাকৃত সিকিউরিটি হার্ডেনিং), তাই টাকা ইনভেস্ট/ফেরত পুরোটাই bKash/Nagad-এর মতো বাইরের
  // পেমেন্টে হয়, ঠিক "জীবন" আবেদন ও অ্যাপ স্টোর রিভিউ কিউ-এর একই pending→admin-approve প্যাটার্নে।
  // ============================================================
  let _investmentOpportunities = [];
  let _myInvestments = [];
  let _investmentActiveTab = 'opportunities';
  let _investmentGlobalUnsub = null;
  let _investmentUploadCoverDataUrl = null;
  let _investmentSelectedOppId = null;

  function _fmtTaka(n) { return '৳' + (Number(n) || 0).toLocaleString('bn-BD'); }

  async function openInvestmentHub() {
    if (!currentUser) { showCartToast('⚠️ দেখতে আগে লগইন করুন'); return; }
    ...
    document.getElementById('investment-admin-add-btn').classList.toggle('hidden', !isAdminUser);
    document.getElementById('investment-tab-admin').classList.toggle('hidden', !isAdminUser);
    _startInvestmentGlobalStatsListener();
    setInvestmentTab('opportunities');
    await loadInvestmentOpportunities();
    await loadMyInvestments();
  }

  function closeInvestmentHub() {
    document.getElementById('investment-modal').classList.add('hidden');
    document.body.style.overflow = 'auto';
    if (_investmentGlobalUnsub) { _investmentGlobalUnsub(); _investmentGlobalUnsub = null; }
  }

  // 🔴 লাইভ মোট ইনভেস্টমেন্ট — onSnapshot দিয়ে রিয়েল-টাইম, সব কাস্টমার একই সংখ্যা দেখবে
  function _startInvestmentGlobalStatsListener() {
    if (_investmentGlobalUnsub) return;
    _investmentGlobalUnsub = firestore.collection('investment_stats').doc('global').onSnapshot(doc => {
      const d = doc.exists ? doc.data() : { totalInvested: 0, totalInvestors: 0 };
      const totalEl = document.getElementById('investment-global-total');
      const investorsEl = document.getElementById('investment-global-investors');
      if (totalEl) totalEl.innerText = _fmtTaka(d.totalInvested);
      if (investorsEl) investorsEl.innerText = (d.totalInvestors || 0).toLocaleString('bn-BD');
    }, () => {});
  }

  function _updateMyInvestmentWalletUI() {
    const active = _myInvestments.filter(i => i.status === 'active' || i.status === 'withdrawal_requested');
    const totalInvested = active.reduce((sum, i) => sum + (i.amount || 0), 0);
    const totalProfit = active.reduce((sum, i) => sum + (i.profitEarned || 0), 0);
    document.getElementById('investment-my-total').innerText = _fmtTaka(totalInvested);
    document.getElementById('investment-my-profit').innerText = _fmtTaka(totalProfit);
  }

  async function loadInvestmentOpportunities() {
    try {
      const snap = await firestore.collection('investment_opportunities').where('status', '==', 'active').limit(100).get();
      _investmentOpportunities = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .sort((a, b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0));
    } catch (e) {
      _investmentOpportunities = [];
    }
    if (_investmentActiveTab === 'opportunities') renderInvestmentContent();
  }

  async function loadMyInvestments() {
    if (!currentUser) return;
    try {
      const snap = await firestore.collection('investments').where('uid', '==', currentUser.uid).limit(200).get();
      _myInvestments = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .sort((a, b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0));
    } catch (e) {
      _myInvestments = [];
    }
    _updateMyInvestmentWalletUI();
    if (_investmentActiveTab === 'mine') renderInvestmentContent();
  }

  function setInvestmentTab(tab) {
    _investmentActiveTab = tab;
    document.querySelectorAll('.investment-tab-btn').forEach(btn => {
      const active = btn.id === 'investment-tab-' + tab;
      btn.classList.toggle('bg-slate-900', active);
      btn.classList.toggle('text-white', active);
    ...
    if (tab === 'admin') { loadInvestmentAdminQueue(); return; }
    renderInvestmentContent();
  }

  const _investmentStatusLabel = {
    pending: { label: 'যাচাই চলছে', cls: 'bg-amber-50 text-amber-700 border-amber-200' },
    active: { label: 'সক্রিয়', cls: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    withdrawal_requested: { label: 'উইথড্র অনুরোধে', cls: 'bg-blue-50 text-blue-700 border-blue-200' },
    withdrawn: { label: 'উইথড্র সম্পন্ন', cls: 'bg-slate-100 text-slate-500 border-slate-200' },
    rejected: { label: 'বাতিল', cls: 'bg-rose-50 text-rose-700 border-rose-200' }
  };

  function renderInvestmentContent() {
    const area = document.getElementById('investment-content-area');
    if (!area) return;

    if (_investmentActiveTab === 'opportunities') {
      if (_investmentOpportunities.length === 0) {
        area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-seedling text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">এখনো কোনো ইনভেস্টমেন্ট সুযোগ যোগ করা হয়নি</p></div>`;
        return;
      }
      area.innerHTML = `<div class='space-y-3'>` + _investmentOpportunities.map(o => `
        <div class='bg-white border border-slate-100 rounded-2xl p-3.5 shadow-sm'>
          <div class='flex items-center gap-3 mb-2.5'>
            <div class='w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden shrink-0'>
              ${o.coverBase64 ? `<img loading="lazy" src="${o.coverBase64}" class="w-full h-full object-cover">` : `<i class='fas fa-seedling text-slate-300 text-lg'></i>`}
            </div>
            <div class='flex-1 min-w-0'>
              <p class='text-xs font-black text-slate-900 truncate'>${escapeHtml(o.title || '')}</p>
              <p class='text-[10px] text-slate-400 mt-0.5 truncate'>${escapeHtml(o.category || '')}</p>
              ${o.expectedProfitLabel ? `<p class='text-[10px] text-teal-600 font-bold mt-0.5'><i class="fas fa-chart-line text-[9px]"></i> ${escapeHtml(o.expectedProfitLabel)}</p>` : ''}
            </div>
          </div>
          ${o.description ? `<p class='text-[11px] text-slate-500 leading-relaxed mb-2.5 line-clamp-2'>${escapeHtml(o.description)}</p>` : ''}
          <div class='flex items-center justify-between mb-3'>
            <span class='text-[10px] text-slate-400'>এই প্রজেক্টে মোট ইনভেস্ট: <b class='text-slate-700'>${_fmtTaka(o.totalInvested)}</b></span>
            <span class='text-[10px] text-slate-400'>সর্বনিম্ন: <b class='text-slate-700'>${_fmtTaka(o.minInvestment)}</b></span>
          </div>
          <button onclick='openInvestmentInvestForm("${o.id}")' class='w-full bg-gradient-to-r from-teal-600 to-emerald-700 text-white font-black text-xs py-2.5 rounded-full active:scale-95 transition'>
            <i class='fas fa-handshake mr-1'></i> ইনভেস্ট করুন
          </button>
        </div>`).join('') + `</div>`;
      return;
    }

    if (_investmentActiveTab === 'mine') {
      if (_myInvestments.length === 0) {
        area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-wallet text-2xl text-slate-300 mb-2 block"></i><p class="text-xs">আপনি এখনো কোনো ইনভেস্টমেন্ট করেননি</p></div>`;
        return;
      }
      area.innerHTML = `<div class='space-y-2.5'>` + _myInvestments.map(i => {
        const st = _investmentStatusLabel[i.status] || _investmentStatusLabel.pending;
        const dateStr = i.createdAt && i.createdAt.toDate ? i.createdAt.toDate().toLocaleDateString('bn-BD') : '';
        return `
        <div class='bg-white border border-slate-100 rounded-2xl p-3.5 shadow-sm'>
          <div class='flex items-center justify-between mb-1.5'>
            <p class='text-xs font-black text-slate-900'>${escapeHtml(i.oppTitle || '')}</p>
            <span class='text-[9px] font-black px-2 py-1 rounded-full border shrink-0 ${st.cls}'>${st.label}</span>
          </div>
          <p class='text-[10px] text-slate-400 mb-2'>${dateStr}</p>
          <div class='grid grid-cols-2 gap-2 mb-2'>
            <div class='bg-slate-50 rounded-xl p-2'>
              <p class='text-[9px] text-slate-400 font-bold'>ইনভেস্ট</p>
              <p class='text-sm font-black text-slate-800'>${_fmtTaka(i.amount)}</p>
            </div>
            <div class='bg-amber-50 rounded-xl p-2'>
              <p class='text-[9px] text-amber-500 font-bold'>লাভ</p>
              <p class='text-sm font-black text-amber-600'>${_fmtTaka(i.profitEarned)}</p>
            </div>
          </div>
          ${i.status === 'active' ? `
          <button onclick='requestInvestmentWithdrawal("${i.id}")' class='w-full bg-slate-900 text-white font-black text-[11px] py-2.5 rounded-full active:scale-95 transition'>
            <i class='fas fa-money-bill-transfer mr-1'></i> উইথড্র অনুরোধ করুন
          </button>` : ''}
          ${i.status === 'withdrawal_requested' ? `<p class='text-center text-[10px] text-blue-500 font-bold'><i class="fas fa-clock"></i> অ্যাডমিন যাচাই করছে, শীঘ্রই টাকা ফেরত পাবেন</p>` : ''}
        </div>`;
      }).join('') + `</div>`;
      return;
    }
  ...
  // ---- ইনভেস্ট করার ফর্ম ----
  function openInvestmentInvestForm(oppId) {
    const o = _investmentOpportunities.find(x => x.id === oppId);
    if (!o) return;
    _investmentSelectedOppId = oppId;
    document.getElementById('investment-invest-opp-title').innerText = o.title || '';
    document.getElementById('investment-min-hint').innerText = `সর্বনিম্ন ইনভেস্ট: ${_fmtTaka(o.minInvestment)}`;
    document.getElementById('investment-amount-input').value = '';
    document.getElementById('investment-amount-input').min = o.minInvestment || 1;
    document.getElementById('investment-trx-id').value = '';
    document.getElementById('investment-invest-modal').classList.remove('hidden');
  }

  function closeInvestmentInvestForm() {
    document.getElementById('investment-invest-modal').classList.add('hidden');
  }

  async function submitInvestment() {
    if (!currentUser) { showCartToast('⚠️ আগে লগইন করুন'); return; }
    const o = _investmentOpportunities.find(x => x.id === _investmentSelectedOppId);
    if (!o) { showCartToast('⚠️ সুযোগটি পাওয়া যায়নি'); return; }
    const amount = parseFloat(document.getElementById('investment-amount-input').value || '0');
    const paymentMethod = document.getElementById('investment-payment-method').value;
    const trxId = document.getElementById('investment-trx-id').value.trim();

    if (!amount || amount < (o.minInvestment || 1)) { showCartToast(`⚠️ সর্বনিম্ন ${_fmtTaka(o.minInvestment)} ইনভেস্ট করতে হবে`); return; }
    if (!trxId) { showCartToast('⚠️ ট্রানজেকশন আইডি দিন'); return; }

    const btn = document.getElementById('investment-submit-btn');
    btn.disabled = true; btn.classList.add('opacity-50');
    try {
      await firestore.collection('investments').add({
        uid: currentUser.uid,
        investorName: currentUser.displayName || 'অজানা',
        oppId: o.id,
        oppTitle: o.title || '',
        amount, paymentMethod, trxId,
        profitEarned: 0,
        status: 'pending',
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      await firestore.collection('admin_notifications').add({
        ...
        message: `${currentUser.displayName || 'একজন কাস্টমার'} — "${o.title}"-এ ${_fmtTaka(amount)} ইনভেস্টের আবেদন করেছেন।`,
        isRead: false,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      closeInvestmentInvestForm();
      showCartToast('✅ আবেদন জমা হয়েছে! অ্যাডমিন যাচাই করার পর সক্রিয় হবে');
      await loadMyInvestments();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    } finally {
  ...
  async function requestInvestmentWithdrawal(investmentId) {
    if (!confirm('এই ইনভেস্টমেন্ট উইথড্র করতে চান? অ্যাডমিন যাচাই করে টাকা ফেরত পাঠাবে।')) return;
    try {
      await firestore.collection('investments').doc(investmentId).update({
        status: 'withdrawal_requested',
        withdrawalRequestedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      showCartToast('✅ উইথড্র অনুরোধ পাঠানো হয়েছে');
      await loadMyInvestments();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  ...
  // ---- অ্যাডমিন: নতুন সুযোগ যোগ করা ----
  function openInvestmentOpportunityForm() {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) { showCartToast('⚠️ অনুমতি নেই'); return; }
    _investmentUploadCoverDataUrl = null;
    document.getElementById('investment-opp-title').value = '';
    document.getElementById('investment-opp-category').value = '';
    document.getElementById('investment-opp-min').value = '';
    document.getElementById('investment-opp-profit-label').value = '';
    document.getElementById('investment-opp-desc').value = '';
    document.getElementById('investment-opp-cover-preview').classList.add('hidden');
    document.getElementById('investment-opp-cover-placeholder').classList.remove('hidden');
    document.getElementById('investment-opportunity-form-modal').classList.remove('hidden');
  }

  function closeInvestmentOpportunityForm() {
    document.getElementById('investment-opportunity-form-modal').classList.add('hidden');
  }

  function handleInvestmentOppCoverPick(event) {
    const file = event.target.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) { showCartToast('⚠️ একটি ছবি ফাইল বেছে নিন'); return; }
    const reader = new FileReader();
    reader.onload = e => {
      const dataUrl = e.target.result;
      if (dataUrl.length > 900000) { showCartToast('⚠️ ছবিটি বড় — ছোট রেজোলিউশনের ছবি দিন'); return; }
      _investmentUploadCoverDataUrl = dataUrl;
      const img = document.getElementById('investment-opp-cover-preview');
      img.src = dataUrl;
      img.classList.remove('hidden');
      document.getElementById('investment-opp-cover-placeholder').classList.add('hidden');
    };
    reader.readAsDataURL(file);
  }

  async function submitInvestmentOpportunity() {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) { showCartToast('⚠️ অনুমতি নেই'); return; }
    const title = document.getElementById('investment-opp-title').value.trim();
    const category = document.getElementById('investment-opp-category').value.trim();
    const minInvestment = parseFloat(document.getElementById('investment-opp-min').value || '0');
    const expectedProfitLabel = document.getElementById('investment-opp-profit-label').value.trim();
    const description = document.getElementById('investment-opp-desc').value.trim();

    if (!title) { showCartToast('⚠️ শিরোনাম লিখুন'); return; }
    if (!minInvestment || minInvestment <= 0) { showCartToast('⚠️ সর্বনিম্ন ইনভেস্টের পরিমাণ দিন'); return; }

    const btn = document.getElementById('investment-opp-submit-btn');
    btn.disabled = true; btn.classList.add('opacity-50');
    try {
      await firestore.collection('investment_opportunities').add({
        title, category, minInvestment, expectedProfitLabel, description,
        coverBase64: _investmentUploadCoverDataUrl || null,
        totalInvested: 0,
        investorCount: 0,
        status: 'active',
      ...
      closeInvestmentOpportunityForm();
      showCartToast('✅ ইনভেস্টমেন্ট সুযোগ যোগ করা হয়েছে');
      await loadInvestmentOpportunities();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    } finally {
  ...
  // ---- অ্যাডমিন: ইনভেস্টমেন্ট রিভিউ কিউ (পেন্ডিং আবেদন + উইথড্র অনুরোধ + সক্রিয় ইনভেস্টমেন্টে লাভ যোগ) ----
  async function loadInvestmentAdminQueue() {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    const area = document.getElementById('investment-content-area');
    area.innerHTML = `<div class="text-center py-10 text-slate-400"><i class="fas fa-spinner fa-spin text-xl text-slate-300 mb-2 block"></i><p class="text-xs">লোড হচ্ছে...</p></div>`;
    try {
      const [pendingSnap, withdrawSnap, activeSnap] = await Promise.all([
        firestore.collection('investments').where('status', '==', 'pending').limit(50).get(),
        firestore.collection('investments').where('status', '==', 'withdrawal_requested').limit(50).get(),
        firestore.collection('investments').where('status', '==', 'active').limit(100).get()
      ]);
      ...
      let html = '';
      html += `<p class='text-[11px] font-black text-slate-400 uppercase tracking-wider mb-2'>নতুন আবেদন (${pending.length})</p>`;
      html += pending.length === 0 ? `<p class='text-[10px] text-slate-400 mb-4'>কোনো পেন্ডিং আবেদন নেই</p>` : `<div class='space-y-2 mb-4'>` + pending.map(p => `
        <div class='bg-white border border-slate-100 rounded-2xl p-3'>
          <p class='text-xs font-black text-slate-800'>${escapeHtml(p.investorName || '')} — ${_fmtTaka(p.amount)}</p>
          <p class='text-[10px] text-slate-400 mt-0.5'>${escapeHtml(p.oppTitle || '')} • ${escapeHtml(p.paymentMethod || '')} • TrxID: ${escapeHtml(p.trxId || '')}</p>
          <div class='grid grid-cols-2 gap-2 mt-2'>
            <button onclick='adminApproveInvestment("${p.id}")' class='bg-emerald-600 text-white text-[10px] font-black py-2 rounded-lg active:scale-95 transition'><i class="fas fa-check"></i> অ্যাপ্রুভ</button>
            <button onclick='adminRejectInvestment("${p.id}")' class='bg-rose-50 text-rose-700 border border-rose-200 text-[10px] font-black py-2 rounded-lg active:scale-95 transition'><i class="fas fa-xmark"></i> রিজেক্ট</button>
          </div>
        </div>`).join('') + `</div>`;

      html += `<p class='text-[11px] font-black text-slate-400 uppercase tracking-wider mb-2'>উইথড্র অনুরোধ (${withdrawals.length})</p>`;
      html += withdrawals.length === 0 ? `<p class='text-[10px] text-slate-400 mb-4'>কোনো উইথড্র অনুরোধ নেই</p>` : `<div class='space-y-2 mb-4'>` + withdrawals.map(w => `
        <div class='bg-white border border-slate-100 rounded-2xl p-3'>
          <p class='text-xs font-black text-slate-800'>${escapeHtml(w.investorName || '')} — ${_fmtTaka((w.amount || 0) + (w.profitEarned || 0))}</p>
          <p class='text-[10px] text-slate-400 mt-0.5'>${escapeHtml(w.oppTitle || '')} (আসল ${_fmtTaka(w.amount)} + লাভ ${_fmtTaka(w.profitEarned)})</p>
          <div class='grid grid-cols-2 gap-2 mt-2'>
            <button onclick='adminApproveInvestmentWithdrawal("${w.id}")' class='bg-emerald-600 text-white text-[10px] font-black py-2 rounded-lg active:scale-95 transition'><i class="fas fa-check"></i> টাকা পাঠানো হয়েছে</button>
            <button onclick='adminRejectInvestmentWithdrawal("${w.id}")' class='bg-rose-50 text-rose-700 border border-rose-200 text-[10px] font-black py-2 rounded-lg active:scale-95 transition'><i class="fas fa-xmark"></i> বাতিল করুন</button>
          </div>
        </div>`).join('') + `</div>`;

      html += `<p class='text-[11px] font-black text-slate-400 uppercase tracking-wider mb-2'>সক্রিয় ইনভেস্টমেন্ট (${active.length}) — লাভ যোগ করুন</p>`;
      html += active.length === 0 ? `<p class='text-[10px] text-slate-400'>কোনো সক্রিয় ইনভেস্টমেন্ট নেই</p>` : `<div class='space-y-2'>` + active.map(a => `
        <div class='bg-white border border-slate-100 rounded-2xl p-3 flex items-center justify-between gap-2'>
          <div class='min-w-0'>
            <p class='text-xs font-black text-slate-800 truncate'>${escapeHtml(a.investorName || '')}</p>
            <p class='text-[10px] text-slate-400 mt-0.5'>${escapeHtml(a.oppTitle || '')} • ইনভেস্ট ${_fmtTaka(a.amount)} • লাভ ${_fmtTaka(a.profitEarned)}</p>
          </div>
          <button onclick='adminAddInvestmentProfit("${a.id}")' class='shrink-0 bg-amber-500 text-white text-[10px] font-black px-3 py-2 rounded-lg active:scale-95 transition'><i class="fas fa-coins"></i> লাভ যোগ</button>
        </div>`).join('') + `</div>`;

      area.innerHTML = html;
    } catch (e) {
      area.innerHTML = `<div class="text-center py-10 text-red-400 text-xs">লোড করা যায়নি: ${e.message}</div>`;
    }
  }

  async function adminApproveInvestment(investmentId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    try {
      const invDoc = await firestore.collection('investments').doc(investmentId).get();
      const inv = invDoc.data();
      if (!inv) return;
      const batch = firestore.batch();
      batch.update(firestore.collection('investments').doc(investmentId), {
        status: 'active',
        approvedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      batch.set(firestore.collection('investment_opportunities').doc(inv.oppId), {
        totalInvested: firebase.firestore.FieldValue.increment(inv.amount || 0),
        investorCount: firebase.firestore.FieldValue.increment(1)
      }, { merge: true });
      batch.set(firestore.collection('investment_stats').doc('global'), {
        totalInvested: firebase.firestore.FieldValue.increment(inv.amount || 0),
        totalInvestors: firebase.firestore.FieldValue.increment(1),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
      }, { merge: true });
      await batch.commit();
      showCartToast('✅ অ্যাপ্রুভ করা হয়েছে');
      loadInvestmentAdminQueue();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  }

  async function adminRejectInvestment(investmentId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    if (!confirm('এই আবেদনটি রিজেক্ট করতে চান?')) return;
    try {
      await firestore.collection('investments').doc(investmentId).update({ status: 'rejected' });
      showCartToast('🗑️ রিজেক্ট করা হয়েছে');
      loadInvestmentAdminQueue();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  ...
  async function adminApproveInvestmentWithdrawal(investmentId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    if (!confirm('বাইরে থেকে (bKash/Nagad) কাস্টমারকে টাকা পাঠানো হয়ে গেছে নিশ্চিত?')) return;
    try {
      const invDoc = await firestore.collection('investments').doc(investmentId).get();
      const inv = invDoc.data();
      if (!inv) return;
      ...
      await batch.commit();
      showCartToast('✅ উইথড্র সম্পন্ন হিসেবে চিহ্নিত হয়েছে');
      loadInvestmentAdminQueue();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  ...
  async function adminRejectInvestmentWithdrawal(investmentId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    try {
      await firestore.collection('investments').doc(investmentId).update({ status: 'active' });
      showCartToast('✅ উইথড্র অনুরোধ বাতিল হয়েছে, ইনভেস্টমেন্ট আবার সক্রিয়');
      loadInvestmentAdminQueue();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  ...
  async function adminAddInvestmentProfit(investmentId) {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    const amountStr = prompt('কত টাকা লাভ যোগ করতে চান?');
    const amount = parseFloat(amountStr || '0');
    if (!amount || amount <= 0) return;
    try {
      await firestore.collection('investments').doc(investmentId).update({
        profitEarned: firebase.firestore.FieldValue.increment(amount)
      });
      showCartToast('✅ লাভ যোগ করা হয়েছে');
      loadInvestmentAdminQueue();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
    }
  ...
  // ============================================================
  // ✅ NEW (feature-97): স্বাস্থ্য তথ্য হাব — সাধারণ রোগ সম্পর্কিত শিক্ষামূলক তথ্য।
  // ⚠️ ইচ্ছাকৃতভাবে এখানে কোনো ওষুধের নাম/ডোজ/ব্যবহারবিধি রাখা হয়নি — শুধু সাধারণ পরিচর্যা তথ্য ও
  // কখন ডাক্তার দেখানো জরুরি সেই নির্দেশনা। ওষুধ সংক্রান্ত সিদ্ধান্ত সবসময় "সেবা" সেকশনের ডাক্তারের
  // পরামর্শ অনুযায়ী নেওয়ার জন্য উৎসাহিত করা হয় — তাই এটা স্ট্যাটিক ডেটা, কোনো Firestore কালেকশন লাগে না।
  // ============================================================
  const HEALTH_TOPICS = [
    { id:'fever', icon:'🌡️', name:'জ্বর', category:'সাধারণ',
      summary:'শরীরের তাপমাত্রা স্বাভাবিকের চেয়ে বেড়ে যাওয়া, প্রায়ই ভাইরাস বা ব্যাকটেরিয়া সংক্রমণের লক্ষণ।',
      care:'পর্যাপ্ত বিশ্রাম নিন, প্রচুর পানি ও তরল খাবার খান, হালকা কাপড় পরুন, ভেজা কাপড় দিয়ে শরীর মুছুন।',
      seekHelp:'জ্বর ৩ দিনের বেশি স্থায়ী হলে, ১০৩°F-এর বেশি হলে, শ্বাসকষ্ট/খিঁচুনি/তীব্র দুর্বলতা বা র‍্যাশ দেখা দিলে সাথে সাথে ডাক্তার দেখান।' },
    { id:'cold_flu', icon:'🤧', name:'সর্দি-কাশি ও ফ্লু', category:'সাধারণ',
      summary:'নাক দিয়ে পানি পড়া, গলা ব্যথা, কাশি ও শরীর ব্যথা সহ ভাইরাল সংক্রমণ।',
      care:'গরম পানি/স্যুপ পান করুন, বিশ্রাম নিন, লবণ পানি দিয়ে গার্গল করুন, ধুলাবালি এড়িয়ে চলুন।',
      seekHelp:'১০ দিনের বেশি স্থায়ী হলে, শ্বাসকষ্ট হলে, বুকে ব্যথা হলে বা কফের সাথে রক্ত দেখা গেলে ডাক্তার দেখান।' },
    { id:'diarrhea', icon:'🚰', name:'ডায়রিয়া', category:'পেট',
      summary:'বারবার পাতলা পায়খানা, সাধারণত সংক্রমণ বা দূষিত খাবার/পানির কারণে হয়।',
      care:'ওরস্যালাইন/পর্যাপ্ত তরল খান যাতে ডিহাইড্রেশন না হয়, হালকা খাবার খান, বাইরের খাবার-পানি এড়িয়ে চলুন।',
      seekHelp:'পায়খানায় রক্ত থাকলে, ২৪ ঘণ্টার বেশি স্থায়ী হলে, তীব্র পেট ব্যথা বা ডিহাইড্রেশনের লক্ষণ (মুখ শুকিয়ে যাওয়া, প্রস্রাব কমে যাওয়া) দেখা দিলে দ্রুত ডাক্তার দেখান।' },
    { id:'gastric', icon:'🔥', name:'গ্যাস্ট্রিক ও বদহজম', category:'পেট',
      summary:'পেটে জ্বালাপোড়া, অতিরিক্ত গ্যাস, বুক জ্বালা — অনিয়মিত খাদ্যাভ্যাস বা মসলাযুক্ত খাবারের কারণে বেশি হয়।',
      care:'নিয়মিত সময়ে খাবার খান, মসলা/তেল কম খান, বেশি সময় খালি পেটে থাকবেন না, ধীরে ধীরে চিবিয়ে খান।',
      seekHelp:'বমিতে রক্ত, কালো পায়খানা, দীর্ঘস্থায়ী তীব্র পেট ব্যথা বা ওজন কমে যাওয়া হলে ডাক্তার দেখান।' },
    { id:'headache', icon:'🤕', name:'মাথাব্যথা ও মাইগ্রেন', category:'সাধারণ',
      summary:'হালকা থেকে তীব্র মাথাব্যথা — মানসিক চাপ, ঘুমের অভাব বা মাইগ্রেনের কারণে হতে পারে।',
      care:'পর্যাপ্ত ঘুম নিন, অন্ধকার/শান্ত ঘরে বিশ্রাম নিন, পানি পান করুন, স্ক্রিন টাইম কমান।',
      seekHelp:'হঠাৎ তীব্র মাথাব্যথা, দৃষ্টি ঝাপসা হওয়া, কথা জড়িয়ে যাওয়া বা শরীরের একপাশ অবশ হয়ে গেলে জরুরি ভিত্তিতে ডাক্তার দেখান।' },
    { id:'asthma', icon:'🫁', name:'হাঁপানি (অ্যাজমা)', category:'শ্বাসতন্ত্র',
      summary:'শ্বাসনালীর দীর্ঘস্থায়ী প্রদাহজনিত রোগ, শ্বাসকষ্ট ও বুকে চাপ অনুভূত হয়।',
      care:'ধুলাবালি/ধোঁয়া/অ্যালার্জেন এড়িয়ে চলুন, নিয়মিত ডাক্তারের চেকআপে থাকুন, ইনহেলার সবসময় সাথে রাখুন।',
      seekHelp:'শ্বাসকষ্ট বেড়ে গেলে, ঠোঁট/নখ নীলচে হয়ে গেলে, কথা বলতে কষ্ট হলে জরুরি বিভাগে যান।' },
    { id:'hypertension', icon:'❤️', name:'উচ্চ রক্তচাপ', category:'হৃদরোগ',
      summary:'রক্তচাপ স্বাভাবিকের চেয়ে বেশি থাকা, দীর্ঘমেয়াদে হৃদরোগ ও স্ট্রোকের ঝুঁকি বাড়ায়।',
      care:'লবণ কম খান, নিয়মিত হাঁটাচলা করুন, ওজন নিয়ন্ত্রণে রাখুন, মানসিক চাপ কমান, নিয়মিত রক্তচাপ মাপুন।',
      seekHelp:'তীব্র মাথাব্যথা, বুকে ব্যথা, দৃষ্টি সমস্যা বা শ্বাসকষ্ট হলে সাথে সাথে ডাক্তার দেখান।' },
    { id:'diabetes', icon:'🩸', name:'ডায়াবেটিস', category:'হরমোন',
      summary:'রক্তে সুগারের মাত্রা বেড়ে যাওয়া, দীর্ঘমেয়াদী পরিচর্যা প্রয়োজন এমন একটি রোগ।',
      care:'নিয়মিত রক্তে সুগার পরীক্ষা করুন, সুষম খাদ্যাভ্যাস মেনে চলুন, নিয়মিত ব্যায়াম করুন, ডাক্তারের পরামর্শ অনুযায়ী চলুন।',
      seekHelp:'অতিরিক্ত তৃষ্ণা/প্রস্রাব, দ্রুত ওজন কমে যাওয়া, ক্ষত না শুকানো বা অজ্ঞান হয়ে যাওয়ার মতো লক্ষণ দেখা দিলে ডাক্তার দেখান।' },
    { id:'skin_allergy', icon:'🌿', name:'চর্মরোগ ও অ্যালার্জি', category:'চর্ম',
      summary:'ত্বকে চুলকানি, র‍্যাশ, লালচে ভাব — অ্যালার্জেন, সংক্রমণ বা আবহাওয়ার কারণে হতে পারে।',
      care:'আক্রান্ত স্থান পরিষ্কার ও শুকনো রাখুন, অ্যালার্জেন এড়িয়ে চলুন, নরম সুতির কাপড় পরুন।',
      seekHelp:'র‍্যাশ দ্রুত ছড়িয়ে পড়লে, পুঁজ/জ্বর হলে বা শ্বাসকষ্টের সাথে র‍্যাশ দেখা দিলে দ্রুত ডাক্তার দেখান।' },
    { id:'toothache', icon:'🦷', name:'দাঁতের ব্যথা', category:'দাঁত',
      summary:'দাঁত বা মাড়িতে ব্যথা, ক্ষয় বা সংক্রমণের কারণে হতে পারে।',
      care:'দিনে দুইবার দাঁত মাজুন, খুব ঠান্ডা/গরম খাবার এড়িয়ে চলুন, লবণ পানি দিয়ে কুলি করুন।',
      seekHelp:'মুখ ফুলে যাওয়া, তীব্র ব্যথা বা জ্বরসহ ব্যথা হলে দ্রুত ডেন্টিস্ট দেখান।' },
    { id:'eye_problem', icon:'👁️', name:'চোখের সমস্যা', category:'চোখ',
      summary:'চোখ লাল হওয়া, চুলকানি, দৃষ্টি ঝাপসা হওয়া — সংক্রমণ, অ্যালার্জি বা চোখের ক্লান্তির কারণে হতে পারে।',
      care:'চোখে হাত দেওয়া কমান, পরিষ্কার পানি দিয়ে চোখ ধুয়ে নিন, স্ক্রিনে বেশিক্ষণ তাকানো এড়িয়ে চলুন।',
      seekHelp:'হঠাৎ দৃষ্টি হারানো, তীব্র ব্যথা বা আঘাতের পর সমস্যা হলে জরুরি ভিত্তিতে ডাক্তার দেখান।' },
    { id:'joint_pain', icon:'🦴', name:'গাঁটে ব্যথা ও আর্থ্রাইটিস', category:'হাড়/গাঁট',
      summary:'জয়েন্টে ব্যথা, ফোলা বা শক্ত হয়ে যাওয়া — বয়সজনিত ক্ষয় বা প্রদাহের কারণে হতে পারে।',
      care:'হালকা ব্যায়াম ও স্ট্রেচিং করুন, ওজন নিয়ন্ত্রণে রাখুন, দীর্ঘক্ষণ এক অবস্থানে না থাকার চেষ্টা করুন।',
      seekHelp:'জয়েন্ট হঠাৎ লাল/গরম হয়ে ফুলে গেলে, জ্বরসহ ব্যথা হলে বা নড়াচড়া করতে না পারলে ডাক্তার দেখান।' },
    { id:'mental_health', icon:'🧠', name:'মানসিক স্বাস্থ্য (উদ্বেগ/বিষণ্নতা)', category:'মানসিক',
      summary:'দীর্ঘস্থায়ী মন খারাপ, উদ্বেগ, ঘুমের সমস্যা বা আগ্রহ হারিয়ে ফেলা মানসিক স্বাস্থ্যের ইঙ্গিত হতে পারে।',
      care:'পরিচিত মানুষদের সাথে কথা বলুন, নিয়মিত ঘুম ও রুটিন মেনে চলুন, একা একা সব সামলানোর চেষ্টা করবেন না।',
      seekHelp:'নিজেকে ক্ষতি করার চিন্তা এলে বা দৈনন্দিন কাজ চালিয়ে যাওয়া কঠিন হয়ে পড়লে দ্রুত একজন মানসিক স্বাস্থ্য বিশেষজ্ঞের সাহায্য নিন।' },
    { id:'pregnancy_care', icon:'🤰', name:'গর্ভাবস্থা পরিচর্যা', category:'প্রসূতি',
      summary:'গর্ভাবস্থায় মা ও শিশুর সুস্থতার জন্য নিয়মিত পরিচর্যা ও চেকআপ প্রয়োজন।',
      care:'সুষম খাবার খান, পর্যাপ্ত বিশ্রাম নিন, নিয়মিত প্রসবপূর্ব চেকআপে যান, ভারী কাজ এড়িয়ে চলুন।',
      seekHelp:'রক্তক্ষরণ, তীব্র পেট ব্যথা, শিশুর নড়াচড়া কমে যাওয়া বা মাথা ঘোরার মতো লক্ষণে সাথে সাথে ডাক্তার দেখান।' },
    { id:'child_health', icon:'🧒', name:'শিশু স্বাস্থ্য', category:'শিশু',
      summary:'শিশুদের বৃদ্ধি, টিকা ও সাধারণ অসুস্থতা সম্পর্কিত পরিচর্যা।',
      care:'নিয়মিত টিকাদান সময়সূচি মেনে চলুন, পুষ্টিকর খাবার দিন, পরিষ্কার-পরিচ্ছন্নতা বজায় রাখুন।',
      seekHelp:'উচ্চ জ্বর, খাওয়া বন্ধ করে দেওয়া, নিস্তেজ হয়ে পড়া বা শ্বাসকষ্ট হলে সাথে সাথে শিশু বিশেষজ্ঞ দেখান।' }
  ];

  function openHealthHub() {
    document.getElementById('health-modal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    _renderHealthCategoryChips();
    renderHealthList();
  }
  ...
  let _healthActiveCategory = 'সব';

  function _renderHealthCategoryChips() {
    const wrap = document.getElementById('health-category-chips');
    if (!wrap) return;
    const categories = ['সব', ...new Set(HEALTH_TOPICS.map(t => t.category))];
    wrap.innerHTML = categories.map(cat => `
      <button onclick="setHealthCategory('${cat}')" class="health-cat-chip shrink-0 text-[11px] font-bold px-3.5 py-1.5 rounded-full ${cat === _healthActiveCategory ? 'bg-red-600 text-white' : 'bg-white text-slate-600 border border-slate-200'} active:scale-95 transition">${cat}</button>
    `).join('');
  }

  function setHealthCategory(cat) {
    _healthActiveCategory = cat;
    _renderHealthCategoryChips();
    renderHealthList();
  }

  function renderHealthList() {
    const area = document.getElementById('health-list-area');
    if (!area) return;
    const q = (document.getElementById('health-search-input')?.value || '').trim().toLowerCase();
    let list = HEALTH_TOPICS;
    if (_healthActiveCategory && _healthActiveCategory !== 'সব') list = list.filter(t => t.category === _healthActiveCategory);
    if (q) list = list.filter(t => t.name.toLowerCase().includes(q) || t.category.toLowerCase().includes(q));
    if (!list.length) {
      area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-magnifying-glass text-2xl mb-2 block"></i><p class="text-xs">কিছু পাওয়া যায়নি</p></div>`;
      return;
    }
    area.innerHTML = list.map(t => `
      <button onclick="openHealthTopicDetail('${t.id}')" class="w-full flex items-center gap-3 bg-white rounded-2xl p-3.5 mb-2.5 shadow-sm border border-slate-100 text-left active:scale-[0.98] transition">
        <div class="w-11 h-11 rounded-xl bg-red-50 flex items-center justify-center text-xl shrink-0">${t.icon}</div>
        <div class="flex-1 min-w-0">
          <p class="text-xs font-black text-slate-800">${t.name}</p>
          <p class="text-[10px] text-slate-400 mt-0.5">${t.category}</p>
        </div>
  ...
  function openHealthTopicDetail(id) {
    const t = HEALTH_TOPICS.find(x => x.id === id);
    if (!t) return;
    document.getElementById('health-detail-title').innerText = t.icon + ' ' + t.name;
    document.getElementById('health-detail-summary').innerText = t.summary;
    document.getElementById('health-detail-care').innerText = t.care;
    document.getElementById('health-detail-seekhelp').innerText = t.seekHelp;
    document.getElementById('health-detail-sheet').classList.remove('hidden');
  }

  // ============================================================
  // ✅ NEW (feature-97): সেবা — বিশ্বের ডাক্তারদের ডিরেক্টরি। অ্যাডমিন যাচাই করে যোগ করেন,
  // যেকোনো ব্যবহারকারী ব্রাউজ করে সরাসরি কল/হোয়াটসঅ্যাপ/ইমেইল করতে পারবেন — সরাসরি appointment
  // booking বা in-app কল নয়, বরং কন্টাক্ট ডিটেইলস দিয়ে সংযোগ করিয়ে দেওয়া (দেশীয় নাম্বার হলে tel:,
  // হোয়াটসঅ্যাপ হলে wa.me লিংক, ইমেইল হলে mailto: ব্যবহার হয় — সবই ডিভাইসের নিজস্ব অ্যাপ খুলবে)
  // ============================================================
  let _doctorsList = [];
  let _doctorActiveSpecialty = 'সব';

  async function openDoctorDirectory() {
    document.getElementById('doctor-modal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    const isAdminUser = currentUser && ADMIN_EMAILS.includes(currentUser.email);
    document.getElementById('doctor-admin-add-btn').classList.toggle('hidden', !isAdminUser);
    await loadDoctors();
  }

  function closeDoctorDirectory() {
    document.getElementById('doctor-modal').classList.add('hidden');
    document.body.style.overflow = 'auto';
  }

  async function loadDoctors() {
    const area = document.getElementById('doctor-list-area');
    try {
      const snap = await firestore.collection('doctors').where('status', '==', 'active').limit(300).get();
      _doctorsList = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    } catch (e) {
      _doctorsList = [];
    }
    _renderDoctorSpecialtyChips();
    renderDoctorList();
  }

  function _renderDoctorSpecialtyChips() {
    const wrap = document.getElementById('doctor-specialty-chips');
    if (!wrap) return;
    const specialties = ['সব', ...new Set(_doctorsList.map(d => d.specialty).filter(Boolean))];
    wrap.innerHTML = specialties.map(s => `
      <button onclick="setDoctorSpecialty('${s.replace(/'/g, "\\'")}')" class="doctor-spec-chip shrink-0 text-[11px] font-bold px-3.5 py-1.5 rounded-full ${s === _doctorActiveSpecialty ? 'bg-cyan-600 text-white' : 'bg-white text-slate-600 border border-slate-200'} active:scale-95 transition">${s}</button>
    `).join('');
  }

  function setDoctorSpecialty(s) {
    _doctorActiveSpecialty = s;
    _renderDoctorSpecialtyChips();
    renderDoctorList();
  }
  ...
  function renderDoctorList() {
    const area = document.getElementById('doctor-list-area');
    if (!area) return;
    const q = (document.getElementById('doctor-search-input')?.value || '').trim().toLowerCase();
    let list = _doctorsList;
    if (_doctorActiveSpecialty && _doctorActiveSpecialty !== 'সব') list = list.filter(d => d.specialty === _doctorActiveSpecialty);
    if (q) list = list.filter(d =>
      (d.name || '').toLowerCase().includes(q) ||
      (d.specialty || '').toLowerCase().includes(q) ||
      (d.country || '').toLowerCase().includes(q));
    if (!list.length) {
      area.innerHTML = `<div class="text-center py-14 text-slate-400"><i class="fas fa-user-doctor text-2xl mb-2 block"></i><p class="text-xs">এখনো কোনো ডাক্তার যোগ করা হয়নি</p></div>`;
      return;
    }
    area.innerHTML = list.map(d => `
      <button onclick="openDoctorDetail('${d.id}')" class="w-full flex items-center gap-3 bg-white rounded-2xl p-3.5 mb-2.5 shadow-sm border border-slate-100 text-left active:scale-[0.98] transition">
        <div class="w-12 h-12 rounded-xl bg-cyan-50 text-cyan-600 flex items-center justify-center shrink-0 overflow-hidden">
          ${d.photoUrl ? `<img src="${d.photoUrl}" class="w-full h-full object-cover">` : `<i class="fas fa-user-doctor"></i>`}
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-xs font-black text-slate-800 truncate">${d.name || ''}</p>
          <p class="text-[10px] text-cyan-700 font-bold mt-0.5 truncate">${d.specialty || ''}</p>
          <p class="text-[9px] text-slate-400 mt-0.5"><i class="fas fa-location-dot"></i> ${d.country || ''}</p>
        </div>
        <i class="fas fa-chevron-right text-slate-300 text-xs"></i>
      </button>
    `).join('');
  ...
  let _activeDoctorContact = null;

  function openDoctorDetail(id) {
    const d = _doctorsList.find(x => x.id === id);
    if (!d) return;
    _activeDoctorContact = d;
    document.getElementById('doctor-detail-name').innerText = d.name || '';
    document.getElementById('doctor-detail-specialty').innerText = d.specialty || '';
    document.getElementById('doctor-detail-country').innerText = d.country || '';
    document.getElementById('doctor-detail-bio').innerText = d.bio || '';
    const avatarWrap = document.getElementById('doctor-detail-avatar-wrap');
    avatarWrap.innerHTML = d.photoUrl ? `<img src="${d.photoUrl}" class="w-full h-full object-cover">` : `<i class="fas fa-user-doctor text-2xl"></i>`;
    document.getElementById('doctor-detail-call-btn').classList.toggle('hidden', !d.phone);
    document.getElementById('doctor-detail-whatsapp-btn').classList.toggle('hidden', !d.whatsapp);
    document.getElementById('doctor-detail-email-btn').classList.toggle('hidden', !d.email);
    document.getElementById('doctor-detail-sheet').classList.remove('hidden');
  }

  function callDoctor() {
    if (!_activeDoctorContact || !_activeDoctorContact.phone) return;
    window.location.href = 'tel:' + _activeDoctorContact.phone;
  }

  function whatsappDoctor() {
    if (!_activeDoctorContact || !_activeDoctorContact.whatsapp) return;
    const num = _activeDoctorContact.whatsapp.replace(/[^0-9]/g, '');
    window.open('https://wa.me/' + num, '_blank');
  }

  function emailDoctor() {
    if (!_activeDoctorContact || !_activeDoctorContact.email) return;
    window.location.href = 'mailto:' + _activeDoctorContact.email;
  }

  // --- অ্যাডমিন: নতুন ডাক্তার যোগ করার ফর্ম ---
  ...
  function closeAddDoctorForm() {
    ...
    ['doctor-form-name', 'doctor-form-specialty', 'doctor-form-country', 'doctor-form-phone',
     'doctor-form-whatsapp', 'doctor-form-email', 'doctor-form-photo', 'doctor-form-bio'].forEach(id => {
  ...
  async function submitNewDoctor() {
    ...
    const specialty = document.getElementById('doctor-form-specialty').value.trim();
    const country = document.getElementById('doctor-form-country').value.trim();
    const phone = document.getElementById('doctor-form-phone').value.trim();
    const whatsapp = document.getElementById('doctor-form-whatsapp').value.trim();
    const email = document.getElementById('doctor-form-email').value.trim();
    const photoUrl = document.getElementById('doctor-form-photo').value.trim();
    const bio = document.getElementById('doctor-form-bio').value.trim();
    if (!name || !specialty || !country || !phone) {
      showCartToast('⚠️ নাম, বিশেষজ্ঞতা, দেশ ও ফোন নম্বর আবশ্যক');
      return;
    }
    const btn = document.getElementById('doctor-form-submit-btn');
    btn.disabled = true; btn.classList.add('opacity-50');
    try {
      await firestore.collection('doctors').add({
        name, specialty, country, phone,
        whatsapp: whatsapp || '', email: email || '', photoUrl: photoUrl || '', bio: bio || '',
        status: 'active', addedBy: currentUser.uid,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      showCartToast('✅ ডাক্তার যোগ করা হয়েছে');
      closeAddDoctorForm();
      await loadDoctors();
    } catch (e) {
      showCartToast('❌ ব্যর্থ হয়েছে: ' + e.message);
  ...
  function renderTejFeedList(type) {
    ...
    if (type === 'x' || type === 'y') {
      docs = docs.filter(d => d.data().postCategory === type);
    } else if (type !== 'all') {
    ...
    // ✅ NEW: ফেসবুকের মতো — মেইন ফিডে (শুধু 'সব' ফিল্টারে, কোনো অথর/হ্যাশট্যাগ ফিল্টার না থাকলে)
    // কয়েকটা পোস্টের পরে একটা "শর্ট" (রিলস স্টাইল) হরাইজন্টাল স্লাইড-করা রো দেখানো হয়
    const showShortsRow = type === 'all' && !_tejAuthorFilter && !_tejHashtagFilter;
    const shortsRowHtml = showShortsRow ? renderTejShortsInlineRow() : '';
    const shortsRowAfterIndex = Math.min(3, docs.length) - 1; // ৩টা পোস্টের পরে (পোস্ট কম থাকলে শেষেরটার পরে)

    let feedHtml = '';
    docs.forEach((doc, idx) => {
      feedHtml += renderTejCard(doc.id, doc.data());
      if (shortsRowHtml && idx === shortsRowAfterIndex) {
        feedHtml += shortsRowHtml;
      }
    });
    list.innerHTML = feedHtml;

    // ✅ NEW (feature-88): শর্টসের মতোই — স্ক্রল করে যে ভিডিওটা স্ক্রিনে ৬০%+ দেখা যাচ্ছে
    // সেটা অটো-প্লে হবে, স্ক্রিন থেকে সরে গেলে পজ হয়ে যাবে। ইউজার ম্যানুয়ালি পজ করলে
    // (userPaused flag) অটো-প্লে সেটা ওভাররাইড করবে না।
    if (_tejFeedVideoObserver) _tejFeedVideoObserver.disconnect();
    _tejFeedVideoObserver = new IntersectionObserver((entries) => {
    ...
    list.querySelectorAll('video[id^="tej-feedvid-"]').forEach(v => _tejFeedVideoObserver.observe(v));
  }

  // ✅ NEW: ফিডে বসানোর জন্য শর্ট ভিডিওর ছোট হরাইজন্টাল প্রিভিউ রো (ডানে থেকে বামে স্ক্রল করা যায়)
  function renderTejShortsInlineRow() {
    const now = Date.now();
    const videos = _tejLastDocs.filter(d => {
      const data = d.data();
    ...
    const cardsHtml = videos.map(doc => {
      const id = doc.id;
      const d = doc.data();
      return `<div class='shrink-0 w-28 h-44 rounded-xl overflow-hidden relative cursor-pointer bg-slate-900 snap-start' onclick='openTejShorts("${id}")'>
        <video src='${d.mediaData}' class='w-full h-full object-cover pointer-events-none' muted preload='metadata' playsinline></video>
        <div class='absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent h-2/3 pointer-events-none'></div>
        <div class='absolute top-1.5 left-1.5 w-6 h-6 rounded-full bg-white/25 backdrop-blur-sm flex items-center justify-center'>
          <i class='fas fa-play text-white text-[9px]'></i>
        </div>
        <div class='absolute inset-x-0 bottom-0 p-1.5'>
          <p class='text-white text-[9px] font-bold truncate'>${escapeHtml(d.userName || 'অজানা')}</p>
        </div>
      </div>`;
    }).join('');
    return `<div class='bg-white rounded-2xl shadow-sm border border-slate-100 p-3 mb-0'>
      <h3 class='text-[11px] font-black text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5'>
        <i class='fas fa-clapperboard text-purple-500'></i> শর্ট ভিডিও
      </h3>
      <div class='flex gap-2 overflow-x-auto snap-x scrollbar-hide pb-0.5'>
        ${cardsHtml}
        <div class='shrink-0 w-28 h-44 rounded-xl bg-slate-50 border border-slate-100 flex flex-col items-center justify-center gap-1.5 cursor-pointer snap-start' onclick='openTejShorts()'>
          <div class='w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-500'><i class='fas fa-chevron-right text-xs'></i></div>
          <p class='text-[10px] font-bold text-slate-400'>সব দেখুন</p>
        </div>
      </div>
    </div>`;
  }
  ...
  function filterTejByAuthor(uid, name, photo) {
    ...
    document.getElementById('tej-feed-scroll-wrap')?.scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  function renderTejShortsList(startId) {
    const scroll = document.getElementById('tej-shorts-scroll');
    const now = Date.now();
    ...
    // ✅ ইনলাইন শর্ট রো থেকে নির্দিষ্ট ভিডিওতে ক্লিক করলে সেটাকেই তালিকার শুরুতে আনা হয়,
    // যাতে ওপেন করার সাথে সাথে ঠিক সেই ভিডিওটা থেকেই দেখা শুরু হয়
    if (startId) {
      const idx = videos.findIndex(doc => doc.id === startId);
      if (idx > 0) {
        const [picked] = videos.splice(idx, 1);
        videos.unshift(picked);
      }
    }
    ...
    scroll.scrollTop = 0; // যেহেতু startId থাকলে সেটাকেই সবার আগে বসানো হয়েছে, শুরু থেকেই সেটা দেখা যাবে
    ...
    _tejShortsObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (!video) return;
        if (entry.isIntersecting && entry.intersectionRatio >= 0.6) {
          // ✅ UPDATED (feature-86): ইউজার ম্যানুয়ালি পজ করলে (userPaused flag) অটো-প্লে যেন সেটা ওভাররাইড না করে
          if (video.dataset.userPaused !== 'true') video.play().catch(() => {});
        } else {
          video.pause();
        }
  ...
  function renderTejShortCard(id, d) {
    ...
    const avatar = d.userPhoto
      ? `<img loading="lazy" src='${d.userPhoto}' class='w-9 h-9 rounded-full object-cover border-2 border-white/40' onerror="this.onerror=null;this.src='https://placehold.co/36x36/f1f5f9/64748b?text=?'">`
    ...
    // ✅ UPDATED (feature-86): ট্যাপে এখন mute না, প্লে/পজ টগল হয় — মিউট আলাদা আইকন বাটনে সরানো হয়েছে,
    // আর নিচে একটা ড্র্যাগযোগ্য প্রোগ্রেস বার যোগ হয়েছে (টেনে টেনে দেখার জন্য)
    return `<div class='tej-short-slide w-full h-full snap-start relative flex items-end' data-id='${id}' style='scroll-snap-stop:always'>
      <video id='tej-short-video-${id}' src='${d.mediaData}' class='absolute inset-0 w-full h-full object-cover bg-black'
        loop playsinline muted ontimeupdate='_tejShortProgressUpdate("${id}")' onclick='toggleTejShortPlay("${id}")'></video>
      <!-- ✅ NEW (feature-86): পজ করলে মাঝখানে প্লে-আইকন দেখাবে (টেপ করলে টগল হয়) -->
      <div id='tej-short-pauseicon-${id}' class='hidden absolute inset-0 flex items-center justify-center pointer-events-none z-10'>
        <div class='w-16 h-16 rounded-full bg-black/40 flex items-center justify-center'>
          <i class='fas fa-play text-white text-2xl ml-1'></i>
        </div>
      </div>
      <!-- ✅ NEW (feature-86): মিউট/আনমিউট আলাদা বাটন -->
      <button onclick='event.stopPropagation(); toggleTejShortMute("${id}")'
        class='absolute top-4 right-4 z-20 w-9 h-9 rounded-full bg-black/40 flex items-center justify-center text-white'>
        <i class='fas fa-volume-xmark text-sm' id='tej-short-muteicon-${id}'></i>
      </button>
      ...
      <div class='relative z-10 w-full'>
        <!-- ✅ NEW (feature-86): ড্র্যাগযোগ্য প্রোগ্রেস/সিক বার — ট্যাপ বা টেনে যেকোনো জায়গায় যাওয়া যাবে -->
        <div class='px-4 pb-2.5' id='tej-short-progresswrap-${id}' onpointerdown='_tejShortSeekStart(event, "${id}")'>
          <div class='h-1 w-full bg-white/25 rounded-full overflow-hidden'>
            <div id='tej-short-progressbar-${id}' class='h-full bg-white rounded-full' style='width:0%'></div>
          </div>
        </div>
        <div class='w-full p-4 pt-0 pb-6 flex items-end justify-between gap-3'>
          <div class='flex-1 min-w-0'>
  ...
  // ✅ NEW (feature-86): ট্যাপে প্লে/পজ টগল — স্ক্রল-ভিত্তিক অটো-প্লে যাতে ম্যানুয়াল পজ ওভাররাইড না করে,
  // সেজন্য video.dataset.userPaused ফ্ল্যাগ ব্যবহার করা হয়েছে (IntersectionObserver এটা চেক করে)
  function toggleTejShortPlay(id) {
    const video = document.getElementById(`tej-short-video-${id}`);
    const icon = document.getElementById(`tej-short-pauseicon-${id}`);
    if (!video) return;
    if (video.paused) {
      video.play().catch(() => {});
  ...
  // ✅ NEW (feature-86): মিউট/আনমিউট টগল (এখন ভিডিওতে ট্যাপ থেকে আলাদা)
  function toggleTejShortMute(id) {
    const video = document.getElementById(`tej-short-video-${id}`);
    const icon = document.getElementById(`tej-short-muteicon-${id}`);
    if (!video) return;
  ...
  // ✅ NEW (feature-86): timeupdate ইভেন্টে প্রোগ্রেস বার আপডেট
  function _tejShortProgressUpdate(id, forcedPct) {
    const bar = document.getElementById(`tej-short-progressbar-${id}`);
    if (!bar) return;
    let pct = forcedPct;
    if (pct == null) {
      const video = document.getElementById(`tej-short-video-${id}`);
      pct = (video && video.duration) ? (video.currentTime / video.duration) * 100 : 0;
    }
    bar.style.width = Math.min(100, Math.max(0, pct)) + '%';
  }

  // ✅ NEW (feature-86): প্রোগ্রেস বারে ট্যাপ/ড্র্যাগ করে ভিডিওর যেকোনো জায়গায় যাওয়া (scrub/seek)
  function _tejShortSeekToClientX(id, clientX) {
    const video = document.getElementById(`tej-short-video-${id}`);
    const wrap = document.getElementById(`tej-short-progresswrap-${id}`);
    if (!video || !wrap || !video.duration) return;
    const rect = wrap.getBoundingClientRect();
    let ratio = (clientX - rect.left) / rect.width;
    ratio = Math.min(1, Math.max(0, ratio));
    video.currentTime = ratio * video.duration;
    _tejShortProgressUpdate(id, ratio * 100);
  }

  function _tejShortSeekStart(event, id) {
    event.stopPropagation();
    const getX = (e) => (e.touches && e.touches[0] ? e.touches[0].clientX : e.clientX);
    _tejShortSeekToClientX(id, getX(event));
    const onMove = (e) => { e.preventDefault(); _tejShortSeekToClientX(id, getX(e)); };
    const onUp = () => {
  ...
  // ✅ NEW (feature-87): ফিড কার্ডের ইনলাইন ভিডিও প্রিভিউয়ের জন্য প্লে/পজ, মিউট, স্ক্রাব —
  // shorts-এর লজিকের মতোই, কিন্তু আলাদা element id প্রিফিক্স (tej-feedvid-) ব্যবহার করে,
  // যাতে একই পোস্ট fullscreen শর্টসেও (tej-short-video-) আলাদাভাবে খোলা থাকলে আইডি ক্ল্যাশ না করে
  function toggleTejFeedVideoPlay(id) {
    const video = document.getElementById(`tej-feedvid-${id}`);
    const icon = document.getElementById(`tej-feedvid-playicon-${id}`);
    if (!video) return;
    if (video.paused) {
      video.play().catch(() => {});
      video.dataset.userPaused = 'false';
  ...
  function toggleTejFeedVideoMute(id) {
    const video = document.getElementById(`tej-feedvid-${id}`);
    const icon = document.getElementById(`tej-feedvid-muteicon-${id}`);
    if (!video) return;
    video.muted = !video.muted;
    if (icon) {
      icon.classList.toggle('fa-volume-xmark', video.muted);
  ...
  function _tejFeedVideoProgressUpdate(id, forcedPct) {
    const bar = document.getElementById(`tej-feedvid-progressbar-${id}`);
    if (!bar) return;
    let pct = forcedPct;
    if (pct == null) {
      const video = document.getElementById(`tej-feedvid-${id}`);
  ...
  function _tejFeedVideoSeekToClientX(id, clientX) {
    const video = document.getElementById(`tej-feedvid-${id}`);
    const wrap = document.getElementById(`tej-feedvid-wrap-${id}`);
    if (!video || !wrap || !video.duration) return;
    const rect = wrap.getBoundingClientRect();
    let ratio = (clientX - rect.left) / rect.width;
    ratio = Math.min(1, Math.max(0, ratio));
    video.currentTime = ratio * video.duration;
    _tejFeedVideoProgressUpdate(id, ratio * 100);
  }

  function _tejFeedVideoSeekStart(event, id) {
    event.stopPropagation();
    const getX = (e) => (e.touches && e.touches[0] ? e.touches[0].clientX : e.clientX);
    _tejFeedVideoSeekToClientX(id, getX(event));
    const onMove = (e) => { e.preventDefault(); _tejFeedVideoSeekToClientX(id, getX(e)); };
    const onUp = () => {
      document.removeEventListener('pointermove', onMove);
      document.removeEventListener('pointerup', onUp);
    };
  ...
  function renderTejCard(id, d) {
    const avatar = d.userPhoto ? `<img loading="lazy" src='${d.userPhoto}' class='w-9 h-9 rounded-full object-cover bg-slate-200' onerror="this.onerror=null;this.src='https://placehold.co/36x36/f1f5f9/64748b?text=?'">` : `<div class='w-9 h-9 rounded-full bg-gradient-to-br from-orange-400 to-pink-500 flex items-center justify-center text-white font-black text-sm'>${(d.userName||'?')[0]}</div>`;
    const timeStr = d.createdAt ? tejTimeAgo(d.createdAt.toDate ? d.createdAt.toDate() : new Date(d.createdAt)) : '';
    let mediaHtml = '';
    // ✅ [FIX - বড় মিডিয়া] ফেসবুকের মতো ছবি/ভিডিও কার্ডের প্যাডিং (p-4) ভেদ করে edge-to-edge
    // ফুল-উইথে দেখানো হচ্ছে (আগে -mx-1 + max-h-64 ছোট বক্সে আটকে ছিল)
    if (d.mediaType === 'image' && d.mediaData) {
      mediaHtml = `<div class='mt-2.5 -mx-4 cursor-pointer bg-slate-100'><img src='${d.mediaData}' class='w-full max-h-[520px] object-cover' loading='lazy' onclick='event.stopPropagation(); openImageLightbox("${d.mediaData}")'></div>`;
    } else if (d.mediaType === 'video' && d.mediaData) {
      // ✅ UPDATED (feature-87): ফিডের প্রিভিউ থেকেই এখন সরাসরি প্লে/পজ ও টেনে টেনে (স্ক্রাব) দেখা যায়,
      // fullscreen শর্টসে যেতে হবে না — শুধু কোণার এক্সপ্যান্ড আইকন থেকে চাইলে fullscreen-এ যাওয়া যাবে
      mediaHtml = `<div class='mt-2.5 -mx-4 bg-black relative' id='tej-feedvid-wrap-${id}'>
        <video id='tej-feedvid-${id}' src='${d.mediaData}' class='w-full max-h-[520px]' preload='metadata' playsinline muted loop
          ontimeupdate='_tejFeedVideoProgressUpdate("${id}")' onclick='toggleTejFeedVideoPlay("${id}")'
          onplay='document.getElementById("tej-feedvid-playicon-${id}")?.classList.add("hidden")'
          onpause='document.getElementById("tej-feedvid-playicon-${id}")?.classList.remove("hidden")'></video>
        <div id='tej-feedvid-playicon-${id}' class='absolute inset-0 flex items-center justify-center pointer-events-none'>
          <div class='w-16 h-16 rounded-full bg-black/40 flex items-center justify-center'>
            <i class='fas fa-play text-white text-2xl ml-1'></i>
          </div>
        </div>
        <button onclick='event.stopPropagation(); toggleTejFeedVideoMute("${id}")'
          class='absolute top-3 right-3 z-20 w-8 h-8 rounded-full bg-black/40 flex items-center justify-center text-white'>
          <i class='fas fa-volume-xmark text-xs' id='tej-feedvid-muteicon-${id}'></i>
        </button>
        <button onclick='event.stopPropagation(); openTejShorts("${id}")'
          class='absolute top-3 right-14 z-20 w-8 h-8 rounded-full bg-black/40 flex items-center justify-center text-white'>
          <i class='fas fa-expand text-xs'></i>
        </button>
        <div class='absolute inset-x-0 bottom-0 px-2.5 pb-2' onpointerdown='_tejFeedVideoSeekStart(event, "${id}")'>
          <div class='h-1 w-full bg-white/25 rounded-full overflow-hidden'>
            <div id='tej-feedvid-progressbar-${id}' class='h-full bg-white rounded-full' style='width:0%'></div>
          </div>
        </div>
      </div>`;
    }
    ...
    } else if (d.postCategory === 'x') {
      topBadgeHtml = `<div class='-mx-4 -mt-4 mb-2.5 px-4 py-1.5 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-[10px] font-black rounded-t-2xl flex items-center gap-1.5'><i class='fas fa-mars'></i> X পোস্ট • পাত্র</div>`;
    } else if (d.postCategory === 'y') {
      topBadgeHtml = `<div class='-mx-4 -mt-4 mb-2.5 px-4 py-1.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[10px] font-black rounded-t-2xl flex items-center gap-1.5'><i class='fas fa-venus'></i> Y পোস্ট • পাত্রী</div>`;
    }
    const cardBorderClass = isDonation ? 'border-rose-200' : (isPendingFuture ? 'border-amber-200' : (d.postCategory === 'x' ? 'border-blue-200' : (d.postCategory === 'y' ? 'border-pink-200' : 'border-slate-100')));

    // ✅ NEW: X/Y পোস্টের নিচে প্রপোজ বাটন — বিপরীত gender-এর যেকেউ প্রপোজ করতে পারবে
    let xyActionHtml = '';
    if ((d.postCategory === 'x' || d.postCategory === 'y') && currentUser && d.uid !== currentUser.uid) {
      const isPartner = _tejXYMyProfile && _tejXYMyProfile.partnerUid === d.uid;
      const sent = _tejXYProposalsSentMap.get(d.uid);
      if (isPartner) {
        xyActionHtml = `<div class='mt-2.5 text-[11px] font-bold text-pink-500 flex items-center gap-1'><i class='fas fa-heart'></i> ${d.postCategory === 'x' ? 'আপনার বয়ফ্রেন্ড' : 'আপনার গার্লফ্রেন্ড'}</div>`;
      } else if (sent && sent.status === 'accepted') {
        xyActionHtml = `<div class='mt-2.5 text-[11px] font-bold text-pink-500 flex items-center gap-1'><i class='fas fa-heart'></i> ম্যাচড!</div>`;
      } else if (sent && sent.status === 'pending') {
        xyActionHtml = `<button disabled class='mt-2.5 w-full bg-slate-100 text-slate-400 text-[11px] font-bold py-2 rounded-xl'><i class='fas fa-clock mr-1'></i>প্রপোজ পাঠানো হয়েছে</button>`;
      } else if (sent && sent.status === 'rejected') {
        xyActionHtml = `<button disabled class='mt-2.5 w-full bg-slate-100 text-slate-400 text-[11px] font-bold py-2 rounded-xl'>প্রত্যাখ্যাত হয়েছে</button>`;
      } else {
        xyActionHtml = `<button onclick="proposeTejXY('${id}','${d.uid}','${d.postCategory}')" class='mt-2.5 w-full bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[11px] font-bold py-2 rounded-xl active:scale-95 transition'><i class='fas fa-heart mr-1'></i>প্রপোজ করুন</button>`;
      }
    }
  ...
  let _tejStoryMediaFile = null; // ✅ [FIX - বড় ফাইল আপলোড] raw File — Firestore-এ base64 না রেখে Storage-এ আপলোড হবে

  function openTejStoryCreate() {
    if (!currentUser) { showCartToast('⚠️ স্টোরি দিতে লগইন করুন'); return; }
    // ✅ [FIX - মেমরি লিক] আগের সিলেক্ট করা মিডিয়ার preview blob URL থাকলে রিভোক করা হচ্ছে
    if (_tejStoryMediaBase64 && _tejStoryMediaBase64.startsWith('blob:')) { try { URL.revokeObjectURL(_tejStoryMediaBase64); } catch (e) {} }
    _tejStoryMediaBase64 = null;
    _tejStoryMediaMime = null;
    _tejStoryMediaFile = null;
  ...
  function closeTejStoryCreate() {
    ...
    document.body.style.overflow = 'auto';
    // ✅ [FIX - মেমরি লিক] মডাল বন্ধ করার সময়ও blob URL রিভোক করা হচ্ছে
    if (_tejStoryMediaBase64 && _tejStoryMediaBase64.startsWith('blob:')) { try { URL.revokeObjectURL(_tejStoryMediaBase64); } catch (e) {} }
    _tejStoryMediaBase64 = null;
    _tejStoryMediaFile = null;
    _tejStoryMediaMime = null;
  ...
  function handleTejStoryMedia(input) {
    ...
    // ✅ [FIX - বড় ফাইল আপলোড] আগে base64 বানিয়ে Firestore-এ (১MB ডকুমেন্ট লিমিট) সেভ হতো,
    // তাই ৫MB ছবি/১৫MB ভিডিও কোনোটাই আসলে সেভ হতো না। এখন ফাইলটা সরাসরি Firebase Storage-এ
    // আপলোড হবে (tej_feed পোস্টের মতোই), Firestore-এ শুধু ছোট্ট download URL থাকবে।
    const maxMB = isVideo ? 100 : 10;
    if (file.size > maxMB * 1024 * 1024) { showCartToast(`⚠️ ফাইল ${maxMB}MB-এর বেশি হওয়া যাবে না`); return; }
    if (_tejStoryMediaBase64) { try { URL.revokeObjectURL(_tejStoryMediaBase64); } catch (e) {} }
    _tejStoryMediaFile = file;
    _tejStoryMediaMime = file.type;
    _tejStoryMediaBase64 = URL.createObjectURL(file); // শুধু প্রিভিউয়ের জন্য (মেমোরি-বান্ধব)
  ...
  async function submitTejStory() {
    ...
    if (!text && !_tejStoryMediaFile) { showCartToast('⚠️ ছবি বা টেক্সট যেকোনো একটি দিন'); return; }
    const btn = document.getElementById('tej-story-submit-btn');
    btn.disabled = true;
    try {
      ...
      // ✅ [FIX - বড় ফাইল আপলোড] Firestore-এ base64 না রেখে Storage-এ আপলোড করে URL সেভ করা হচ্ছে
      if (_tejStoryMediaFile) {
        ...
        const safeName = _tejStoryMediaFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
        const storagePath = `tej_feed/${currentUser.uid}/story_${Date.now()}_${safeName}`;
        const uploadTask = storage.ref(storagePath).put(_tejStoryMediaFile, { contentType: _tejStoryMediaMime });
        await new Promise((resolve, reject) => {
          uploadTask.on('state_changed', snap => {
            const pct = Math.round((snap.bytesTransferred / snap.totalBytes) * 100);
      ...
      btn.innerHTML = `<i class='fas fa-spinner fa-spin mr-2'></i> শেয়ার হচ্ছে...`;
      ...
      showCartToast('❌ স্টোরি শেয়ার করা যায়নি: ' + (e.message || ''));
      btn.disabled = false;
      btn.innerHTML = `<i class='fas fa-circle-play'></i> স্টোরি শেয়ার করুন`;
    }
  }
  ...
  function closeTejStoryView() {
    ...
    document.body.style.overflow = 'auto'; // ✅ [FIX - স্ক্রল লক] আগে এটা রিসেট হতো না
    const vid = document.getElementById('tej-story-view-video');
    vid.pause(); vid.src = '';
    _tejStoryViewList = [];
  }
  ...
  // ============================================================
  // ✅ [NEW feature-67] লাইফ পার্টনার — বিয়ের জন্য পাত্র-পাত্রী খোঁজার ফিচার
  // ============================================================
  let lifePartnerMyProfile = null;
  let _lifePartnerPhotoFiles = [null, null, null];
  let _lifePartnerVideoFile = null;
  let lifePartnerBrowseGender = 'all';
  let lifePartnerBrowseList = [];
  let _lifePartnerMatchListenerUnsub = null;

  // ✅ হাব খোলা — প্রথমবার ক্লিকে সরাসরি CV ফর্ম, পরে হাব (ব্রাউজ/আমার সিভি/প্রপোজ)
  async function openLifePartnerHub() {
    if (!currentUser) { showCartToast('⚠️ লাইফ পার্টনার ফিচার ব্যবহার করতে আগে Google দিয়ে লগইন করুন'); return; }
    try {
      const doc = await firestore.collection('life_partner_profiles').doc(currentUser.uid).get();
      lifePartnerMyProfile = doc.exists ? doc.data() : null;
    } catch (e) {
      console.error('Life partner profile load error:', e);
      lifePartnerMyProfile = null;
    }
    if (!lifePartnerMyProfile) {
      // ✅ প্রথমবার ক্লিক করলে সরাসরি CV ইন্টারফেস আসবে
      openLifePartnerCvForm();
      return;
    }
    document.getElementById('life-partner-hub-modal')?.classList.remove('hidden');
    switchLifePartnerTab('browse');
  }

  function closeLifePartnerHub() {
    document.getElementById('life-partner-hub-modal')?.classList.add('hidden');
  }

  function switchLifePartnerTab(tab) {
    ['browse', 'mycv', 'proposals'].forEach(t => {
      const active = t === tab;
      document.getElementById('life-partner-tab-' + t)?.classList.toggle('hidden', !active);
      const btn = document.getElementById('life-partner-tabbtn-' + t);
      if (btn) {
    ...
    if (tab === 'browse') loadLifePartnerBrowseList();
    if (tab === 'mycv') renderLifePartnerMyCvTab();
    if (tab === 'proposals') loadLifePartnerProposals();
  }

  // ---------- ব্রাউজ ----------
  function filterLifePartnerGender(gender) {
    lifePartnerBrowseGender = gender;
    ['all', 'male', 'female'].forEach(g => {
      const btn = document.getElementById('lp-gender-' + g);
      if (!btn) return;
    ...
    loadLifePartnerBrowseList();
  }

  async function loadLifePartnerBrowseList() {
    const grid = document.getElementById('life-partner-browse-grid');
    if (!grid) return;
    grid.innerHTML = `<div class="col-span-2 text-center py-8 text-slate-400 text-xs"><i class="fas fa-spinner fa-spin mr-1"></i> লোড হচ্ছে...</div>`;
    try {
      let query = firestore.collection('life_partner_profiles').where('isPublic', '==', true);
      if (lifePartnerBrowseGender !== 'all') query = query.where('gender', '==', lifePartnerBrowseGender);
      const snap = await query.limit(60).get();
      lifePartnerBrowseList = snap.docs.map(d => d.data()).filter(p => p.uid !== currentUser.uid);
      renderLifePartnerBrowseGrid(lifePartnerBrowseList);
    } catch (e) {
      console.error('Life partner browse error:', e);
      grid.innerHTML = `<div class="col-span-2 text-center py-8 text-red-400 text-xs">লোড করা যায়নি। Firestore Rules/Index চেক করুন।</div>`;
    }
  }

  function renderLifePartnerBrowseGrid(list) {
    const grid = document.getElementById('life-partner-browse-grid');
    if (!grid) return;
    if (list.length === 0) {
      grid.innerHTML = `<div class="col-span-2 text-center py-10 text-slate-400 text-xs"><i class="fas fa-heart-crack text-xl text-slate-300 mb-2 block"></i>এখনো কোনো পাবলিক প্রোফাইল নেই</div>`;
      return;
    }
    grid.innerHTML = list.map(p => `
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden cursor-pointer" onclick="openLifePartnerProfileDetail('${p.uid}')">
        <div class="h-40 bg-slate-100 overflow-hidden">
          <img loading="lazy" src="${escapeHtml((p.photos && p.photos[0]) || '')}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/200x240/fdf2f8/ec4899?text=?'">
        </div>
        <div class="p-2.5">
          <p class="font-bold text-slate-800 text-xs truncate">${escapeHtml(p.name)}, ${p.age || '?'}</p>
          <p class="text-[10px] text-slate-400 truncate mt-0.5"><i class="fas fa-briefcase mr-1"></i>${escapeHtml(p.profession || 'পেশা উল্লেখ নেই')}</p>
          <p class="text-[10px] text-slate-400 truncate"><i class="fas fa-location-dot mr-1"></i>${escapeHtml(p.presentAddress || '')}</p>
        </div>
      </div>
    `).join('');
  }
  ...
  // ---------- আমার সিভি ----------
  function renderLifePartnerMyCvTab() {
    const wrap = document.getElementById('life-partner-mycv-content');
    if (!wrap) return;
    if (!lifePartnerMyProfile) {
      wrap.innerHTML = `<div class="text-center py-10"><p class="text-xs text-slate-400 mb-3">আপনার এখনো কোনো সিভি তৈরি হয়নি</p><button onclick="openLifePartnerCvForm()" class="bg-rose-500 text-white text-xs font-bold px-4 py-2 rounded-xl">সিভি তৈরি করুন</button></div>`;
      return;
    }
    const p = lifePartnerMyProfile;
    wrap.innerHTML = `
      <div class="bg-white rounded-2xl border border-slate-100 p-4">
        <div class="flex items-center gap-3 mb-3">
          <img src="${escapeHtml((p.photos && p.photos[0]) || '')}" class="w-16 h-16 rounded-full object-cover bg-slate-100 border-2 border-white shadow" onerror="this.src='https://placehold.co/64x64/fdf2f8/ec4899?text=?'">
          <div class="flex-1">
            <p class="font-bold text-slate-800 text-sm">${escapeHtml(p.name)}, ${p.age || '?'}</p>
            <p class="text-[11px] text-slate-400">${escapeHtml(p.profession || '')}</p>
          </div>
        </div>
        <div class="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2.5 mb-3">
          <span class="text-xs font-bold text-slate-700">প্রোফাইল পাবলিক</span>
          <button onclick="toggleLifePartnerPublic()" class="w-11 h-6 rounded-full transition ${p.isPublic ? 'bg-emerald-500' : 'bg-slate-300'} relative">
            <span class="absolute top-0.5 ${p.isPublic ? 'left-5' : 'left-0.5'} w-5 h-5 bg-white rounded-full shadow transition"></span>
          </button>
        </div>
        <p class="text-[11px] text-slate-500 mb-3">${p.isPublic ? '✅ আপনার সিভি সবাই দেখতে পারছে ও প্রপোজ করতে পারবে।' : '🔒 আপনার সিভি এখন প্রাইভেট — কেউ দেখতে পারছে না।'}</p>
        <button onclick="openLifePartnerCvForm()" class="w-full bg-slate-900 text-white text-xs font-bold py-2.5 rounded-xl">সিভি এডিট করুন</button>
      </div>
    `;
  }

  async function toggleLifePartnerPublic() {
    if (!lifePartnerMyProfile) return;
    const newVal = !lifePartnerMyProfile.isPublic;
    if (newVal && (!lifePartnerMyProfile.photos || lifePartnerMyProfile.photos.length === 0)) {
      showCartToast('⚠️ পাবলিক করার আগে কমপক্ষে ১টা ছবি যুক্ত করুন'); return;
    }
    try {
      await firestore.collection('life_partner_profiles').doc(currentUser.uid).update({
        isPublic: newVal, updatedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      lifePartnerMyProfile.isPublic = newVal;
      renderLifePartnerMyCvTab();
      showCartToast(newVal ? '✅ প্রোফাইল পাবলিক হলো' : '🔒 প্রোফাইল প্রাইভেট হলো');
    } catch (e) { console.error(e); showCartToast('⚠️ আপডেট ব্যর্থ হয়েছে'); }
  }

  // ---------- CV ফর্ম ----------
  function openLifePartnerCvForm() {
    document.getElementById('life-partner-hub-modal')?.classList.add('hidden');
    document.getElementById('life-partner-cv-form-modal')?.classList.remove('hidden');
    const p = lifePartnerMyProfile || {};
    document.getElementById('lp-form-name').value = p.name || (currentUser.displayName || '');
    document.getElementById('lp-form-age').value = p.age || '';
    document.getElementById('lp-form-height').value = p.height || '';
    document.getElementById('lp-form-profession').value = p.profession || '';
    document.getElementById('lp-form-education').value = p.education || '';
    document.getElementById('lp-form-religion').value = p.religion || '';
    document.getElementById('lp-form-maritalstatus').value = p.maritalStatus || '';
    document.getElementById('lp-form-address').value = p.presentAddress || '';
    document.getElementById('lp-form-about').value = p.about || '';
    document.getElementsByName('lp-form-gender').forEach(r => { r.checked = (r.value === (p.gender || 'male')); });
    _lifePartnerPhotoFiles = [null, null, null];
    for (let i = 0; i < 3; i++) {
      const img = document.getElementById('lp-photo-preview-' + i);
      const existing = (p.photos && p.photos[i]) || '';
      if (img) img.src = existing || 'https://placehold.co/100x100/fdf2f8/ec4899?text=%2B';
    }
    _lifePartnerVideoFile = null;
    const videoPreview = document.getElementById('lp-video-preview');
    const videoWrap = document.getElementById('lp-video-preview-wrap');
    if (videoPreview) videoPreview.src = p.videoUrl || '';
    if (videoWrap) videoWrap.classList.toggle('hidden', !p.videoUrl);
  }

  function closeLifePartnerCvForm() {
    document.getElementById('life-partner-cv-form-modal')?.classList.add('hidden');
    if (lifePartnerMyProfile) document.getElementById('life-partner-hub-modal')?.classList.remove('hidden');
  }

  function lpSelectPhoto(index, input) {
    const file = input.files[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { showCartToast('⚠️ ছবি ৫MB এর বেশি হওয়া যাবে না'); input.value = ''; return; }
    _lifePartnerPhotoFiles[index] = file;
    const img = document.getElementById('lp-photo-preview-' + index);
    if (img) img.src = URL.createObjectURL(file);
  }

  function lpSelectVideo(input) {
    const file = input.files[0];
    if (!file) return;
    if (file.size > 30 * 1024 * 1024) { showCartToast('⚠️ ভিডিও ৩০MB এর বেশি হওয়া যাবে না'); input.value = ''; return; }
    _lifePartnerVideoFile = file;
    const preview = document.getElementById('lp-video-preview');
    const wrap = document.getElementById('lp-video-preview-wrap');
    if (preview) preview.src = URL.createObjectURL(file);
    if (wrap) wrap.classList.remove('hidden');
  }

  async function saveLifePartnerCv() {
    if (!currentUser) return;
    const name = document.getElementById('lp-form-name').value.trim();
    const age = parseInt(document.getElementById('lp-form-age').value, 10);
    const genderEl = document.querySelector('input[name="lp-form-gender"]:checked');
    if (!name || !age || !genderEl) { showCartToast('⚠️ নাম, বয়স ও লিঙ্গ অবশ্যই দিতে হবে'); return; }

    const btn = document.getElementById('lp-form-save-btn');
    ...
    try {
      const photos = [...(lifePartnerMyProfile?.photos || [])];
      for (let i = 0; i < 3; i++) {
        if (_lifePartnerPhotoFiles[i]) {
          const safeName = _lifePartnerPhotoFiles[i].name.replace(/[^a-zA-Z0-9._-]/g, '_');
          const path = `life_partner/${currentUser.uid}/photo_${i}_${Date.now()}_${safeName}`;
          const task = storage.ref(path).put(_lifePartnerPhotoFiles[i], { contentType: _lifePartnerPhotoFiles[i].type });
          await new Promise((res, rej) => task.on('state_changed', null, rej, res));
          photos[i] = await task.snapshot.ref.getDownloadURL();
        }
      }
      const cleanPhotos = photos.filter(Boolean);

      let videoUrl = lifePartnerMyProfile?.videoUrl || null;
      if (_lifePartnerVideoFile) {
        const safeName = _lifePartnerVideoFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
        const path = `life_partner/${currentUser.uid}/video_${Date.now()}_${safeName}`;
        const task = storage.ref(path).put(_lifePartnerVideoFile, { contentType: _lifePartnerVideoFile.type });
        await new Promise((res, rej) => task.on('state_changed', null, rej, res));
        videoUrl = await task.snapshot.ref.getDownloadURL();
      }
      ...
      const data = {
        ...
        age,
        height: document.getElementById('lp-form-height').value.trim(),
        profession: document.getElementById('lp-form-profession').value.trim(),
        education: document.getElementById('lp-form-education').value.trim(),
        religion: document.getElementById('lp-form-religion').value.trim(),
        maritalStatus: document.getElementById('lp-form-maritalstatus').value.trim(),
        presentAddress: document.getElementById('lp-form-address').value.trim(),
        about: document.getElementById('lp-form-about').value.trim(),
        photos: cleanPhotos,
        videoUrl,
        isPublic: lifePartnerMyProfile?.isPublic || false,
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      };
      if (!lifePartnerMyProfile) data.createdAt = firebase.firestore.FieldValue.serverTimestamp();

      await firestore.collection('life_partner_profiles').doc(currentUser.uid).set(data, { merge: true });
      lifePartnerMyProfile = { ...(lifePartnerMyProfile || {}), ...data };
      closeLifePartnerCvForm();
      showCartToast('✅ সিভি সেভ হয়েছে');
      document.getElementById('life-partner-hub-modal')?.classList.remove('hidden');
      switchLifePartnerTab('mycv');
    } catch (e) {
      console.error('Life partner CV save error:', e);
      showCartToast('⚠️ সেভ করা যায়নি, আবার চেষ্টা করুন');
    } finally {
      if (btn) { btn.disabled = false; btn.innerHTML = btnOriginalHtml; }
    }
  }

  // ---------- প্রোফাইল ডিটেইল + প্রপোজ ----------
  async function openLifePartnerProfileDetail(uid) {
    document.getElementById('life-partner-profile-detail-modal')?.classList.remove('hidden');
    const body = document.getElementById('life-partner-detail-body');
    if (body) body.innerHTML = `<div class="text-center py-10 text-slate-400 text-xs"><i class="fas fa-spinner fa-spin mr-1"></i> লোড হচ্ছে...</div>`;
    try {
      const doc = await firestore.collection('life_partner_profiles').doc(uid).get();
      if (!doc.exists) { if (body) body.innerHTML = `<div class="text-center py-10 text-slate-400 text-xs">প্রোফাইল পাওয়া যায়নি</div>`; return; }
      const p = doc.data();
      const state = await getLifePartnerProposalState(uid);
      renderLifePartnerDetail(p, state);
    } catch (e) {
      console.error(e);
      if (body) body.innerHTML = `<div class="text-center py-10 text-red-400 text-xs">লোড করা যায়নি</div>`;
    }
  }

  function closeLifePartnerProfileDetail() {
    document.getElementById('life-partner-profile-detail-modal')?.classList.add('hidden');
  }

  async function getLifePartnerProposalState(otherUid) {
    const myUid = currentUser.uid;
    const sentId = `${myUid}_${otherUid}`;
    const receivedId = `${otherUid}_${myUid}`;
    const [sentDoc, receivedDoc] = await Promise.all([
      firestore.collection('life_partner_proposals').doc(sentId).get(),
      firestore.collection('life_partner_proposals').doc(receivedId).get(),
    ]);
    if (sentDoc.exists) {
      const d = sentDoc.data();
      if (d.status === 'accepted') return { type: 'matched', proposalId: sentId };
      if (d.status === 'pending') return { type: 'sent_pending', proposalId: sentId };
      if (d.status === 'rejected') return { type: 'sent_rejected', proposalId: sentId };
    }
    if (receivedDoc.exists) {
      const d = receivedDoc.data();
      if (d.status === 'accepted') return { type: 'matched', proposalId: receivedId };
      if (d.status === 'pending') return { type: 'received_pending', proposalId: receivedId };
    }
    return { type: 'none' };
  }

  function renderLifePartnerDetail(p, state) {
    const body = document.getElementById('life-partner-detail-body');
    if (!body) return;
    const photosHtml = (p.photos || []).map(url => `<img loading="lazy" src="${escapeHtml(url)}" class="w-full h-56 object-cover rounded-xl bg-slate-100 shrink-0 snap-center" onerror="this.src='https://placehold.co/300x300/fdf2f8/ec4899?text=?'">`).join('');
    const videoHtml = p.videoUrl ? `<video controls class="w-full rounded-xl mt-3 bg-black" src="${escapeHtml(p.videoUrl)}"></video>` : '';

    let actionHtml = '';
    if (state.type === 'none') {
      actionHtml = `<button onclick="sendLifePartnerProposal('${p.uid}')" class="w-full bg-rose-500 text-white text-sm font-bold py-3 rounded-xl active:scale-95 transition"><i class="fas fa-heart mr-1.5"></i>প্রপোজ করুন</button>`;
    } else if (state.type === 'sent_pending') {
      actionHtml = `<button disabled class="w-full bg-slate-200 text-slate-500 text-sm font-bold py-3 rounded-xl"><i class="fas fa-clock mr-1.5"></i>প্রপোজ পাঠানো হয়েছে</button>`;
    } else if (state.type === 'sent_rejected') {
      actionHtml = `<button disabled class="w-full bg-slate-200 text-slate-500 text-sm font-bold py-3 rounded-xl">প্রত্যাখ্যাত হয়েছে</button>`;
    } else if (state.type === 'received_pending') {
      actionHtml = `
        <div class="flex gap-2">
          <button onclick="acceptLifePartnerProposal('${state.proposalId}', '${p.uid}')" class="flex-1 bg-emerald-500 text-white text-sm font-bold py-3 rounded-xl active:scale-95 transition">গ্রহণ করুন</button>
          <button onclick="rejectLifePartnerProposal('${state.proposalId}')" class="flex-1 bg-slate-200 text-slate-600 text-sm font-bold py-3 rounded-xl active:scale-95 transition">প্রত্যাখ্যান</button>
        </div>`;
    } else if (state.type === 'matched') {
      actionHtml = `<div class="w-full bg-pink-50 border border-pink-200 text-pink-600 text-sm font-bold py-3 rounded-xl text-center"><i class="fas fa-heart-circle-check mr-1.5"></i>ম্যাচড! 💕 আপনারা একে অপরকে পছন্দ করেছেন</div>`;
    }

    body.innerHTML = `
      <div class="flex gap-2 overflow-x-auto snap-x pb-1">${photosHtml || `<div class="w-full h-56 rounded-xl bg-slate-100 flex items-center justify-center text-slate-300"><i class="fas fa-user text-3xl"></i></div>`}</div>
      ${videoHtml}
      <div class="mt-4">
        <h3 class="font-black text-lg text-slate-800">${escapeHtml(p.name)}, ${p.age || '?'}</h3>
        <p class="text-xs text-slate-400 mt-0.5">${p.gender === 'male' ? 'পাত্র' : 'পাত্রী'}</p>
        <div class="grid grid-cols-2 gap-2 mt-3 text-xs">
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">উচ্চতা</p><p class="font-bold text-slate-700">${escapeHtml(p.height || '—')}</p></div>
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">পেশা</p><p class="font-bold text-slate-700">${escapeHtml(p.profession || '—')}</p></div>
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">শিক্ষা</p><p class="font-bold text-slate-700">${escapeHtml(p.education || '—')}</p></div>
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">ধর্ম</p><p class="font-bold text-slate-700">${escapeHtml(p.religion || '—')}</p></div>
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">বৈবাহিক অবস্থা</p><p class="font-bold text-slate-700">${escapeHtml(p.maritalStatus || '—')}</p></div>
          <div class="bg-slate-50 rounded-lg px-3 py-2"><p class="text-slate-400 text-[10px]">ঠিকানা</p><p class="font-bold text-slate-700">${escapeHtml(p.presentAddress || '—')}</p></div>
        </div>
        ${p.about ? `<p class="text-xs text-slate-600 mt-3 leading-relaxed">${escapeHtml(p.about)}</p>` : ''}
      </div>
      <div class="mt-4">${actionHtml}</div>
    `;
  }

  async function sendLifePartnerProposal(toUid) {
    if (!currentUser || !lifePartnerMyProfile) return;
    if (!lifePartnerMyProfile.isPublic) { showCartToast('⚠️ প্রপোজ করতে আগে আপনার সিভি পাবলিক করুন (আমার সিভি ট্যাব)'); return; }
    const proposalId = `${currentUser.uid}_${toUid}`;
    try {
      await firestore.collection('life_partner_proposals').doc(proposalId).set({
        fromUid: currentUser.uid,
        toUid,
        fromName: lifePartnerMyProfile.name || currentUser.displayName || '',
        fromPhoto: (lifePartnerMyProfile.photos && lifePartnerMyProfile.photos[0]) || '',
        status: 'pending',
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      showCartToast('💌 প্রপোজ পাঠানো হয়েছে');
      openLifePartnerProfileDetail(toUid); // বাটন স্টেট রিফ্রেশ
    } catch (e) { console.error(e); showCartToast('⚠️ প্রপোজ পাঠানো যায়নি'); }
  }

  async function acceptLifePartnerProposal(proposalId, fromUid) {
    try {
      await firestore.collection('life_partner_proposals').doc(proposalId).update({
        status: 'accepted', respondedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      showCartToast('💕 প্রপোজ গ্রহণ করা হয়েছে!');
      const doc = await firestore.collection('life_partner_profiles').doc(fromUid).get();
      openLifePartnerMatchCelebration(doc.exists ? doc.data() : null);
    } catch (e) { console.error(e); showCartToast('⚠️ কিছু ভুল হয়েছে'); }
  }

  async function rejectLifePartnerProposal(proposalId) {
    try {
      await firestore.collection('life_partner_proposals').doc(proposalId).update({
        status: 'rejected', respondedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      showCartToast('প্রপোজ প্রত্যাখ্যান করা হয়েছে');
      closeLifePartnerProfileDetail();
      loadLifePartnerProposals();
    } catch (e) { console.error(e); showCartToast('⚠️ কিছু ভুল হয়েছে'); }
  }

  // ---------- ম্যাচ সেলিব্রেশন — accept করার সাথে সাথেই উভয় পক্ষ একে অপরের প্রোফাইল দেখবে ----------
  function openLifePartnerMatchCelebration(otherProfile) {
    const modal = document.getElementById('life-partner-match-modal');
    if (!modal) return;
    const myPhoto = (lifePartnerMyProfile && lifePartnerMyProfile.photos && lifePartnerMyProfile.photos[0]) || currentUser.photoURL || '';
    const otherPhoto = (otherProfile && otherProfile.photos && otherProfile.photos[0]) || '';
    const myImg = document.getElementById('lp-match-my-photo');
    const otherImg = document.getElementById('lp-match-other-photo');
    const nameEl = document.getElementById('lp-match-other-name');
    if (myImg) myImg.src = myPhoto || 'https://placehold.co/96x96/fdf2f8/ec4899?text=?';
    if (otherImg) otherImg.src = otherPhoto || 'https://placehold.co/96x96/fdf2f8/ec4899?text=?';
    if (nameEl) nameEl.innerText = (otherProfile && otherProfile.name) || 'পার্টনার';
    modal.dataset.otherUid = (otherProfile && otherProfile.uid) || '';
    modal.classList.remove('hidden');
  }

  function closeLifePartnerMatchModal() {
    document.getElementById('life-partner-match-modal')?.classList.add('hidden');
  }

  function lpViewMatchedProfile() {
    const uid = document.getElementById('life-partner-match-modal')?.dataset.otherUid;
    closeLifePartnerMatchModal();
    if (uid) openLifePartnerProfileDetail(uid);
  }

  // ---------- প্রপোজ তালিকা (পাওয়া/পাঠানো) ----------
  async function loadLifePartnerProposals() {
    const wrap = document.getElementById('life-partner-proposals-content');
    if (!wrap || !currentUser) return;
    wrap.innerHTML = `<div class="text-center py-8 text-slate-400 text-xs"><i class="fas fa-spinner fa-spin mr-1"></i> লোড হচ্ছে...</div>`;
    try {
      const [receivedSnap, sentSnap] = await Promise.all([
        firestore.collection('life_partner_proposals').where('toUid', '==', currentUser.uid).get(),
        firestore.collection('life_partner_proposals').where('fromUid', '==', currentUser.uid).get(),
      ]);
      const received = receivedSnap.docs.map(d => ({ id: d.id, ...d.data() }));
      const sent = sentSnap.docs.map(d => ({ id: d.id, ...d.data() }));
      renderLifePartnerProposalsList(received, sent);
    } catch (e) {
      console.error(e);
      wrap.innerHTML = `<div class="text-center py-8 text-red-400 text-xs">লোড করা যায়নি</div>`;
    }
  }

  function renderLifePartnerProposalsList(received, sent) {
    const wrap = document.getElementById('life-partner-proposals-content');
    if (!wrap) return;
    const statusLabel = { pending: '🕐 অপেক্ষমান', accepted: '💕 গৃহীত', rejected: '❌ প্রত্যাখ্যাত' };
    const receivedHtml = received.length ? received.map(r => `
      <div class="flex items-center gap-2.5 bg-white border border-slate-100 rounded-xl p-2.5 mb-2 cursor-pointer" onclick="openLifePartnerProfileDetail('${r.fromUid}')">
        <img loading="lazy" src="${escapeHtml(r.fromPhoto) || 'https://placehold.co/40x40/fdf2f8/ec4899?text=?'}" class="w-10 h-10 rounded-full object-cover bg-slate-100">
        <div class="flex-1"><p class="text-xs font-bold text-slate-800">${escapeHtml(r.fromName || 'কেউ একজন')}</p><p class="text-[10px] text-slate-400">${statusLabel[r.status] || r.status}</p></div>
        <i class="fas fa-chevron-right text-slate-300 text-[10px]"></i>
      </div>`).join('') : `<p class="text-center text-slate-400 text-xs py-4">কোনো প্রপোজ পাওয়া যায়নি</p>`;

    const sentHtml = sent.length ? sent.map(s => `
      <div class="flex items-center gap-2.5 bg-white border border-slate-100 rounded-xl p-2.5 mb-2 cursor-pointer" onclick="openLifePartnerProfileDetail('${s.toUid}')">
        <div class="flex-1"><p class="text-xs font-bold text-slate-800">প্রোফাইল দেখুন</p><p class="text-[10px] text-slate-400">${statusLabel[s.status] || s.status}</p></div>
        <i class="fas fa-chevron-right text-slate-300 text-[10px]"></i>
      </div>`).join('') : `<p class="text-center text-slate-400 text-xs py-4">আপনি এখনো কাউকে প্রপোজ করেননি</p>`;

    wrap.innerHTML = `
      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1.5">পাওয়া প্রপোজ</p>
      ${receivedHtml}
      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1.5 mt-4">পাঠানো প্রপোজ</p>
      ${sentHtml}
    `;
  }

  // ---------- রিয়েল-টাইম ম্যাচ লিসেনার — নিজে পাঠানো প্রপোজ কেউ accept করলে সাথে সাথে সেলিব্রেশন দেখাবে ----------
  function initLifePartnerMatchListener() {
    if (!currentUser) return;
    if (_lifePartnerMatchListenerUnsub) { _lifePartnerMatchListenerUnsub(); _lifePartnerMatchListenerUnsub = null; }
    const seenKey = 'lifePartnerSeenMatches_' + currentUser.uid;
    const seen = new Set(safeJSONParse(localStorage.getItem(seenKey), []));
    _lifePartnerMatchListenerUnsub = firestore.collection('life_partner_proposals')
      ...
      .onSnapshot(async (snap) => {
        for (const docSnap of snap.docs) {
          ...
          try {
            const profDoc = await firestore.collection('life_partner_profiles').doc(d.toUid).get();
            openLifePartnerMatchCelebration(profDoc.exists ? profDoc.data() : null);
          } catch (e) { console.error(e); }
        }
      }, (e) => console.error('Life partner match listener error:', e));
  }

  // ============================================================
  // ✅ NEW: X/Y পোস্ট ফিল্টার — শুধু ছেলে (X) / শুধু মেয়ে (Y) পোস্ট + প্রপোজ সিস্টেম
  // - X: শুধু ছেলেরা পোস্ট করতে পারবে, যেকোনো মেয়ে প্রপোজ করতে পারবে
  // - Y: শুধু মেয়েরা পোস্ট করতে পারবে, যেকোনো ছেলে প্রপোজ করতে পারবে
  // - প্রথমবার পোস্ট/প্রপোজ করার আগে "যোগ্যতার ফরম" (নাম/বয়স/পেশা ইত্যাদি) পূরণ করতে হয়,
  //   এতে ব্যবহারকারীর gender (male/female) স্থায়ীভাবে সেট হয়ে যায় — এরপর থেকে স্বাভাবিকভাবে পোস্ট/প্রপোজ করা যায়।
  // - এক্সেপ্ট করলে উভয়ের tej_xy_profiles ডকে (যার যার নিজের অ্যাকাউন্ট থেকেই, Firestore Rules মেনে) partnerUid সেভ হয়।
  // ============================================================
  const TEJ_XY_PROFILES_COLLECTION = 'tej_xy_profiles';
  const TEJ_XY_PROPOSALS_COLLECTION = 'tej_xy_proposals';
  let _tejXYMyProfile = null; // ক্যাশড: {uid, gender, name, age, profession, education, location, about, photoURL, partnerUid, relationshipSince}
  let _tejXYProposalsSentMap = new Map(); // toUid -> {id, status}
  let _tejXYMatchListenerUnsub = null;
  let _tejXYReceivedUnsub = null;
  let _tejXYPendingGender = null; // ফরম খোলার সময় কোন gender-এর জন্য খোলা হলো, তা মনে রাখতে
  let _tejXYPendingCallback = null; // ফরম সাবমিট হওয়ার পর যেটা করতে হবে (পোস্ট-টাইপ সুইচ / প্রপোজ পাঠানো)

  // ---------- মডিউল চালু (লগইনের পর একবার) ----------
  async function initTejXYModule() {
    if (!currentUser) return;
    try {
      const snap = await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(currentUser.uid).get();
      _tejXYMyProfile = snap.exists ? snap.data() : null;
    } catch (e) { console.error('Tej XY profile load error:', e); }
    await loadTejXYSentCache();
    initTejXYMatchListener();
    initTejXYReceivedBadgeListener();
  }

  async function loadTejXYSentCache() {
    if (!currentUser) return;
    try {
      const snap = await firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).where('fromUid', '==', currentUser.uid).get();
      _tejXYProposalsSentMap.clear();
      snap.docs.forEach(d => {
        const v = d.data();
        _tejXYProposalsSentMap.set(v.toUid, { id: d.id, status: v.status });
      });
    } catch (e) { console.error('Tej XY sent cache error:', e); }
  }

  // ---------- যোগ্যতার ফরম না পূরণ করা থাকলে আগে সেটা খোলো, তারপর আসল কাজ করো ----------
  async function tejXYRequireProfileAndProceed(requiredGender, onReady) {
    if (!currentUser) { showCartToast('⚠️ আগে লগইন করুন'); return; }
    if (!_tejXYMyProfile) {
      try {
        const snap = await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(currentUser.uid).get();
    ...
    if (_tejXYMyProfile) {
      if (_tejXYMyProfile.gender !== requiredGender) {
        showCartToast(requiredGender === 'male' ? '⚠️ এই অপশনটি শুধু ছেলেদের জন্য' : '⚠️ এই অপশনটি শুধু মেয়েদের জন্য');
        return;
      }
      onReady();
      return;
    }
    _tejXYPendingCallback = onReady;
    openTejXYQualForm(requiredGender);
  }

  function openTejXYQualForm(requiredGender) {
    _tejXYPendingGender = requiredGender;
    const label = document.getElementById('tej-xy-qual-gender-label');
    if (label) label.textContent = requiredGender === 'male' ? 'ছেলে (X)' : 'মেয়ে (Y)';
    document.getElementById('tej-xy-qual-name').value = (currentUser && currentUser.displayName) || '';
    document.getElementById('tej-xy-qual-age').value = '';
    document.getElementById('tej-xy-qual-profession').value = '';
    document.getElementById('tej-xy-qual-education').value = '';
    document.getElementById('tej-xy-qual-location').value = '';
    document.getElementById('tej-xy-qual-about').value = '';
    document.getElementById('tej-xy-qual-form-modal')?.classList.remove('hidden');
  }

  function closeTejXYQualForm() {
    document.getElementById('tej-xy-qual-form-modal')?.classList.add('hidden');
    _tejXYPendingCallback = null;
  }

  async function submitTejXYQualForm() {
    const name = document.getElementById('tej-xy-qual-name').value.trim();
    const ageVal = document.getElementById('tej-xy-qual-age').value.trim();
    const age = parseInt(ageVal, 10);
    const profession = document.getElementById('tej-xy-qual-profession').value.trim();
    const education = document.getElementById('tej-xy-qual-education').value.trim();
    const location = document.getElementById('tej-xy-qual-location').value.trim();
    const about = document.getElementById('tej-xy-qual-about').value.trim();
    if (!name || !ageVal || isNaN(age)) { showCartToast('⚠️ নাম ও বয়স অবশ্যই দিতে হবে'); return; }
    if (age < 18) { showCartToast('⚠️ দুঃখিত, এই ফিচারটি শুধু ১৮+ বছর বয়সীদের জন্য'); return; }
    if (age > 90) { showCartToast('⚠️ সঠিক বয়স দিন'); return; }
    const btn = document.getElementById('tej-xy-qual-submit-btn');
    btn.disabled = true;
    try {
      const data = {
        uid: currentUser.uid,
        gender: _tejXYPendingGender,
        name, age, profession, education, location, about,
        photoURL: currentUser.photoURL || '',
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      };
      await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(currentUser.uid).set(data);
      _tejXYMyProfile = data;
      document.getElementById('tej-xy-qual-form-modal')?.classList.add('hidden');
      showCartToast('✅ তথ্য সেভ হয়েছে');
      const cb = _tejXYPendingCallback;
      _tejXYPendingCallback = null;
      if (cb) cb();
    } catch (e) {
      console.error(e);
      showCartToast('❌ সেভ করা যায়নি, আবার চেষ্টা করুন');
    }
    btn.disabled = false;
  }

  // ---------- প্রপোজ পাঠানো ----------
  function proposeTejXY(postId, toUid, postCategory) {
    if (!currentUser) { showCartToast('⚠️ প্রপোজ করতে আগে লগইন করুন'); return; }
    if (toUid === currentUser.uid) return;
    const requiredGender = postCategory === 'x' ? 'female' : 'male';
    tejXYRequireProfileAndProceed(requiredGender, () => _actuallySendTejXYProposal(toUid));
  }

  async function _actuallySendTejXYProposal(toUid) {
    if (_tejXYMyProfile && _tejXYMyProfile.partnerUid) {
      showCartToast('⚠️ আপনার ইতিমধ্যে একজন পার্টনার আছে'); return;
    }
    if (_tejXYProposalsSentMap.has(toUid)) {
      showCartToast('⚠️ আপনি ইতিমধ্যে প্রপোজ পাঠিয়েছেন'); return;
    }
    const proposalId = `${currentUser.uid}_${toUid}`;
    try {
      await firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).doc(proposalId).set({
        fromUid: currentUser.uid,
        toUid,
        fromName: _tejXYMyProfile.name || currentUser.displayName || '',
        fromGender: _tejXYMyProfile.gender,
        status: 'pending',
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      _tejXYProposalsSentMap.set(toUid, { id: proposalId, status: 'pending' });
      showCartToast('💌 প্রপোজ পাঠানো হয়েছে');
      renderTejFeedList(_tejCurrentFilter);
    } catch (e) { console.error(e); showCartToast('⚠️ প্রপোজ পাঠানো যায়নি'); }
  }

  async function acceptTejXYProposal(proposalId, fromUid) {
    try {
      await firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).doc(proposalId).update({
        status: 'accepted', respondedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      // ✅ শুধু নিজের ডকই আপডেট করা হচ্ছে (Firestore Rules অনুযায়ী প্রতিটি ইউজার শুধু নিজের প্রোফাইল লিখতে পারবে);
      // অপরপক্ষের ডকে partnerUid বসে initTejXYMatchListener() এর মাধ্যমে, যখন সে নিজে অনলাইনে থাকবে/লগইন করবে।
      await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(currentUser.uid).update({
        partnerUid: fromUid, relationshipSince: firebase.firestore.FieldValue.serverTimestamp(),
      });
      if (_tejXYMyProfile) _tejXYMyProfile.partnerUid = fromUid;
      showCartToast('💕 প্রপোজ গ্রহণ করা হয়েছে!');
      const fromProfSnap = await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(fromUid).get();
      openTejXYMatchCelebration(fromProfSnap.exists ? fromProfSnap.data() : null);
      loadTejXYProposals();
    } catch (e) { console.error(e); showCartToast('⚠️ কিছু ভুল হয়েছে'); }
  }

  async function rejectTejXYProposal(proposalId) {
    try {
      await firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).doc(proposalId).update({
        status: 'rejected', respondedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
  ...
  // ---------- ম্যাচ সেলিব্রেশন ----------
  function openTejXYMatchCelebration(otherProfile) {
    const modal = document.getElementById('tej-xy-match-modal');
    if (!modal) return;
    const myImg = document.getElementById('tej-xy-match-my-photo');
    const otherImg = document.getElementById('tej-xy-match-other-photo');
    const nameEl = document.getElementById('tej-xy-match-other-name');
    const labelEl = document.getElementById('tej-xy-match-label');
    if (myImg) myImg.src = (currentUser && currentUser.photoURL) || 'https://placehold.co/96x96/fdf2f8/ec4899?text=?';
    if (otherImg) otherImg.src = (otherProfile && otherProfile.photoURL) || 'https://placehold.co/96x96/fdf2f8/ec4899?text=?';
    if (nameEl) nameEl.innerText = (otherProfile && otherProfile.name) || 'পার্টনার';
    if (labelEl) labelEl.innerText = (_tejXYMyProfile && _tejXYMyProfile.gender === 'male') ? 'সে এখন আপনার গার্লফ্রেন্ড! 💕' : 'সে এখন আপনার বয়ফ্রেন্ড! 💕';
    modal.classList.remove('hidden');
  }

  function closeTejXYMatchModal() {
    document.getElementById('tej-xy-match-modal')?.classList.add('hidden');
  }

  // ---------- রিয়েল-টাইম ম্যাচ লিসেনার — নিজে পাঠানো প্রপোজ কেউ accept করলে সাথে সাথে সেলিব্রেশন + নিজের প্রোফাইলে partnerUid সেভ ----------
  function initTejXYMatchListener() {
    if (!currentUser) return;
    if (_tejXYMatchListenerUnsub) { _tejXYMatchListenerUnsub(); _tejXYMatchListenerUnsub = null; }
    const seenKey = 'tejXYSeenMatches_' + currentUser.uid;
    const seen = new Set(safeJSONParse(localStorage.getItem(seenKey), []));
    _tejXYMatchListenerUnsub = firestore.collection(TEJ_XY_PROPOSALS_COLLECTION)
      .where('fromUid', '==', currentUser.uid)
      .where('status', '==', 'accepted')
      .onSnapshot(async (snap) => {
        for (const docSnap of snap.docs) {
          ...
          try {
            await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(currentUser.uid).update({
              partnerUid: d.toUid, relationshipSince: firebase.firestore.FieldValue.serverTimestamp(),
            });
            if (_tejXYMyProfile) _tejXYMyProfile.partnerUid = d.toUid;
            _tejXYProposalsSentMap.set(d.toUid, { id: docSnap.id, status: 'accepted' });
            const toProfSnap = await firestore.collection(TEJ_XY_PROFILES_COLLECTION).doc(d.toUid).get();
            openTejXYMatchCelebration(toProfSnap.exists ? toProfSnap.data() : null);
          } catch (e) { console.error(e); }
        }
      }, (e) => console.error('Tej XY match listener error:', e));
  }

  // ---------- হেডারে "প্রপোজ" ব্যাজ — কতগুলো পেন্ডিং প্রপোজ এসেছে তা দেখানোর জন্য ----------
  function initTejXYReceivedBadgeListener() {
    if (!currentUser) return;
    if (_tejXYReceivedUnsub) { _tejXYReceivedUnsub(); _tejXYReceivedUnsub = null; }
    _tejXYReceivedUnsub = firestore.collection(TEJ_XY_PROPOSALS_COLLECTION)
      .where('toUid', '==', currentUser.uid)
      .where('status', '==', 'pending')
      .onSnapshot(snap => {
        const badge = document.getElementById('tej-xy-propose-badge');
        if (!badge) return;
        if (snap.size > 0) { badge.textContent = snap.size > 9 ? '9+' : snap.size; badge.classList.remove('hidden'); }
        else badge.classList.add('hidden');
      }, () => {});
  }

  // ---------- প্রপোজ ইনবক্স (পাওয়া/পাঠানো) ----------
  function openTejXYProposalsInbox() {
    if (!currentUser) { showCartToast('⚠️ লগইন করুন'); return; }
    document.getElementById('tej-xy-proposals-modal')?.classList.remove('hidden');
    loadTejXYProposals();
  }

  function closeTejXYProposalsInbox() {
    document.getElementById('tej-xy-proposals-modal')?.classList.add('hidden');
  }

  async function loadTejXYProposals() {
    const wrap = document.getElementById('tej-xy-proposals-content');
    if (!wrap || !currentUser) return;
    wrap.innerHTML = `<div class="text-center py-8 text-slate-400 text-xs"><i class="fas fa-spinner fa-spin mr-1"></i> লোড হচ্ছে...</div>`;
    try {
      const [receivedSnap, sentSnap] = await Promise.all([
        firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).where('toUid', '==', currentUser.uid).get(),
        firestore.collection(TEJ_XY_PROPOSALS_COLLECTION).where('fromUid', '==', currentUser.uid).get(),
      ]);
      const received = receivedSnap.docs.map(d => ({ id: d.id, ...d.data() }));
      const sent = sentSnap.docs.map(d => ({ id: d.id, ...d.data() }));
      renderTejXYProposalsList(received, sent);
    } catch (e) {
      console.error(e);
      wrap.innerHTML = `<div class="text-center py-8 text-red-400 text-xs">লোড করা যায়নি</div>`;
    }
  ...
  function renderTejXYProposalsList(received, sent) {
    const wrap = document.getElementById('tej-xy-proposals-content');
    if (!wrap) return;
    const statusLabel = { pending: '🕐 অপেক্ষমান', accepted: '💕 গৃহীত', rejected: '❌ প্রত্যাখ্যাত' };
    const receivedHtml = received.length ? received.map(r => `
      <div class="flex items-center gap-2.5 bg-white border border-slate-100 rounded-xl p-2.5 mb-2">
        ${r.fromPhoto || r.photoURL ? `<img loading="lazy" src="${escapeHtml(r.photoURL || '')}" class="w-9 h-9 rounded-full object-cover bg-slate-100 shrink-0">` : `<div class="w-9 h-9 rounded-full bg-gradient-to-br from-pink-400 to-rose-500 flex items-center justify-center text-white font-black text-xs shrink-0">${escapeHtml((r.fromName || '?')[0])}</div>`}
        <div class="flex-1 min-w-0">
          <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(r.fromName || 'কেউ একজন')}</p>
          <p class="text-[10px] text-slate-400">${statusLabel[r.status] || r.status}</p>
        </div>
        ${r.status === 'pending' ? `
        <div class="flex gap-1.5 shrink-0">
          <button onclick="acceptTejXYProposal('${r.id}','${r.fromUid}')" class="bg-emerald-500 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg active:scale-95 transition">গ্রহণ</button>
          <button onclick="rejectTejXYProposal('${r.id}')" class="bg-slate-200 text-slate-600 text-[10px] font-bold px-2.5 py-1.5 rounded-lg active:scale-95 transition">না</button>
        </div>` : ''}
      </div>`).join('') : `<p class="text-center text-slate-400 text-xs py-4">কোনো প্রপোজ পাওয়া যায়নি</p>`;

    const sentHtml = sent.length ? sent.map(s => `
      <div class="flex items-center gap-2.5 bg-white border border-slate-100 rounded-xl p-2.5 mb-2">
        <div class="flex-1"><p class="text-xs font-bold text-slate-800">প্রপোজ পাঠানো হয়েছে</p><p class="text-[10px] text-slate-400">${statusLabel[s.status] || s.status}</p></div>
      </div>`).join('') : `<p class="text-center text-slate-400 text-xs py-4">আপনি এখনো কাউকে প্রপোজ করেননি</p>`;
  ...
  function closeTejCreate() {
    ...
    document.body.style.overflow = 'auto'; // ✅ [FIX - স্ক্রল লক] প্রতিটা পোস্টের পর/বাতিলে এটা রিসেট হতো না
  }
  ...
  // ✅ NEW: X/Y ট্যাবে সুইচ করার আগে যোগ্যতার ফরম/gender চেক করে নেয়, বাকি টাইপগুলো আগের মতোই সরাসরি সুইচ হয়
  function switchTejPostType(type) {
    if (type === 'x' || type === 'y') {
      const requiredGender = type === 'x' ? 'male' : 'female';
      tejXYRequireProfileAndProceed(requiredGender, () => _applyTejPostTypeUI(type));
      return;
    }
    _applyTejPostTypeUI(type);
  }

  function _applyTejPostTypeUI(type) {
    ...
    if (type === 'post') {
      ...
      } else { // future, donation, x, y — ছবি অথবা ভিডিও যেকোনো একটি ঐচ্ছিক
  ...
  async function submitTejPost() {
    ...
    } else if (_tejCurrentPostType === 'x' || _tejCurrentPostType === 'y') {
      postCategory = _tejCurrentPostType;
      // ✅ ডিফেন্সিভ চেক — কেউ যেন সরাসরি gender ছাড়া X/Y পোস্ট করতে না পারে
      const requiredGender = postCategory === 'x' ? 'male' : 'female';
      if (!_tejXYMyProfile || _tejXYMyProfile.gender !== requiredGender) {
        showCartToast('⚠️ আগে যোগ্যতার ফরম পূরণ করুন'); return;
      }
    }
    ...
    try {
      ...
      tejBatch.set(newTejPostRef, {
        ...
        mediaData: mediaUrl,
        postCategory,
        xyGender: (postCategory === 'x' || postCategory === 'y') ? _tejXYMyProfile.gender : null,
        publishAt,
        likes: 0,
        likedBy: [],
        commentCount: 0,
  ...
  // ✅ [FIX] প্রোফাইল মডালে পোস্টগুলো এখন ছোট থাম্বনেইল গ্রিডের বদলে ফেসবুকের
  // মতো ফুল-সাইজ কার্ড আকারে একটার পর একটা (single column) নিচে দেখানো হয় —
  // বিদ্যমান renderTejCard() ব্যবহার করা হচ্ছে, তাই মূল ফিডের কার্ডের সাথে
  // ডিজাইন হুবহু মিলে যাবে (ছবি, টেক্সট, লাইক/কমেন্ট/শেয়ার সবই দেখা যাবে)।
  async function loadUserProfilePosts(uid) {
    ...
    try {
      const snap = await firestore.collection('tej_posts')
        ...
        .get();
      if (snap.empty) {
        grid.innerHTML = `<div class="text-center py-6 text-slate-400 text-xs">এখনো কোনো পোস্ট নেই</div>`;
        return;
      }
      grid.innerHTML = snap.docs.map(doc => renderTejCard(doc.id, doc.data())).join('');
    } catch (e) {
      grid.innerHTML = `<div class="text-center py-6 text-red-300 text-xs">পোস্ট লোড করা যায়নি</div>`;
    }
  }
  ...
  function renderUserProfileDetail(p, friendStatus, locked, isSelf, isFollowingState) {
    ...
    if (locked) {
      ...
      bioLineHtml = `<p class="text-slate-400 text-xs mt-1.5 italic">${t('noBioLabel')}</p>`;
    }
    ...
    if (!locked && (aboutRows.length > 0 || isSelf)) {
      aboutCardHtml = `
        <div class="mt-4 pt-4 border-t border-slate-100">
          ...
          <div class="divide-y divide-slate-50">${aboutRows.join('') || '<p class="text-[11px] text-slate-400 py-2">কোনো তথ্য যোগ করা হয়নি</p>'}</div>
    ...
    if (!locked) {
      communitiesCardHtml = `
        <div class="mt-4 pt-4 border-t border-slate-100">
          ...
          <p class="text-[11px] text-slate-400"><i class="fas fa-hourglass-half mr-1"></i> শীঘ্রই আসছে</p>
        </div>`;
    }
    ...
    card.innerHTML = `
      ...
      <div class="pt-12 pb-5 px-5">
        ...
        <div class="mt-4 pt-4 border-t border-slate-100">
          ...
          <div id="user-profile-posts-grid" class="space-y-3">
            <div class="text-center py-6 text-slate-400 text-xs"><i class="fas fa-spinner fa-spin"></i></div>
          </div>
        </div>`}
      </div>
    `;
  ...
  function renderPeerChatMessages(msgs) {
    ...
    container.innerHTML = visibleMsgs.map(m => {
      ...
      // ✅ NEW: নিজের মেসেজে এডিট/ডিলিট মেনু বাটন (শুধু নিজের পাঠানো মেসেজে দেখাবে)
      const ownMenuHtml = mine ? `
        <button onclick="event.stopPropagation(); togglePeerMsgMenu('${m.id}')" class="absolute -top-1.5 -left-1.5 w-5 h-5 rounded-full bg-black/30 text-white/80 flex items-center justify-center text-[9px]">
          <i class="fas fa-ellipsis-vertical"></i>
        </button>
        <div id="peer-msg-menu-${m.id}" class="hidden absolute top-4 -left-1.5 z-10 bg-slate-800 text-white text-[11px] rounded-lg shadow-lg overflow-hidden min-w-[92px]">
          ${m.type === 'text' ? `<button onclick="event.stopPropagation(); startEditPeerMessage('${m.id}', decodeURIComponent('${encodeURIComponent(m.text || '')}'))" class="w-full text-left px-3 py-2 hover:bg-slate-700"><i class="fas fa-pen mr-1.5"></i>এডিট করুন</button>` : ''}
          <button onclick="event.stopPropagation(); deletePeerMessage('${m.id}')" class="w-full text-left px-3 py-2 hover:bg-slate-700 text-rose-400"><i class="fas fa-trash-alt mr-1.5"></i>ডিলিট করুন</button>
        </div>` : '';
      return `<div class="flex ${mine ? 'justify-end' : 'justify-start'}" id="peer-msg-${m.id}">
        <div class="${mine ? 'chat-bubble-customer' : 'chat-bubble-seller'} relative" style="transition:opacity 0.4s,transform 0.4s">${ownMenuHtml}${buildPeerChatBubbleContent(m, mine)}</div>
      </div>`;
    }).join('');
  ...
  function buildPeerChatBubbleContent(m, mine) {
    ...
    return `<span id="peer-msg-body-${m.id}">${escapeHtml(m.text || '')}</span>${m.editedAt ? `<span class="text-[9px] opacity-60 ml-1">(এডিট করা)</span>` : ''}`;
  }

  // ✅ NEW: নিজের মেসেজের এডিট/ডিলিট মেনু খোলা/বন্ধ করা (একসাথে একটাই মেনু খোলা থাকবে)
  function togglePeerMsgMenu(msgId) {
    document.querySelectorAll('[id^="peer-msg-menu-"]').forEach(el => {
      if (el.id !== 'peer-msg-menu-' + msgId) el.classList.add('hidden');
    });
    document.getElementById('peer-msg-menu-' + msgId)?.classList.toggle('hidden');
  }
  // ✅ মেনুর বাইরে ট্যাপ করলে খোলা মেনু বন্ধ হয়ে যাবে
  document.addEventListener('click', () => {
    document.querySelectorAll('[id^="peer-msg-menu-"]').forEach(el => el.classList.add('hidden'));
  });

  // ✅ NEW: নিজের টেক্সট মেসেজ এডিট মোডে নেওয়া — বাবলের ভেতরেই ইনলাইন টেক্সটএরিয়া দেখাবে
  function startEditPeerMessage(msgId, currentText) {
    document.getElementById('peer-msg-menu-' + msgId)?.classList.add('hidden');
    const body = document.getElementById('peer-msg-body-' + msgId);
    if (!body) return;
    const wrap = document.createElement('div');
    wrap.innerHTML = `
      <textarea id="peer-edit-input-${msgId}" class="w-full bg-white/10 border border-white/30 rounded-lg p-1.5 text-sm outline-none resize-none" rows="2"></textarea>
      <div class="flex gap-3 justify-end mt-1">
        <button onclick="cancelEditPeerMessage()" class="text-[10px] opacity-70">বাতিল</button>
        <button onclick="saveEditPeerMessage('${msgId}')" class="text-[10px] font-bold underline">সেভ করুন</button>
      </div>`;
    body.replaceWith(wrap);
    const ta = document.getElementById('peer-edit-input-' + msgId);
    if (ta) { ta.value = currentText; ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); }
  }

  // ✅ এডিট বাতিল করলে চ্যাট লিস্ট আবার আগের অবস্থায় রি-রেন্ডার হবে
  function cancelEditPeerMessage() {
    if (_peerChatLastMsgs.length) renderPeerChatMessages(_peerChatLastMsgs);
  }

  // ✅ NEW: এডিট করা টেক্সট Firestore-এ সেভ করা (শুধু নিজের মেসেজ, Firestore Rules এটা এনফোর্স করে)
  async function saveEditPeerMessage(msgId) {
    const ta = document.getElementById('peer-edit-input-' + msgId);
    if (!ta || !_peerChatId) return;
    const newText = ta.value.trim();
    if (!newText) { showCartToast('⚠️ মেসেজ খালি রাখা যাবে না'); return; }
    try {
      await firestore.collection('peer_chats').doc(_peerChatId).collection('messages').doc(msgId)
        .update({ text: newText, editedAt: firebase.firestore.FieldValue.serverTimestamp() });
    } catch (e) {
      showCartToast('❌ এডিট করা যায়নি: ' + e.message);
      cancelEditPeerMessage();
    }
  }

  // ✅ NEW: নিজের মেসেজ (টেক্সট/ছবি/ভয়েস) ইচ্ছামতো ডিলিট করা
  async function deletePeerMessage(msgId) {
    document.getElementById('peer-msg-menu-' + msgId)?.classList.add('hidden');
    if (!_peerChatId) return;
    if (!confirm('এই মেসেজটি মুছে ফেলতে চান? এটা আর ফেরত পাওয়া যাবে না।')) return;
    try {
      await firestore.collection('peer_chats').doc(_peerChatId).collection('messages').doc(msgId).delete();
    } catch (e) {
      showCartToast('❌ ডিলিট করা যায়নি: ' + e.message);
    }
  }
  ...
  function showIncomingCall(callId, data) {
    ...
    _callAnswerTimeout = setTimeout(() => {
      if (_activeCallId === callId && !_callStartedAt) {
        ...
        writeCallLogEntry('missed', 0); // ✅ NEW (feature-98): কল হিস্টরিতে "মিসড কল" হিসেবে সেভ করো
        stopRingtone();
  ...
  async function startOutgoingCall(otherUid, otherName, otherPhoto, type) {
    ...
    try {
      _localStream = await navigator.mediaDevices.getUserMedia({
        ...
        video: type === 'video' ? getCallVideoConstraints() : false
      });
    } catch (e) {
    ...
    playRingtone('outgoing'); // ✅ [রিংটোন ফিক্স] কলার সাইডে রিং-ব্যাক টোন — আগে এটা কোথাও কল করা হতো না
    ...
    try {
      ...
      applyHDEncodingParams(_pc); // ✅ [FULL HD আপডেট] বিটরেট সিলিং বাড়ানো
      await callRef.set({
      ...
      // ✅ NEW: কল ডকুমেন্ট সফলভাবে লেখা হওয়ার পরই আসলে অপর প্রান্তে রিং হওয়া শুরু হয় —
      // তাই এখন থেকেই স্ট্যাটাস টেক্সট "কল হচ্ছে..." থেকে "রিং হচ্ছে..." তে বদলে দাও
      // (অডিও ও ভিডিও — দুই ধরনের কলেই প্রযোজ্য)
      if (_activeCallId === callRef.id) {
        const statusEl = document.getElementById('call-status-text');
        if (statusEl) statusEl.innerText = t('ringingLabel');
      }
    } catch (e) {
    ...
    _callDocUnsub = callRef.onSnapshot(async snap => {
      ...
      if (data.status === 'accepted' && data.answer && _pc && !_pc.currentRemoteDescription) {
        ...
        stopRingtone();
        endCall(true, data.status === 'missed' ? 'missed' : undefined);
      }
  ...
  async function acceptIncomingCall() {
    ...
    try {
      ...
      _localStream = await navigator.mediaDevices.getUserMedia({
        audio: getCallAudioConstraints(),
        video: _activeCallType === 'video' ? getCallVideoConstraints() : false
      });
      _ownCandidateIds = [];
      _isMicMuted = false; _iceRestartAttempted = false;
      _pc = new RTCPeerConnection(ICE_SERVERS);
      ...
      writeCallLogEntry('rejected', 0); // ✅ NEW (feature-98): ডিভাইস/পারমিশন এরর হলেও কল হিস্টরিতে রেকর্ড থাকুক
      cleanupCallState();
      hideCallModal();
    }
  }
  ...
  function rejectIncomingCall() {
    ...
    if (_activeCallId) {
      firestore.collection('calls').doc(_activeCallId).update({ status: 'rejected' }).catch(() => {});
      writeCallLogEntry('rejected', 0); // ✅ NEW (feature-98): কল হিস্টরিতে "প্রত্যাখ্যান" হিসেবে সেভ করো
    }
    cleanupCallState();
    hideCallModal();
  }

  function endCall(skipRemoteUpdate, reasonStatus) {
    stopRingtone();
    if (navigator.vibrate) navigator.vibrate(0);
    clearTimeout(_callAnswerTimeout);
    ...
    // ✅ NEW (feature-98): কল হিস্টরি লগ — status না দেওয়া থাকলে কল কানেক্টেড হয়েছিল কিনা তা দিয়ে অনুমান করো
    // (কানেক্টেড হলে 'completed', caller নিজে কেটে দিলে (কানেক্ট হওয়ার আগে) 'cancelled', callee কেটে দিলে 'rejected')
    const _wasConnected = !!_callStartedAt;
    const _durationSec  = _wasConnected ? Math.max(0, Math.floor((Date.now() - _callStartedAt) / 1000)) : 0;
    const _finalStatus  = reasonStatus || (_wasConnected ? 'completed' : (_isCallCaller ? 'cancelled' : 'rejected'));
    writeCallLogEntry(_finalStatus, _durationSec);
  ...
  // ============================================================
  // ✅ NEW (feature-98): কল হিস্টরি লগিং (ইনকামিং/আউটগোয়িং/মিসড) + মিসড কল অ্যালার্ট
  // — প্রতিটা কলের জন্য দুটো আলাদা লগ ডকুমেন্ট তৈরি হয় (caller-এর জন্য "outgoing", callee-এর
  //   জন্য "incoming"), যাতে প্রত্যেকে নিজের হিস্টরি স্বাধীনভাবে ডিলিট করতে পারে (একজন ডিলিট
  //   করলে অন্যজনের এন্ট্রিতে প্রভাব পড়বে না)।
  // — ডকুমেন্ট আইডি deterministic (callId + '_out' / '_in') রাখা হয়েছে যাতে caller/callee
  //   দুই ডিভাইস থেকেই লেখা হলেও (idempotent) কখনো ডুপ্লিকেট এন্ট্রি তৈরি না হয়।
  // ============================================================
  async function writeCallLogEntry(finalStatus, durationSec) {
    try {
      if (!_activeCallId || !currentUser) return;
      const callId      = _activeCallId;
      const callerUid   = _isCallCaller ? currentUser.uid : _callOtherUid;
      const callerName  = _isCallCaller ? (currentUser.displayName || t('roleLabelCustomer')) : (_callOtherName || t('roleLabelCustomer'));
      const callerPhoto = _isCallCaller ? (currentUser.photoURL || '') : (_callOtherPhoto || '');
      const calleeUid   = _isCallCaller ? _callOtherUid : currentUser.uid;
      const calleeName  = _isCallCaller ? (_callOtherName || t('roleLabelCustomer')) : (currentUser.displayName || t('roleLabelCustomer'));
      const calleePhoto = _isCallCaller ? (_callOtherPhoto || '') : (currentUser.photoURL || '');
      if (!callerUid || !calleeUid || callerUid === calleeUid) return;
      const callType = _activeCallType || 'audio';
      const base = {
        callerUid, calleeUid, callType,
        status: finalStatus,
        durationSec: durationSec || 0,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      };
      const batch = firestore.batch();
      batch.set(firestore.collection('call_logs').doc(callId + '_out'), {
        ...base, ownerUid: callerUid, otherUid: calleeUid,
        otherName: calleeName, otherPhoto: calleePhoto,
        direction: 'outgoing', read: true
      }, { merge: true });
      batch.set(firestore.collection('call_logs').doc(callId + '_in'), {
        ...base, ownerUid: calleeUid, otherUid: callerUid,
        otherName: callerName, otherPhoto: callerPhoto,
        direction: 'incoming', read: finalStatus !== 'missed'
      }, { merge: true });
      await batch.commit();
    } catch (e) { /* বেস্ট-এফোর্ট — লগ লেখা ব্যর্থ হলেও কল ফাংশনালিটিতে প্রভাব পড়বে না */ }
  }

  // ✅ লগইনের পরে চালু হয় — নিজের আনরিড মিসড কল সংখ্যা রিয়েল-টাইমে ব্যাজে দেখায় + নতুন মিসড কলে পুশ নোটিফিকেশন
  function initMissedCallBadgeListener(uid) {
    if (_missedCallBadgeUnsub) _missedCallBadgeUnsub();
    _missedCallFirstLoad = true;
    _missedCallBadgeUnsub = firestore.collection('call_logs')
      .where('ownerUid', '==', uid)
      .where('direction', '==', 'incoming')
      .where('status', '==', 'missed')
      .where('read', '==', false)
      .onSnapshot(snap => {
        const badge = document.getElementById('tej-message-inbox-badge');
        if (badge) {
          badge.innerText = snap.size > 9 ? '9+' : String(snap.size);
          badge.classList.toggle('hidden', snap.size === 0);
        }
        if (!_missedCallFirstLoad && localStorage.getItem('notif_pref_social') !== 'off') {
          snap.docChanges().forEach(change => {
            if (change.type === 'added') {
              const d = change.doc.data();
              sendPushNotification('📵 মিসড কল', (d.otherName || 'কেউ একজন') + ' আপনাকে কল করেছিলেন।');
            }
          });
        }
        _missedCallFirstLoad = false;
      }, () => {});
  }

  // ✅ মেসেজ ইনবক্স মডালে "চ্যাট" ⇄ "কল" ট্যাব সুইচ করো
  function switchTejInboxTab(tab) {
    _tejInboxActiveTab = tab;
    const chatTabBtn  = document.getElementById('tej-inbox-tab-chat');
    const callTabBtn  = document.getElementById('tej-inbox-tab-calls');
    const chatPane    = document.getElementById('tej-message-inbox-list');
    const callPane    = document.getElementById('tej-call-history-list');
    const searchWrap  = document.getElementById('tej-inbox-search-wrap');
    const callToolbar = document.getElementById('tej-call-history-toolbar');
    if (tab === 'calls') {
      chatTabBtn?.classList.remove('text-white', 'border-orange-400');
      chatTabBtn?.classList.add('text-white/50', 'border-transparent');
      callTabBtn?.classList.add('text-white', 'border-orange-400');
      callTabBtn?.classList.remove('text-white/50', 'border-transparent');
      chatPane?.classList.add('hidden');
      callPane?.classList.remove('hidden');
      searchWrap?.classList.add('hidden');
      callToolbar?.classList.remove('hidden');
      _loadTejCallHistory();
    } else {
      callTabBtn?.classList.remove('text-white', 'border-orange-400');
      callTabBtn?.classList.add('text-white/50', 'border-transparent');
      chatTabBtn?.classList.add('text-white', 'border-orange-400');
      chatTabBtn?.classList.remove('text-white/50', 'border-transparent');
      callPane?.classList.add('hidden');
      chatPane?.classList.remove('hidden');
      searchWrap?.classList.remove('hidden');
      callToolbar?.classList.add('hidden');
    }
  }

  // ⚠️ REQUIRED FIRESTORE INDEX (feature-98, কল হিস্টরি লিস্ট): এই ফাংশনের কোয়েরি ownerUid দিয়ে
  // ফিল্টার + createdAt দিয়ে সর্ট করে, তাই Firebase Console → Firestore → Indexes → Composite এ
  // একটা ইনডেক্স লাগবে: Collection: call_logs | Field 1: ownerUid (Ascending) | Field 2: createdAt (Descending)
  async function _loadTejCallHistory() {
    const listEl = document.getElementById('tej-call-history-list');
    if (!listEl) return;
    listEl.innerHTML = `<div class='text-center py-10 text-slate-400'><i class='fas fa-spinner fa-spin text-xl text-slate-300 mb-2 block'></i><p class='text-xs'>কল হিস্টরি লোড হচ্ছে...</p></div>`;
    try {
      const snapshot = await firestore.collection('call_logs')
        .where('ownerUid', '==', currentUser.uid)
        .orderBy('createdAt', 'desc')
        .limit(100)
        .get();
      _tejCallLogData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      _renderTejCallHistory(_tejCallLogData);
      // ✅ এই ট্যাব খোলার পরে সব আনরিড মিসড কল "read" হিসেবে মার্ক করো (ব্যাজ ক্লিয়ার হবে)
      markMissedCallsRead(_tejCallLogData);
    } catch (e) {
      listEl.innerHTML = `<div class='text-center py-10 text-red-400 text-xs'>লোড ব্যর্থ: ${escapeHtml(e.message)}</div>`;
    }
  }

  function markMissedCallsRead(logs) {
    const unread = logs.filter(l => l.direction === 'incoming' && l.status === 'missed' && l.read === false);
    if (!unread.length) return;
    const batch = firestore.batch();
    unread.forEach(l => batch.update(firestore.collection('call_logs').doc(l.id), { read: true }));
    batch.commit().catch(() => {});
  }

  function _renderTejCallHistory(logs) {
    const listEl = document.getElementById('tej-call-history-list');
    if (!listEl) return;
    if (!logs.length) {
      listEl.innerHTML = `<div class='text-center py-10 text-slate-400'><i class='fas fa-phone-slash text-2xl text-slate-300 mb-2 block'></i><p class='text-xs'>কোনো কল হিস্টরি নেই</p></div>`;
      return;
    }
    listEl.innerHTML = logs.map(l => {
      const name = escapeHtml(l.otherName || 'ইউজার');
      const photo = l.otherPhoto || '';
      const avatar = photo
        ? `<img loading="lazy" src='${escapeHtml(photo)}' class='w-11 h-11 rounded-full object-cover shrink-0 border border-slate-200' onerror="this.src='https://placehold.co/44x44/f1f5f9/64748b?text=?'">`
        : `<div class='w-11 h-11 rounded-full bg-slate-200 flex items-center justify-center shrink-0'><i class='fas fa-user text-slate-400 text-sm'></i></div>`;
      const isMissed = l.status === 'missed' && l.direction === 'incoming';
      let dirIcon, dirColor, statusLabel;
      if (l.direction === 'incoming') {
        if (l.status === 'missed') { dirIcon = 'fa-arrow-down-left'; dirColor = 'text-rose-500'; statusLabel = 'মিসড কল'; }
        else if (l.status === 'rejected') { dirIcon = 'fa-arrow-down-left'; dirColor = 'text-rose-400'; statusLabel = 'প্রত্যাখ্যান করেছেন'; }
        else { dirIcon = 'fa-arrow-down-left'; dirColor = 'text-emerald-500'; statusLabel = 'ইনকামিং'; }
      } else {
        if (l.status === 'missed') { dirIcon = 'fa-arrow-up-right'; dirColor = 'text-amber-500'; statusLabel = 'রিসিভ করেননি'; }
        else if (l.status === 'rejected') { dirIcon = 'fa-arrow-up-right'; dirColor = 'text-rose-400'; statusLabel = 'প্রত্যাখ্যাত'; }
        else if (l.status === 'cancelled') { dirIcon = 'fa-arrow-up-right'; dirColor = 'text-slate-400'; statusLabel = 'বাতিল করেছেন'; }
        else { dirIcon = 'fa-arrow-up-right'; dirColor = 'text-blue-500'; statusLabel = 'আউটগোয়িং'; }
      }
      const typeIcon = l.callType === 'video' ? 'fa-video' : 'fa-phone';
      const durLabel = l.durationSec > 0 ? ` · ${Math.floor(l.durationSec / 60)}:${String(l.durationSec % 60).padStart(2, '0')}` : '';
      const timeLabel = l.createdAt && l.createdAt.toDate ? l.createdAt.toDate().toLocaleString('bn-BD', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }) : '';
      const safeName = name.replace(/'/g, "\\'");
      const safePhoto = escapeHtml(photo).replace(/'/g, "\\'");
      const otherUid = l.otherUid;
      return `
        <div class='w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition ${isMissed ? 'bg-rose-50/60' : ''}'>
          <button onclick="closeTejMessageInbox(); openUserProfileDetail('${otherUid}')" class='shrink-0'>${avatar}</button>
          <div class='flex-1 min-w-0 text-left' onclick="closeTejMessageInbox(); openUserProfileDetail('${otherUid}')">
            <p class='font-black text-xs ${isMissed ? 'text-rose-600' : 'text-slate-800'} truncate'>${name}</p>
            <p class='text-[10px] ${dirColor} truncate mt-0.5 flex items-center gap-1'>
              <i class='fas ${dirIcon}'></i> ${statusLabel}${durLabel}
            </p>
            <p class='text-[9px] text-slate-400 mt-0.5'>${timeLabel}</p>
          </div>
          <button onclick="startOutgoingCall('${otherUid}', '${safeName}', '${safePhoto}', '${l.callType === 'video' ? 'video' : 'audio'}')"
            class='w-9 h-9 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500 shrink-0 active:scale-90 transition'>
            <i class='fas ${typeIcon} text-xs'></i>
          </button>
          <button onclick='deleteCallLogEntry("${l.id}", this)' class='w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 shrink-0 active:scale-90 transition'>
  ...
  // ✅ প্রত্যেকে শুধু নিজের কল হিস্টরি এন্ট্রি ডিলিট করতে পারবে (Firestore Rules: ownerUid == auth.uid)
  function deleteCallLogEntry(logId, btnEl) {
    firestore.collection('call_logs').doc(logId).delete()
      .then(() => {
        _tejCallLogData = _tejCallLogData.filter(l => l.id !== logId);
        _renderTejCallHistory(_tejCallLogData);
      })
      .catch(e => showCartToast('⚠️ ডিলিট ব্যর্থ: ' + e.message));
  }

  // ✅ পুরো কল হিস্টরি একসাথে ডিলিট করো (নিজের এন্ট্রিগুলোই — অন্যপক্ষের হিস্টরিতে প্রভাব পড়বে না)
  async function clearAllTejCallHistory() {
    if (!_tejCallLogData.length) return;
    if (!confirm('আপনার সব কল হিস্টরি ডিলিট করতে চান? এটি ফিরিয়ে আনা যাবে না।')) return;
    try {
      const batch = firestore.batch();
      _tejCallLogData.forEach(l => batch.delete(firestore.collection('call_logs').doc(l.id)));
      await batch.commit();
      _tejCallLogData = [];
      _renderTejCallHistory([]);
      showCartToast('✅ কল হিস্টরি মুছে ফেলা হয়েছে');
    } catch (e) {
      showCartToast('⚠️ ডিলিট ব্যর্থ: ' + e.message);
    }
  }
  ...
  function renderCallModal(state) {
    ...
    if (state === 'connected' && isVideo) {
      ...
      content.innerHTML = `
        ...
        ` : ''}
        ${(isVideo && state === 'outgoing') ? `
          <!-- ✅ NEW: আউটগোয়িং ভিডিও কলে ডায়াল করা মাত্রই নিজের সামনের ক্যামেরা প্রিভিউ দেখাও —
               callee রিসিভ করার জন্য অপেক্ষা করার সময়েও (getUserMedia ইতিমধ্যে startOutgoingCall-এ
               সম্পন্ন হয়ে গেছে renderCallModal কল হওয়ার আগেই) -->
          <video id="call-local-video" autoplay playsinline muted class="absolute bottom-28 right-4 w-24 h-36 rounded-2xl object-cover border-2 border-white/30 shadow-xl bg-slate-800 z-10"></video>
        ` : ''}
      `;
      if (state === 'connected') {
      ...
      } else if (state === 'outgoing' && isVideo) {
        // ✅ NEW: ডায়াল করার সাথে সাথেই সামনের ক্যামেরা প্রিভিউ বাইন্ড করো
        const localVid = document.getElementById('call-local-video');
        if (localVid && _localStream) localVid.srcObject = _localStream;
      }
    }
  }
  ...
  async function switchCallCamera() {
    ...
    try {
      const newStream = await navigator.mediaDevices.getUserMedia({ video: getCallVideoConstraints(), audio: false });
  ...
  // ============================================================
  // ✅ NEW: কাস্টম রিংটোন প্রেফারেন্স — প্রতিটি ইউজার নিজের পছন্দের ইনকামিং কল রিংটোন
  // বেছে নিতে পারবে — হয় বিল্ট-ইন প্রিসেট থেকে, অথবা নিজের ফোন থেকে একটি অডিও ফাইল
  // (mp3/m4a/ogg/wav — যেমন ফোনে সেভ করা রিংটোন) আপলোড করে। ব্রাউজার থেকে ফোনের সিস্টেম
  // রিংটোন লাইব্রেরি সরাসরি অ্যাক্সেস করার কোনো ওয়েব API নেই (প্রাইভেসি/স্যান্ডবক্সিং কারণে),
  // তাই ফাইল-পিকার দিয়ে ফোন থেকে অডিও ফাইল বেছে নেওয়াই ওয়েব অ্যাপে বাস্তবসম্মত উপায়।
  // সিলেকশন localStorage (এই ডিভাইস) + Firestore users/{uid}.customRingtoneUrl (অ্যাকাউন্ট,
  // তাই নতুন ডিভাইসে লগইন করলেও কাস্টম রিংটোন ফাইলটা পাওয়া যাবে) — দুই জায়গাতেই সেভ থাকে।
  // ============================================================
  const RINGTONE_PRESETS = [
    { id: 'classic', notes: [{ f: 880,  s: 0,    d: 0.18 }, { f: 1100, s: 0.22, d: 0.18 }], loopLen: 0.9 },
    { id: 'chime',   notes: [{ f: 523,  s: 0,    d: 0.16 }, { f: 659,  s: 0.18, d: 0.16 }, { f: 784, s: 0.36, d: 0.22 }], loopLen: 1.1 },
    { id: 'marimba', notes: [{ f: 330,  s: 0,    d: 0.30 }, { f: 392,  s: 0.28, d: 0.30 }, { f: 494, s: 0.56, d: 0.35 }], loopLen: 1.3 },
    { id: 'digital', notes: [{ f: 1200, s: 0,    d: 0.10 }, { f: 1200, s: 0.14, d: 0.10 }, { f: 1200, s: 0.28, d: 0.10 }], loopLen: 0.7 },
    { id: 'gentle',  notes: [{ f: 440,  s: 0,    d: 0.7  }], loopLen: 1.0 }
  ];
  const RINGTONE_DEFAULT_ID = 'classic';

  function _cacheCustomRingtoneUrl(url) {
    try {
      if (url) localStorage.setItem('ringtone_custom_url', url);
      else localStorage.removeItem('ringtone_custom_url');
    } catch (e) {}
  }
  function _getCachedCustomRingtoneUrl() {
    try { return localStorage.getItem('ringtone_custom_url') || null; } catch (e) { return null; }
  }

  function getRingtonePref() {
    const id = localStorage.getItem('ringtone_pref_v1');
    if (id === 'custom' && _getCachedCustomRingtoneUrl()) return 'custom';
    return RINGTONE_PRESETS.some(r => r.id === id) ? id : RINGTONE_DEFAULT_ID;
  }

  function getRingtonePreset(id) {
    return RINGTONE_PRESETS.find(r => r.id === id) || RINGTONE_PRESETS[0];
  }

  function selectRingtone(id) {
    if (id !== 'custom' && !RINGTONE_PRESETS.some(r => r.id === id)) return;
    if (id === 'custom' && !_getCachedCustomRingtoneUrl()) return; // এখনো আপলোড হয়নি
    localStorage.setItem('ringtone_pref_v1', id);
    refreshRingtoneSettingsUi();
    previewRingtone(id);
    if (typeof showCartToast === 'function') showCartToast(t('ringtoneSavedToast'));
  }

  function refreshRingtoneSettingsUi() {
    const selected = getRingtonePref();
    RINGTONE_PRESETS.forEach(r => {
      const card  = document.getElementById(`ringtone-card-${r.id}`);
      const check = document.getElementById(`ringtone-check-${r.id}`);
      if (!card) return;
      const isSel = r.id === selected;
      card.classList.toggle('border-orange-500', isSel);
      card.classList.toggle('bg-orange-50', isSel);
      card.classList.toggle('border-slate-200', !isSel);
      if (check) check.classList.toggle('hidden', !isSel);
    });
    // ✅ কাস্টম (ফোন থেকে আপলোড করা) কার্ড — দুই অবস্থা: আপলোড করা আছে / নেই
    const customUrl    = _getCachedCustomRingtoneUrl();
    const uploadState   = document.getElementById('ringtone-custom-upload-state');
    const uploadedState = document.getElementById('ringtone-custom-uploaded-state');
    const customCard    = document.getElementById('ringtone-card-custom');
    const customCheck   = document.getElementById('ringtone-check-custom');
    if (uploadState && uploadedState) {
      uploadState.classList.toggle('hidden', !!customUrl);
      uploadedState.classList.toggle('hidden', !customUrl);
      if (customUrl) {
        const nameEl = document.getElementById('ringtone-custom-filename');
        if (nameEl) {
          const cachedName = localStorage.getItem('ringtone_custom_filename');
          nameEl.innerText = cachedName || nameEl.innerText || 'আমার রিংটোন';
        }
      }
    }
    if (customCard) {
      const isSel = selected === 'custom';
      customCard.classList.toggle('border-orange-500', isSel);
      customCard.classList.toggle('bg-orange-50', isSel);
      customCard.classList.toggle('border-slate-200', !isSel);
    }
    if (customCheck) customCheck.classList.toggle('hidden', selected !== 'custom');
  }

  // ✅ ফোন থেকে অডিও ফাইল বেছে আপলোড করা — Firebase Storage-এ প্রাইভেট uid-স্কোপড পাথে,
  // এবং users/{uid}.customRingtoneUrl-এ রেফারেন্স সেভ (Firestore rules ইতিমধ্যে owner-কে
  // নিজের ডকুমেন্টের নতুন ফিল্ড merge করতে দেয় বলে rules বদলাতে হয়নি)
  async function uploadCustomRingtone(inputEl) {
    const file = inputEl && inputEl.files && inputEl.files[0];
    if (!file) return;
    if (!currentUser) { showCartToast('⚠️ আপলোড করতে আগে লগইন করুন'); inputEl.value = ''; return; }
    if (!file.type.startsWith('audio/')) { showCartToast('⚠️ শুধু অডিও ফাইল (mp3/m4a/ogg/wav) আপলোড করা যাবে'); inputEl.value = ''; return; }
    if (file.size > 15 * 1024 * 1024) { showCartToast('⚠️ ফাইল সাইজ ১৫MB-এর কম হতে হবে'); inputEl.value = ''; return; }

    const statusEl = document.getElementById('ringtone-custom-upload-status');
    if (statusEl) { statusEl.classList.remove('hidden'); statusEl.innerText = '⏳ আপলোড হচ্ছে...'; }

    try {
      const oldPath = localStorage.getItem('ringtone_custom_storage_path');
      const ext = (file.name.split('.').pop() || 'mp3').toLowerCase().replace(/[^a-z0-9]/g, '') || 'mp3';
      const path = `ringtones/${currentUser.uid}/custom_${Date.now()}.${ext}`;
      const ref  = firebase.storage().ref(path);
      await ref.put(file, { contentType: file.type });
      const url = await ref.getDownloadURL();

      await firestore.collection('users').doc(currentUser.uid).set({ customRingtoneUrl: url }, { merge: true });

      userCustomRingtoneUrl = url;
      _cacheCustomRingtoneUrl(url);
      localStorage.setItem('ringtone_custom_storage_path', path);
      localStorage.setItem('ringtone_custom_filename', file.name);

      // ✅ পুরনো ফাইল থাকলে ব্যাকগ্রাউন্ডে ডিলিট (স্টোরেজ জমে থাকা এড়াতে) — ব্যর্থ হলেও সমস্যা নেই
      if (oldPath && oldPath !== path) {
        firebase.storage().ref(oldPath).delete().catch(() => {});
      }
      ...
      console.error('Custom ringtone upload error:', e);
      if (statusEl) { statusEl.innerText = '❌ আপলোড ব্যর্থ হয়েছে, আবার চেষ্টা করুন'; }
      showCartToast('❌ রিংটোন আপলোড করা যায়নি');
    } finally {
      inputEl.value = '';
    }
  }
  ...
  function removeCustomRingtone() {
    const path = localStorage.getItem('ringtone_custom_storage_path');
    if (path) firebase.storage().ref(path).delete().catch(() => {});
    if (currentUser) {
      firestore.collection('users').doc(currentUser.uid)
        .set({ customRingtoneUrl: firebase.firestore.FieldValue.delete() }, { merge: true })
        .catch(() => {});
    }
    userCustomRingtoneUrl = null;
    _cacheCustomRingtoneUrl(null);
    localStorage.removeItem('ringtone_custom_storage_path');
    localStorage.removeItem('ringtone_custom_filename');
    if (getRingtonePref() === 'custom' || localStorage.getItem('ringtone_pref_v1') === 'custom') {
      localStorage.setItem('ringtone_pref_v1', RINGTONE_DEFAULT_ID);
    }
    refreshRingtoneSettingsUi();
    showCartToast('🗑️ কাস্টম রিংটোন মুছে ফেলা হয়েছে');
  }

  // ✅ অফলাইন WAV জেনারেটর — একাধিক নোট (মেলোডি) থেকে একটি লুপ-যোগ্য বিপ ওয়েভফাইল তৈরি করে
  function _buildMelodyWav(notes, loopLen) {
    const sampleRate = 22050;
    const numSamples  = Math.floor(sampleRate * loopLen);
    const buf = new Int16Array(numSamples);
    notes.forEach(n => {
      const startI = Math.floor(n.s * sampleRate);
      const endI   = Math.min(numSamples, Math.floor((n.s + n.d) * sampleRate));
      for (let i = startI; i < endI; i++) {
        const t   = (i - startI) / sampleRate;
        const env = t < 0.03 ? (t / 0.03) : (t > n.d - 0.05 ? Math.max(0, (n.d - t) / 0.05) : 1);
        const v   = Math.sin(2 * Math.PI * n.f * (i / sampleRate));
        buf[i] = Math.max(-32000, Math.min(32000, buf[i] + Math.round(v * env * 24000)));
      }
    });
  ...
  let _ringtonePreviewAudio = null;
  function previewRingtone(id) {
    try {
      if (_ringtonePreviewAudio) { _ringtonePreviewAudio.pause(); _ringtonePreviewAudio.src = ''; _ringtonePreviewAudio = null; }
      if (id === 'custom') {
        const url = _getCachedCustomRingtoneUrl();
        if (!url) return;
        _ringtonePreviewAudio = new Audio(url);
      } else {
        const preset = getRingtonePreset(id);
        const uri = _buildMelodyWav(preset.notes, preset.loopLen);
        _ringtonePreviewAudio = new Audio(uri);
        _ringtonePreviewAudio.loop = true;
      }
      _ringtonePreviewAudio.play().catch(() => {});
      setTimeout(() => {
        if (_ringtonePreviewAudio) { _ringtonePreviewAudio.pause(); _ringtonePreviewAudio.src = ''; _ringtonePreviewAudio = null; }
      }, 2600);
    } catch (e) {}
    ...
    try {
      ...
      if (navigator.vibrate) {
        if (type === 'incoming') {
          ...
          _vibrateInterval = setInterval(() => navigator.vibrate([200, 800]), 3000);
        }
      }

      // ── 2. Audio element (data URI বা কাস্টম আপলোড করা ফাইলের URL) ─────────
      try {
        // ✅ ইনকামিং কলের জন্য ইউজারের সিলেক্ট করা রিংটোন প্রেফারেন্স ব্যবহার হবে
        // (বিল্ট-ইন প্রিসেট অথবা ফোন থেকে আপলোড করা কাস্টম ফাইল); আউটগোয়িং রিং-ব্যাক টোন
        // সবসময় স্ট্যান্ডার্ড থাকে
        let audioSrc;
        const pref = type === 'incoming' ? getRingtonePref() : null;
        if (pref === 'custom') {
          audioSrc = _getCachedCustomRingtoneUrl();
        }
        if (!audioSrc) {
          audioSrc = type === 'incoming'
            ? _buildMelodyWav(getRingtonePreset(pref || RINGTONE_DEFAULT_ID).notes, getRingtonePreset(pref || RINGTONE_DEFAULT_ID).loopLen)
            : _buildBeepWav(type);
        }
        _ringtoneAudio = new Audio(audioSrc);
        _ringtoneAudio.loop = true;
        _ringtoneAudio.volume = 1.0;
        // ✅ কাস্টম ফাইল লোড ব্যর্থ হলে (মুছে গেছে/নেটওয়ার্ক সমস্যা) ক্লাসিক প্রিসেটে ফলব্যাক
        if (pref === 'custom') {
          _ringtoneAudio.addEventListener('error', () => {
            if (_ringtoneAudio) { try { _ringtoneAudio.pause(); } catch (e) {} }
            _ringtoneAudio = new Audio(_buildMelodyWav(getRingtonePreset(RINGTONE_DEFAULT_ID).notes, getRingtonePreset(RINGTONE_DEFAULT_ID).loopLen));
            _ringtoneAudio.loop = true; _ringtoneAudio.volume = 1.0;
            _ringtoneAudio.play().catch(() => _webAudioFallback(type));
          }, { once: true });
        }
  ...
  function _webAudioFallback(type) {
    ...
    try {
      ...
      const beep = () => {
        ...
        const resume = () => {
          // ✅ ফলব্যাক টোনেও ইউজারের সিলেক্ট করা রিংটোনের ফ্রিকোয়েন্সি ব্যবহার হবে
          const freqs = type === 'incoming'
            ? getRingtonePreset(getRingtonePref()).notes.map(n => n.f)
            : [440];
  ...
  // ============================================================
  // ✅ NEW (feature-90): লাইভ ভাউচার সিস্টেম — কেনা পণ্যের উপর লাইভ করলে
  // পণ্যের দামের একটা শতাংশ ভাউচার হিসেবে জমা হয়, যা পরে পণ্য কিনতে ব্যবহার করা যায়
  // ============================================================
  let userVoucherBalance = 0; // প্রোফাইলে দেখানো ভাউচার ব্যালেন্স
  let userCustomRingtoneUrl = null; // ✅ NEW: ইউজারের নিজের ফোন থেকে আপলোড করা কাস্টম রিংটোনের ডাউনলোড URL
  let _liveVoucherSession = null; // {productId, name, price, token, credited} — টোকেন যাচাই সফল হলে সেট হয়, লাইভ শেষে ভাউচার জমা করতে ব্যবহৃত হয়
  const VOUCHER_PERCENT = 0.05; // ℹ️ (feature-91) এই মানটা এখন শুধু তথ্যের জন্য — আসল হিসাব হয় Cloud Function-এ (functions/index.js এর VOUCHER_PERCENT এর সাথে মিলিয়ে রাখবেন)
  let voucherRedeemActive = false; // চেকআউটে ভাউচার রিডিম চেকবক্স সক্রিয় কিনা
  // ✅ NEW (অ্যান্টি-অ্যাবিউজ ফিক্স): টোকেন বারবার ব্যবহার করে ভাউচার "ফার্মিং" ও সাথে সাথে
  // লাইভ শেষ করে ভাউচার নেওয়া ঠেকাতে —
  // ✅ NEW (feature-91): ন্যূনতম লাইভ-সময়ের চেক এখন সম্পূর্ণ Cloud Function-এ (সার্ভার-সাইড) হয়,
  // তাই আগের ক্লায়েন্ট-সাইড টাইমার ভেরিয়েবল আর দরকার নেই।
  let isMobileNumberVerified = false;
  let base64SelfieString = "";
  let base64NidString = "";
  let base64SellerLogoString = "";
  let base64UploadedProductImageString = "";
  // ✅ NEW (feature-74): একাধিক ছবি + ভিডিও আপলোডের জন্য
  let sellerProductImageFiles = []; // File অবজেক্টের অ্যারে (সর্বোচ্চ ৬টি)
  const SELLER_PRODUCT_MAX_IMAGES = 6;
  let sellerProductVideoFile = null; // একটি ভিডিও ফাইল (ঐচ্ছিক)
      ...
      <div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-100 flex flex-col justify-between group">
        <div>
          <div class="overflow-hidden relative cursor-pointer" onclick="openProductDetail('${p.id}')">
            <img src="${escapeHtml(p.image)}" loading="lazy" class="w-full h-64 object-cover transform group-hover:scale-105 transition duration-500" alt="${escapeHtml(p.name)}" onerror="this.src='https://placehold.co/400x400/f8fafc/94a3b8?text=ছবি+নেই'">
          ...
          <div class="p-3 pb-1">
            <h4 onclick="openProductDetail('${p.id}')" class="font-bold text-slate-800 text-xs leading-snug line-clamp-2 cursor-pointer">${escapeHtml(p.name)}</h4>
            ${renderStarsHtml(getAvgRating(p), p.ratingCount || 0)}
            <p class="text-[9px] font-semibold text-slate-400 mt-1 mb-1 uppercase tracking-wider">সাইজ সিলেক্ট করুন</p>
            <div class="flex flex-wrap gap-1.5 mb-1" id="size-container-${p.id}">${sizeButtonsHtml}</div>
        ...
        <div class="p-3 pt-0">
          ...
          <div class="flex items-center justify-center gap-3 mb-1.5 pt-1.5 border-t border-slate-50">
          ...
          <div class="flex items-center justify-between mt-1.5 pt-1.5 border-t border-slate-50">
            ...
            <button onclick="addToCart('${p.id}', this)" ${p.stock <= 0 ? 'disabled class="bg-slate-300 text-slate-500 text-[10px] font-bold px-3 py-2 rounded-xl cursor-not-allowed"' : 'class="bg-gradient-to-r from-fuchsia-600 to-purple-700 text-white text-[10px] font-bold px-3 py-2 rounded-xl shadow-md shadow-fuchsia-500/30 active:scale-95 transition-all"'}>
  ...
  function openProductDetail(productId) {
    ...
    if (product.stock <= 0) {
      ...
      addBtn.className = 'w-full bg-gradient-to-r from-fuchsia-600 to-purple-700 text-white font-bold text-sm py-3 rounded-2xl shadow-lg shadow-fuchsia-500/30 hover:shadow-fuchsia-500/50 active:scale-95 transition';
    ...
    renderDetailVideo(product); // ✅ NEW (feature-74)
  ...
  // ✅ কার্ট ফাংশন
  // ============================================================
  // ✅ NEW: ক্লিক করার সাথে সাথে বাটনের রং সাময়িক পরিবর্তন (সবুজ ✓) করে ফিডব্যাক দেখানো
  function flashAddToCartButton(btn) {
    if (!btn) return;
    if (btn.dataset.flashing === '1') return; // ডাবল-ক্লিকে ওভারল্যাপ এড়ানো
    btn.dataset.flashing = '1';
    const originalClass = btn.className;
    const originalHTML = btn.innerHTML;
    btn.className = originalClass.replace(/bg-gradient-to-r from-fuchsia-600 to-purple-700/g, 'bg-emerald-600').replace(/shadow-fuchsia-500\/\d+/g, 'shadow-emerald-500/30').replace(/hover:shadow-fuchsia-500\/\d+/g, '');
    btn.innerHTML = `<i class="fas fa-check"></i> ${originalHTML.includes('কার্টে') ? 'যোগ হয়েছে!' : originalHTML}`;
    setTimeout(() => {
  ...
  function addToCart(id, btnEl) {
    ...
    flashAddToCartButton(btnEl || document.getElementById('detail-add-to-cart-btn'));
  ...
  function calculateOrderTotals() {
    ...
    // ✅ NEW (feature-90): ভাউচার রিডিম — ভাউচার ব্যালেন্স সরাসরি টাকার সমান, তাই ১:১ ছাড়
    let voucherDiscount = 0;
    if (voucherRedeemActive && userVoucherBalance > 0) {
      const remainingAfterOthers = Math.max(subtotal - couponDiscount - pointsDiscount, 0);
      voucherDiscount = Math.min(userVoucherBalance, remainingAfterOthers);
    }

    const totalDiscount = Math.min(couponDiscount + pointsDiscount + voucherDiscount, subtotal);
    const total = Math.max(subtotal - totalDiscount, 0);
    const pointsUsed = pointsDiscount * 10;

    return { subtotal, couponDiscount, pointsDiscount, voucherDiscount, totalDiscount, total, pointsUsed };
  }
  ...
  function updateOrderSummaryUi() {
    ...
    // ✅ NEW (feature-90): ভাউচার রিডিম বক্স দেখাও/লুকাও
    const voucherBox = document.getElementById('voucher-redeem-box');
    const voucherLabel = document.getElementById('voucher-redeem-label');
    if (voucherBox && voucherLabel) {
      if (currentUser && userVoucherBalance > 0) {
        voucherBox.classList.remove('hidden');
        voucherLabel.innerText = `আপনার ভাউচারে ৳${userVoucherBalance.toLocaleString('bn-BD')} আছে — এটি দিয়ে ছাড় নিতে চান?`;
      } else {
        voucherBox.classList.add('hidden');
        voucherRedeemActive = false;
      }
    }
  }

  function toggleVoucherRedeem() {
    voucherRedeemActive = document.getElementById('voucher-redeem-checkbox').checked;
    updateOrderSummaryUi();
  }
  ...
  async function placeOrder() {
    ...
    const orderData = {
      ...
      // ✅ NEW (feature-90): প্রতিটা আইটেমের সাথে ইউনিক টোকেন — প্রোফাইলের "product history"-তে
      // এই টোকেন কপি করে পরবর্তীতে লাইভ স্ট্রিমিংয়ে যাচাইয়ের জন্য ব্যবহার করা যাবে
      items: cart.map(item => ({ id: item.id, name: item.name, price: item.price, quantity: item.quantity, size: item.size, image: item.image, sellerUid: item.sellerUid || 'default', token: generateProductToken() })),
      ...
      voucherRedeemed: Math.round(totals.voucherDiscount), // ✅ NEW (feature-90)
    ...
    try {
      ...
      // ✅ NEW (feature-90): ভাউচার রিডিম হলে ব্যালেন্স থেকে কাটো
      if (currentUser && totals.voucherDiscount > 0) {
        userVoucherBalance = Math.max(0, userVoucherBalance - Math.round(totals.voucherDiscount));
        firestore.collection('users').doc(currentUser.uid).set({
          voucherBalance: userVoucherBalance
        }, { merge: true }).catch(e => console.error("Voucher deduct error:", e));
      }
      ...
      appliedCoupon = null;
      loyaltyRedeemActive = false;
      voucherRedeemActive = false; // ✅ NEW (feature-90)
      ...
      const voucherCheckbox = document.getElementById('voucher-redeem-checkbox');
      if (voucherCheckbox) voucherCheckbox.checked = false;
      updateVoucherUI(); // ✅ NEW (feature-90)
  ...
  async function startCustomerOwnLive() {
    ...
    isBroadcasting = true;
    _liveVoucherSession = null; // ✅ NEW (feature-90): নতুন লাইভ শুরুতে আগের সেশনের ভাউচার তথ্য রিসেট
  ...
  async function closeLiveStreamModule() {
    ...
    if (isBroadcasting && currentUser) {
      await db.ref(`live_streams/${currentUser.uid}`).remove();
      // ✅ NEW (feature-90): কাস্টমার নিজে লাইভ শেষ করলে — টোকেন-যাচাইকৃত পণ্য থাকলে ভাউচার জমা করো
      await _creditLiveVoucherIfPending();
    }
    isBroadcasting = false;
  }

  // ============================================================
  // ✅ NEW (feature-90): লাইভ শেষ হলে ভাউচার জমা করার লজিক (একবারই জমা হবে)
  // ============================================================
  async function _creditLiveVoucherIfPending() {
    if (!_liveVoucherSession || !currentUser) { _liveVoucherSession = null; return; }
    const session = _liveVoucherSession;
    _liveVoucherSession = null; // সেশন এখানেই ক্লিয়ার করা হলো যাতে ভুলবশত দ্বিতীয়বার কল না হয়

    // ✅ NEW (feature-91): ভাউচার ক্রেডিট করা এখন সম্পূর্ণ সার্ভার-সাইড (Cloud Function)।
    // এখানে শুধু সেশন আইডি পাঠানো হয় — সময়সীমা (ন্যূনতম ৩ মিনিট), পরিমাণ হিসাব,
    // এবং একবার-ব্যবহার লক — সবকিছু Google-এর সার্ভারে যাচাই হয়, ক্লায়েন্ট থেকে কারচুপির সুযোগ নেই।
    try {
      const creditFn = cloudFunctions.httpsCallable('creditLiveVoucher');
      const result = await creditFn({ sessionId: session.sessionId });
      const data = result.data || {};
      if (data.credited) {
        userVoucherBalance = (data.newBalance != null) ? data.newBalance : (userVoucherBalance + (data.amount || 0));
        updateVoucherUI();
        showCartToast(`🎉 লাইভ শেষ হয়েছে! আপনার ভাউচারে ৳${data.amount} জমা হয়েছে।`, 'success');
      } else {
        showCartToast('ℹ️ ' + (data.message || 'এবার ভাউচার জমা হয়নি।'), 'info');
      }
    } catch (e) {
      console.error('Voucher credit error:', e);
      const msg = (e && e.message) ? e.message : 'ভাউচার জমা করা যায়নি — ইন্টারনেট সংযোগ চেক করুন।';
      showCartToast('⚠️ ' + msg, 'error');
    }
  }
  ...
  async function selectLiveProductForBroadcast(productId) {
    ...
    // ✅ NEW (feature-90): কাস্টমার নিজের লাইভে নিজের কেনা পণ্য দেখাতে চাইলে প্রথমে
    // টোকেন যাচাই বাধ্যতামূলক — মিলে গেলেই লাইভে পণ্য যুক্ত হবে
    if (_liveProductPickerContext === 'customer') {
      closeLiveProductPicker();
      openLiveTokenVerifyModal(product);
      return;
    }

    await _applyLiveProductSelection(product);
  }

  // পণ্য আসলেই লাইভে সেট করার কমন লজিক (টোকেন যাচাইয়ের প্রয়োজন নেই এমন সেলার-কনটেক্সট,
  // এবং যাচাই সফল হওয়া কাস্টমার-কনটেক্সট — দুটোতেই ব্যবহৃত হয়)
  async function _applyLiveProductSelection(product) {
    try {
    ...
    if (badge) {
      const voucherHint = _liveProductPickerContext === 'customer'
        ? `<p class='text-[9px] text-fuchsia-600 font-bold mt-0.5'><i class="fas fa-ticket"></i> লাইভ শেষে ভাউচার জমা হবে</p>` : '';
      badge.classList.remove('hidden');
      badge.innerHTML = `
  ...
  // ============================================================
  // ✅ NEW (feature-90): লাইভে পণ্য দেখানোর আগে ক্রয়-টোকেন যাচাই মডাল (শুধু কাস্টমারের নিজের লাইভ)
  // ============================================================
  let _pendingLiveTokenProduct = null;

  function openLiveTokenVerifyModal(product) {
    _pendingLiveTokenProduct = product;
    const nameEl = document.getElementById('live-token-verify-product-name');
    if (nameEl) nameEl.innerText = product.name;
    const input = document.getElementById('live-token-verify-input');
    if (input) input.value = '';
    const err = document.getElementById('live-token-verify-error');
    if (err) err.classList.add('hidden');
    document.getElementById('live-token-verify-modal')?.classList.remove('hidden');
  }

  function closeLiveTokenVerifyModal() {
    _pendingLiveTokenProduct = null;
    document.getElementById('live-token-verify-modal')?.classList.add('hidden');
  }

  async function submitLiveTokenVerify() {
    if (!_pendingLiveTokenProduct || !currentUser) return;
    const input = document.getElementById('live-token-verify-input');
    const token = (input?.value || '').trim().toUpperCase();
    const errEl = document.getElementById('live-token-verify-error');
    const btn = document.getElementById('live-token-verify-submit-btn');
    if (!token) { if (errEl) { errEl.innerText = 'টোকেন লিখুন!'; errEl.classList.remove('hidden'); } return; }

    if (btn) { btn.disabled = true; btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i> যাচাই হচ্ছে...`; }
    try {
      // ✅ NEW (feature-91): টোকেন-অর্ডার মিলানো, বাতিল-অর্ডার বাদ দেওয়া, আগে-ব্যবহৃত-টোকেন চেক —
      // সবকিছু এখন সার্ভারে (Cloud Function) হয়, ক্লায়েন্ট শুধু ফলাফল পায়। এভাবে কেউ ব্রাউজার
      // থেকে সরাসরি ডাটাবেজ কারচুপি করে ভাউচার নিতে পারবে না।
      const startSession = cloudFunctions.httpsCallable('startLiveVoucherSession');
      const result = await startSession({ productId: _pendingLiveTokenProduct.id, token });
      const data = result.data || {};

      // ✅ সেশন আইডি সার্ভার থেকে আসে — লাইভ শেষে এটা দিয়েই ভাউচার ক্রেডিট চাওয়া হবে
      _liveVoucherSession = { sessionId: data.sessionId, name: data.productName || _pendingLiveTokenProduct.name };
      await _applyLiveProductSelection(_pendingLiveTokenProduct);
      closeLiveTokenVerifyModal();
      showCartToast('✅ টোকেন যাচাই সফল! অন্তত ৩ মিনিট লাইভ চালিয়ে শেষ করলে ভাউচার জমা হবে।', 'success');
    } catch (e) {
      console.error('Token verify error:', e);
      // ✅ Cloud Function (HttpsError) থেকে আসা মেসেজ সরাসরি দেখাও (যেমন: "টোকেনটি সঠিক নয়...")
      const msg = (e && e.message) ? e.message : 'যাচাই করা যায়নি। ইন্টারনেট চেক করে আবার চেষ্টা করুন।';
  ...
  async function handleSellerProductUpload() {
    ...
    if (!sellerProductImageFiles.length) { alert('দয়া করে পণ্যের অন্তত একটি ছবি সিলেক্ট করুন!'); return; }
    ...
    if (uploadBtn) { uploadBtn.disabled = true; uploadBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i> ছবি/ভিডিও আপলোড হচ্ছে...`; }
    ...
    try {
      // ✅ NEW (feature-74): সবগুলো ছবি Firebase Storage-এ আপলোড করো (products/{uid}/...)
      const uploadedImageUrls = [];
      for (let i = 0; i < sellerProductImageFiles.length; i++) {
        const file = sellerProductImageFiles[i];
        const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
        const path = `products/${currentUser.uid}/img_${Date.now()}_${i}_${safeName}`;
        if (uploadBtn) uploadBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i> ছবি আপলোড হচ্ছে (${i + 1}/${sellerProductImageFiles.length})...`;
      ...
      // ✅ NEW (feature-74): ভিডিও থাকলে সেটিও Storage-এ আপলোড করো
      let uploadedVideoUrl = null;
      if (sellerProductVideoFile) {
        if (uploadBtn) uploadBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i> ভিডিও আপলোড হচ্ছে...`;
        const safeName = sellerProductVideoFile.name.replace(/[^a-zA-Z0-9._-]/g, '_');
        const path = `products/${currentUser.uid}/video_${Date.now()}_${safeName}`;
        const task = storage.ref(path).put(sellerProductVideoFile, { contentType: sellerProductVideoFile.type });
        await new Promise((res, rej) => task.on('state_changed', null, rej, res));
        uploadedVideoUrl = await task.snapshot.ref.getDownloadURL();
      }

      const newProduct = {
        name, price, stock: isNaN(stockInput) ? 10 : stockInput,
        image: uploadedImageUrls[0], // ব্যাকওয়ার্ড-কম্প্যাটিবিলিটির জন্য প্রধান ছবি
        images: uploadedImageUrls,   // ✅ NEW (feature-74): সবগুলো ছবির অ্যারে
        videoUrl: uploadedVideoUrl,  // ✅ NEW (feature-74): পণ্যের ভিডিও (থাকলে)
        sizes: rawSizesArray,
      ...
      if (uploadBtn) uploadBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-1"></i> সেভ হচ্ছে...`;
  ...
  function clearProductUploadFormFields() {
    ...
    // ✅ NEW (feature-74): ছবি/ভিডিও স্টেট রিসেট
    sellerProductImageFiles = [];
    sellerProductVideoFile = null;
    renderSellerProductImagePreviews();
    resetSellerProductVideoUi();
  }

  // ✅ NEW (feature-74): একাধিক ছবি সিলেক্ট করলে অ্যারেতে যোগ করো (সর্বোচ্চ ৬টি)
  function processProductImageUpload(input) {
    if (!input.files || !input.files.length) return;
    const incoming = Array.from(input.files);
    const room = SELLER_PRODUCT_MAX_IMAGES - sellerProductImageFiles.length;
    if (room <= 0) {
      showCartToast(`⚠️ সর্বোচ্চ ${SELLER_PRODUCT_MAX_IMAGES}টি ছবি আপলোড করা যাবে`);
      input.value = '';
      return;
    }
    sellerProductImageFiles = sellerProductImageFiles.concat(incoming.slice(0, room));
    if (incoming.length > room) {
      showCartToast(`⚠️ সর্বোচ্চ ${SELLER_PRODUCT_MAX_IMAGES}টি ছবি রাখা হয়েছে`);
    }
    input.value = ''; // একই ফাইল আবার সিলেক্ট করা যাবে
    renderSellerProductImagePreviews();
  }

  // ✅ NEW (feature-74): নির্বাচিত ছবি প্রিভিউ গ্রিড রেন্ডার করো
  function renderSellerProductImagePreviews() {
    const grid = document.getElementById('product-images-preview-grid');
    if (!grid) return;
    const statusText = document.getElementById('product-file-status-text');
    let html = sellerProductImageFiles.map((file, i) => {
      const objUrl = URL.createObjectURL(file);
      return `
        <div class='relative aspect-square'>
          <img src="${objUrl}" class='w-full h-full object-cover rounded-xl border border-slate-200'/>
          ${i === 0 ? `<span class='absolute bottom-1 left-1 bg-orange-600 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full'>প্রধান</span>` : ''}
          <button type='button' onclick='removeSellerProductImage(${i})' class='absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 text-white text-[9px] flex items-center justify-center'>
            <i class='fas fa-times'></i>
          </button>
        </div>`;
    }).join('');
    if (sellerProductImageFiles.length < SELLER_PRODUCT_MAX_IMAGES) {
      html += `
        <label class='flex flex-col items-center justify-center aspect-square border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition'>
          <i class='fas fa-plus text-slate-400 text-base'></i>
          <span class='text-[9px] font-bold text-slate-400 mt-1' id='product-file-status-text'>ছবি যোগ করুন</span>
          <input class='hidden' onchange='processProductImageUpload(this)' type='file' accept='image/*' multiple/>
        </label>`;
    }
    grid.innerHTML = html;
    if (!statusText) { /* re-render already includes fresh status text node above */ }
  }

  // ✅ NEW (feature-74): নির্দিষ্ট ইনডেক্সের ছবি বাদ দাও
  function removeSellerProductImage(index) {
    sellerProductImageFiles.splice(index, 1);
    renderSellerProductImagePreviews();
  }

  // ✅ NEW (feature-74): পণ্যের ভিডিও সিলেক্ট হ্যান্ডলার
  function processProductVideoUpload(input) {
    if (input.files && input.files[0]) {
      const file = input.files[0];
      if (file.size > 80 * 1024 * 1024) {
        showCartToast('⚠️ ভিডিও ফাইলের সাইজ ৮০MB এর কম হতে হবে');
        input.value = '';
        return;
      }
      sellerProductVideoFile = file;
      const objUrl = URL.createObjectURL(file);
      document.getElementById('product-video-upload-ui-box')?.classList.add('hidden');
      const previewVid = document.getElementById('product-video-preview');
      if (previewVid) { previewVid.src = objUrl; previewVid.classList.remove('hidden'); }
      document.getElementById('product-video-remove-btn')?.classList.remove('hidden');
      document.getElementById('product-video-remove-btn')?.classList.add('flex');
    }
  }

  // ✅ NEW (feature-74): সিলেক্ট করা ভিডিও বাদ দাও
  function removeSellerProductVideo() {
    sellerProductVideoFile = null;
    resetSellerProductVideoUi();
  }

  function resetSellerProductVideoUi() {
    const uploader = document.getElementById('product-video-uploader');
    if (uploader) uploader.value = '';
    document.getElementById('product-video-upload-ui-box')?.classList.remove('hidden');
    const previewVid = document.getElementById('product-video-preview');
    if (previewVid) { previewVid.src = ''; previewVid.classList.add('hidden'); }
    const removeBtn = document.getElementById('product-video-remove-btn');
  ...
  function openProfile() {
    ...
    // ✅ FIX: প্রোফাইল মোডাল এখন লগইন থাকুক বা না থাকুক — সবসময় ফুলস্ক্রিন (100vh, কোনো blur গ্যাপ নেই)
    ...
    if (!currentUser) {
      ...
      updateVoucherUI(); // ✅ NEW (feature-90): ভাউচার ব্যালেন্স দেখাও
      renderSellerPanelByRole();
  ...
  let _adminNotifUnsub = null; // ✅ NEW (feature-70): লগ আউটে বন্ধ করার জন্য রেফারেন্স রাখা হলো
  function initAdminNotificationListener() {
    if (!currentUser || !ADMIN_EMAILS.includes(currentUser.email)) return;
    if (_adminNotifUnsub) { _adminNotifUnsub(); _adminNotifUnsub = null; }

    _adminNotifUnsub = firestore.collection("admin_notifications")
      .where("isRead", "==", false)
      .onSnapshot((snapshot) => {
      ...
      }, () => {}); // ✅ NEW (feature-70): এরর হ্যান্ডলার — লগ আউটের সময় permission-denied কনসোলে স্প্যাম না করার জন্য
  }
  ...
  function openBottomSheet(sheetId) {
    ...
    if (sheetId === 'ringtone-settings-sheet') refreshRingtoneSettingsUi();
    if (sheetId === 'my-orders-sheet') loadMyOrdersList();
  }
  function closeBottomSheet(sheetId) {
    const sheet = document.getElementById(sheetId);
    if (sheet) sheet.classList.add('hidden');
    if (sheetId === 'ringtone-settings-sheet' && _ringtonePreviewAudio) {
      try { _ringtonePreviewAudio.pause(); _ringtonePreviewAudio.src = ''; } catch (e) {}
      _ringtonePreviewAudio = null;
    }
  }
  ...
  // ============================================================
  // ✅ NEW (feature-90): ভাউচার কার্ড UI
  // ============================================================
  function updateVoucherUI() {
    const amountText = '৳' + (userVoucherBalance || 0).toLocaleString('bn-BD');
    const headerEl = document.getElementById('profile-voucher-amount');
    const sheetEl  = document.getElementById('voucher-sheet-amount');
    if (headerEl) headerEl.innerText = amountText;
    if (sheetEl)  sheetEl.innerText  = amountText;
  }

  function openVoucherSheet() {
    if (!currentUser) { alert('ভাউচার দেখতে লগইন করুন প্রথমে!'); return; }
    updateVoucherUI();
    openBottomSheet('voucher-sheet');
    loadVoucherHistory();
  }

  // নিজের ভাউচার জমার হিস্টরি লোড করো (সর্বশেষ ২০টি)
  async function loadVoucherHistory() {
    const listEl = document.getElementById('voucher-history-list');
    if (!listEl || !currentUser) return;
    try {
      const snap = await firebase.firestore().collection('voucher_history')
        .where('uid', '==', currentUser.uid)
        .orderBy('createdAt', 'desc')
        .limit(20)
        .get();
      if (snap.empty) {
        listEl.innerHTML = `<p class='text-[10px] text-slate-400 text-center py-3' id='voucher-history-empty'>এখনো কোনো ভাউচার জমা হয়নি</p>`;
        return;
      }
      listEl.innerHTML = snap.docs.map(doc => {
        const d = doc.data();
        ...
        return `
          <div class='bg-white border border-slate-200 rounded-xl p-3 flex items-center justify-between gap-2'>
            <div>
              <p class='text-xs font-black text-slate-800'>+৳${(d.amount || 0).toLocaleString('bn-BD')}</p>
              <p class='text-[9px] text-slate-400 mt-0.5'>লাইভে "${escapeHtml(d.productName || '')}" প্রমোট করার জন্য • ${dateStr}</p>
            </div>
            <span class='text-[9px] font-black px-2 py-1 rounded-full border shrink-0 bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200'><i class="fas fa-ticket"></i></span>
          </div>`;
      }).join('');
    } catch (e) {
      console.error('Voucher history load error:', e);
  ...
  function renderOrderTrackResult(data) {
    ...
    resultDiv.innerHTML = `
      <div class="bg-white border border-slate-100 rounded-3xl p-5 mt-1 shadow-sm overflow-hidden">
        ...
        ${_buildOrderItemsTokenSection(data)}
  ...
  // ============================================================
  // ✅ NEW (feature-90): "product history" — কেনা প্রতিটি পণ্যের ডিটেলস ও টোকেন কপি বাটন
  // ============================================================
  function _buildOrderItemsTokenSection(data) {
    const items = data.items || [];
    if (items.length === 0) return '';
    const rows = items.map((item, idx) => {
      const tokenBlock = item.token ? `
          <div class='flex items-center gap-1.5 mt-1.5'>
            <code class='text-[9px] bg-slate-100 border border-slate-200 rounded-lg px-2 py-1 font-bold text-slate-600 tracking-wider select-all'>${escapeHtml(item.token)}</code>
            <button onclick="copyProductToken('${escapeHtml(item.token)}', this)" class='text-[9px] font-black bg-fuchsia-50 text-fuchsia-700 border border-fuchsia-200 px-2 py-1 rounded-lg active:scale-95 transition'>
              <i class='fas fa-copy'></i> টোকেন কপি
            </button>
          </div>` : '';
      return `
        <div class='flex items-center gap-2.5 py-2 ${idx > 0 ? 'border-t border-slate-100' : ''}'>
          <img src="${item.image || ''}" loading="lazy" class='w-11 h-11 rounded-lg object-cover bg-slate-100 shrink-0' onerror="this.src='https://placehold.co/44x44/f8fafc/94a3b8?text=?'">
          <div class='min-w-0 flex-1'>
            <p class='text-[11px] font-bold text-slate-800 truncate'>${escapeHtml(item.name || '')} ${item.quantity > 1 ? `× ${item.quantity}` : ''}</p>
            <p class='text-[10px] text-orange-500 font-black'>৳${item.price}</p>
            ${tokenBlock}
          </div>
        </div>`;
    }).join('');
    return `
        <div class='mb-5 pb-4 border-b border-slate-100'>
          <p class='text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2'>পণ্যের ডিটেলস <span class='normal-case text-slate-300'>(লাইভে দেখাতে টোকেন লাগবে)</span></p>
          ${rows}
        </div>`;
  }
  ...
  // ============================================================
  // ✅ UPDATED (feature-93): জীবন-এর ডকুমেন্টগুলো আগে base64 হিসেবে সরাসরি Firestore ডকে
  // সেভ হতো — ৩টা ছবিতে সহজেই Firestore-এর ১MB ডকুমেন্ট লিমিট ছাড়িয়ে যেত। এখন আসল ফাইল
  // Firebase Storage-এ (jibon_docs/{uid}/...) আপলোড হয়, Firestore-এ শুধু ডাউনলোড URL থাকে।
  let jibonDocs  = { medical: null, chairman: null, nid: null }; // প্রিভিউ base64 (শুধু UI-তে দেখানোর জন্য)
  let jibonFiles = { medical: null, chairman: null, nid: null }; // আসল File অবজেক্ট — আপলোডের সময় ব্যবহৃত হবে
  ...
  function previewJibonDoc(input, type) {
    ...
    if (!file.type.startsWith('image/')) { showCartToast('⚠️ একটি ছবি ফাইল বেছে নিন'); return; }
    const sizeMB = file.size / (1024 * 1024);
    if (sizeMB > 10) { showCartToast('⚠️ ছবিটি অনেক বড় (১০MB-এর বেশি)'); return; }
    jibonFiles[type] = file; // ✅ NEW (feature-93): আপলোডের জন্য আসল ফাইল রাখা হলো
    const reader = new FileReader();
    reader.onload = function(e) {
      jibonDocs[type] = e.target.result; // শুধু প্রিভিউ দেখানোর জন্য
  ...
  // ✅ NEW (feature-93): একটা জীবন ডকুমেন্ট ফাইল Firebase Storage-এ আপলোড করে ডাউনলোড URL রিটার্ন করে
  async function _uploadJibonDoc(type, file) {
    const safeName = (file.name || `${type}.jpg`).replace(/[^a-zA-Z0-9._-]/g, '_');
    const storagePath = `jibon_docs/${currentUser.uid}/${type}_${Date.now()}_${safeName}`;
    const uploadTask = storage.ref(storagePath).put(file, { contentType: file.type });
    await new Promise((resolve, reject) => uploadTask.on('state_changed', null, reject, resolve));
  ...
  async function submitJibonApplication() {
    ...
    if (!jibonFiles.medical)  { alert('মেডিকেল রিপোর্টের ছবি আপলোড করুন (ডাক্তারের স্বাক্ষরসহ)!'); return; }
    if (!jibonFiles.chairman) { alert('চেয়ারম্যানের সার্টিফিকেট আপলোড করুন (স্বাক্ষরসহ)!'); return; }
    if (!jibonFiles.nid)      { alert('জাতীয় পরিচয়পত্র / পাসপোর্টের ছবি আপলোড করুন!'); return; }
    if (!currentUser) { alert('আবেদন করতে আগে লগইন করুন!'); return; }

    const btn = document.getElementById('jibon-submit-btn');
    if (btn) { btn.disabled = true; btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> আপলোড হচ্ছে...`; }

    try {
      // ✅ NEW (feature-93): তিনটা ডকুমেন্ট Firebase Storage-এ আপলোড করা — Firestore-এ শুধু URL থাকবে
      const [medicalRes, chairmanRes, nidRes] = await Promise.all([
        _uploadJibonDoc('medical', jibonFiles.medical),
        _uploadJibonDoc('chairman', jibonFiles.chairman),
        _uploadJibonDoc('nid', jibonFiles.nid)
      ]);

      if (btn) btn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> জমা হচ্ছে...`;
      ...
      const applicationData = {
        ...
        applicantUid: currentUser.uid,
        medicalReportUrl: medicalRes.url,
        medicalReportPath: medicalRes.path,
        chairmanCertificateUrl: chairmanRes.url,
        chairmanCertificatePath: chairmanRes.path,
        nidOrPassportUrl: nidRes.url,
        nidOrPassportPath: nidRes.path,
        status: 'পেন্ডিং 🕐',
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      };
      ...
      jibonFiles = { medical: null, chairman: null, nid: null };
...
চামড়ার জুতো,2500,shoe,"40,41,42,43",15,খাঁটি চামড়ার তৈরি দীর্ঘস্থায়ী জুতো,https://example.com/shoe.jpg`;
  ...
  const TRANSLATIONS = {
    bn: {
      ...
      notifPrefSoundTitle:         'নোটিফিকেশন সাউন্ড',
      notifPrefSoundDesc:          'নতুন নোটিফিকেশন এলে টোন বাজবে',
      notifSoundTestBtn:           'টেস্ট',
      ...
      customerRowRingtoneLabel:    'কল রিংটোন',
      ringtoneSheetTitle:          'কল রিংটোন',
      ringtoneSheetHint:           'ইনকামিং অডিও/ভিডিও কলের জন্য আপনার পছন্দের রিংটোন বেছে নিন। প্রিভিউ শুনতে ▶ বাটনে চাপুন।',
      ringtoneClassic:             'ক্লাসিক',
      ringtoneChime:               'চাইম',
      ringtoneMarimba:             'মারিম্বা',
      ringtoneDigital:             'ডিজিটাল',
      ringtoneGentle:              'জেন্টল',
      ringtoneSavedToast:          '✅ রিংটোন সেভ হয়েছে',
      ringtoneCustomSectionLabel:  'আপনার ফোন থেকে',
      ringtoneCustomUploadTitle:   'ফোন থেকে রিংটোন আপলোড করুন',
      ringtoneCustomUploadDesc:    'আপনার ফোনে থাকা mp3/m4a/ogg ফাইল বেছে নিন',
      ringtoneCustomUploadBtn:     'আপলোড',
      ringtoneCustomChangeBtn:     'পরিবর্তন করুন',
      ringtoneCustomRemoveBtn:     'মুছে ফেলুন',
      ...
      productImageLabel:   'পণ্যের ছবিসমূহ (একাধিক আপলোড করুন) *',
      productImageSelectLabel: 'ছবি যোগ করুন',
      productVideoLabel:   'পণ্যের ভিডিও (ঐচ্ছিক)',
      productVideoSelectLabel: 'ভিডিও সিলেক্ট করুন',
    ...
    en: {
      ...
      notifPrefSoundTitle:         'Notification Sound',
      notifPrefSoundDesc:          'Play a tone when a new notification arrives',
      notifSoundTestBtn:           'Test',
      ...
      customerRowRingtoneLabel:    'Call Ringtone',
      ringtoneSheetTitle:          'Call Ringtone',
      ringtoneSheetHint:           'Choose your preferred ringtone for incoming audio/video calls. Tap ▶ to preview.',
      ringtoneClassic:             'Classic',
      ringtoneChime:               'Chime',
      ringtoneMarimba:             'Marimba',
      ringtoneDigital:             'Digital',
      ringtoneGentle:              'Gentle',
      ringtoneSavedToast:          '✅ Ringtone saved',
      ringtoneCustomSectionLabel:  'From your phone',
      ringtoneCustomUploadTitle:   'Upload a ringtone from your phone',
      ringtoneCustomUploadDesc:    'Choose an mp3/m4a/ogg file from your phone',
      ringtoneCustomUploadBtn:     'Upload',
      ringtoneCustomChangeBtn:     'Change',
      ringtoneCustomRemoveBtn:     'Remove',
      // Seller Panel — Top Section
      ...
      productImageLabel:   'Product Images (upload multiple) *',
      productImageSelectLabel: 'Add Image',
      productVideoLabel:   'Product Video (optional)',
      productVideoSelectLabel: 'Select Video',
  ...
  function applyLanguage() {
    ...
    const customerPanelLabels = {
      ...
      'notif-pref-sound-title':       'notifPrefSoundTitle',
      'notif-pref-sound-desc':        'notifPrefSoundDesc',
      'notif-sound-test-btn':         'notifSoundTestBtn',
      'customer-row-ringtone-label':  'customerRowRingtoneLabel',
      'ringtone-sheet-title':         'ringtoneSheetTitle',
      'ringtone-sheet-hint':          'ringtoneSheetHint',
      'ringtone-name-classic':        'ringtoneClassic',
      'ringtone-name-chime':          'ringtoneChime',
      'ringtone-name-marimba':        'ringtoneMarimba',
      'ringtone-name-digital':        'ringtoneDigital',
      'ringtone-name-gentle':         'ringtoneGentle',
      'ringtone-custom-section-label': 'ringtoneCustomSectionLabel',
      'ringtone-custom-upload-title':  'ringtoneCustomUploadTitle',
      'ringtone-custom-upload-desc':   'ringtoneCustomUploadDesc',
      'ringtone-custom-upload-btn':    'ringtoneCustomUploadBtn',
      'ringtone-custom-change-btn':    'ringtoneCustomChangeBtn',
      'ringtone-custom-remove-btn':    'ringtoneCustomRemoveBtn',
    };
    ...
    const sellerPanelLowerLabels = {
      ...
      'product-video-label':          'productVideoLabel',
      'product-video-file-status-text': 'productVideoSelectLabel',
  ...
  // ============================================================
  // ✅ NEW: নোটিফিকেশন টোন — Web Audio দিয়ে সিন্থেসাইজড ছোট্ট "ডিং-ডং" সাউন্ড
  // (কোনো এক্সটার্নাল অডিও ফাইল/নেটওয়ার্ক লাগে না — রিংটোন সিস্টেমের মতোই অফলাইন-সেফ)
  // ============================================================
  let _notifToneAudio = null;
  function playNotificationTone() {
    try {
      if (!getNotifPref('sound')) return; // ✅ ইউজার বন্ধ করে রাখলে বাজবে না
      if (_notifToneAudio) { try { _notifToneAudio.pause(); } catch (e) {} }
      // দুই-নোটের ছোট্ট চাইম (C6 → G6)
      const notes = [
        { f: 1046.5, s: 0,    d: 0.14 },
        { f: 1568.0, s: 0.12, d: 0.24 }
      ];
      const uri = _buildMelodyWav(notes, 0.38);
      _notifToneAudio = new Audio(uri);
      _notifToneAudio.volume = 0.55;
      _notifToneAudio.play().catch(() => {});
    } catch (e) { /* নিরব — টোন ব্যর্থ হলেও নোটিফিকেশন যেন আটকে না যায় */ }
  }

  // ✅ টগল সেটিংস থেকে টোন প্রিভিউ করার জন্য
  function previewNotificationTone() { playNotificationTone(); }

  function sendPushNotification(title, body, icon, badge, url) {
    if (Notification.permission !== 'granted') return;
    playNotificationTone(); // ✅ NEW: প্রতিটা নোটিফিকেশনের সাথে সাউন্ড টোন
  ...
  async function syncFcmToken() {
    // ⚠️ TEMP DEBUG: প্রতিটা early-return-এ toast দেখাচ্ছে যাতে মোবাইলে DevTools
    // ছাড়াই বোঝা যায় কোথায় থেমে যাচ্ছে। সমস্যা ধরা পড়লে এই toast গুলো সরিয়ে দিতে হবে।
    try {
      if (!currentUser) { showCartToast?.('🔧 DEBUG: currentUser নেই', 'warning'); return; }
      if (!('Notification' in window) || Notification.permission !== 'granted') {
        showCartToast?.('🔧 DEBUG: Notification permission = ' + Notification.permission, 'warning'); return;
      }
      if (!firebase.messaging || !firebase.messaging.isSupported || !firebase.messaging.isSupported()) {
        showCartToast?.('🔧 DEBUG: এই ব্রাউজারে FCM messaging সাপোর্ট নেই', 'warning'); return;
      }

      const swReg = await navigator.serviceWorker.ready;
      showCartToast?.('🔧 DEBUG: SW scope = ' + swReg.scope + ' | SW script = ' + (swReg.active ? swReg.active.scriptURL : 'কোনো active SW নেই'), 'success');

      const messaging = firebase.messaging();
      const token = await messaging.getToken({ vapidKey: FCM_VAPID_KEY, serviceWorkerRegistration: swReg });
      if (!token) { showCartToast?.('🔧 DEBUG: getToken() খালি token রিটার্ন করেছে', 'warning'); return; }

      showCartToast?.('🔧 DEBUG: token পাওয়া গেছে, Firestore-এ সেভ হচ্ছে...', 'success');

      await firestore.collection('users').doc(currentUser.uid).set({
        fcmTokens: firebase.firestore.FieldValue.arrayUnion(token)
      }, { merge: true });

      showCartToast?.('✅ DEBUG: FCM token সফলভাবে সেভ হয়েছে!', 'success');
      ...
      showCartToast?.('❌ DEBUG এরর: ' + e.message, 'error');
    }
  }
  ...
  function refreshNotificationSettingsUi() {
    ...
    // ক্যাটেগরি টগল
    ['orders', 'promo', 'sound'].forEach(type => {
  ...
  // ✅ NEW: লাইটবক্সে জুম করা ছবিটা ডিভাইসে সেভ/শেয়ার করার বাটন — মোবাইলে সাপোর্ট থাকলে নেটিভ
  // শেয়ার শিট (গ্যালারিতে সরাসরি সেভ করার অপশনসহ) খুলবে, না হলে সরাসরি ডাউনলোড হবে
  async function saveOrShareImage(url) {
    if (!url) return;
    try {
      ...
      const a = document.createElement('a');
      a.href = objUrl;
      a.download = `bdbigbazzar_photo_${Date.now()}.jpg`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(objUrl), 2000);
    } catch (e) {
      showCartToast('❌ ছবি সেভ করা যায়নি, আবার চেষ্টা করুন');
    }
  }
  function saveLightboxImage() {
    const img = document.getElementById('lightbox-img');
    if (img && img.src) saveOrShareImage(img.src);
  }
  ...
  function switchDetailImage(src, thumbEl) {
    ...
    // ✅ NEW (feature-74): swipe/arrow ইনডেক্স সিঙ্ক করো
    if (Array.isArray(_detailImageList)) {
      const idx = _detailImageList.indexOf(src);
      if (idx !== -1) _detailImageIndex = idx;
    }
  }

  // ✅ NEW (feature-74): একাধিক ছবির মধ্যে স্লাইড করার জন্য স্টেট ও ফাংশন
  let _detailImageList = [];
  let _detailImageIndex = 0;

  function slideDetailImage(direction) {
    if (!Array.isArray(_detailImageList) || _detailImageList.length <= 1) return;
    _detailImageIndex = (_detailImageIndex + direction + _detailImageList.length) % _detailImageList.length;
    const mainImg = document.getElementById('detail-product-image');
    if (mainImg) mainImg.src = _detailImageList[_detailImageIndex];
    document.querySelectorAll('.detail-thumb').forEach((t, i) => t.classList.toggle('active', i === _detailImageIndex));
  }
  ...
  function renderDetailThumbs(product) {
    ...
    const arrowsWrap = document.getElementById('detail-image-arrows');
    if (!strip) return;
    ...
    const imgList = (Array.isArray(product.images) && product.images.length > 1)
      ? product.images
      : [product.image];
    _detailImageList = imgList;
    _detailImageIndex = 0;
    if (imgList.length <= 1) {
      strip.innerHTML = '';
      if (arrowsWrap) arrowsWrap.classList.add('hidden');
      return;
    }
    ...
    if (arrowsWrap) arrowsWrap.classList.remove('hidden');
  }

  // ✅ NEW (feature-74): পণ্যের ভিডিও দেখানো (থাকলে)
  ...
  // ✅ NEW (feature-74): মেইন ছবিতে টাচ-সোয়াইপ করে স্লাইড করার সাপোর্ট
  let _detailSwipeStartX = null;
  function handleDetailImageTouchStart(e) {
    _detailSwipeStartX = e.touches ? e.touches[0].clientX : null;
  }
  function handleDetailImageTouchEnd(e) {
    if (_detailSwipeStartX === null) return;
    const endX = (e.changedTouches && e.changedTouches[0]) ? e.changedTouches[0].clientX : null;
    if (endX === null) { _detailSwipeStartX = null; return; }
    const diff = endX - _detailSwipeStartX;
    if (Math.abs(diff) > 40) slideDetailImage(diff < 0 ? 1 : -1);
    _detailSwipeStartX = null;
  }
  // পণ্যের বিবরণ দেখানো
  ...
  // ============================================================
  // ✅ [FIX] Service Worker Registration — এখন রিয়েল স্ট্যাটিক ফাইল দিয়ে
  // (আগে Blob URL দিয়ে রেজিস্টার করা হতো, কারণ Blogspot-এ আলাদা .js ফাইল হোস্ট
  //  করা যেত না। কিন্তু blob: URL অ্যাপ পুরোপুরি বন্ধ থাকা অবস্থায় বা ব্রাউজার
  //  রিস্টার্টের পর ইনভ্যালিড হয়ে যেত — ফলে push event ফায়ার হতো না এবং
  //  ব্যাকগ্রাউন্ডে/অ্যাপ বন্ধ থাকা অবস্থায় নোটিফিকেশন আসত না। এখন Firebase
  //  Hosting-এ মাইগ্রেশনের পর /firebase-messaging-sw.js রিয়েল ফাইল হিসেবে
  //  সার্ভ করা হচ্ছে, যেটা browser browser restart / app fully-closed অবস্থাতেও
  //  রিলায়েবলি push event পায়।)
  // ============================================================
