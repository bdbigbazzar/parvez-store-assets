  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
      ...
      // ✅ [NEW] প্রত্যেক ইউজারের নিজের পছন্দের রিংটোন লোড করো (Firestore + localStorage cache)
  ...
  // ✅ [NEW] রিংটোন সেটিং — প্রতিটি প্রিসেট Web Audio দিয়ে তৈরি (কোনো ফাইল/নেটওয়ার্ক লাগে না, তাই সবসময় কাজ করবে)
  // প্রতিটি নোট [ফ্রিকোয়েন্সি, শুরু-সময়, শেষ-সময়] (সেকেন্ডে), cycle = পুরো একবার বাজার সময়কাল (তারপর লুপ হয়)
  const RINGTONE_PRESETS = {
    classic: { label: 'ক্লাসিক ডিং',   cycle: 0.9,
      notes: [[880, 0, 0.18], [1100, 0.22, 0.40]] },
    chime:   { label: 'মৃদু চাইম',      cycle: 1.6,
      notes: [[523, 0, 0.22], [659, 0.24, 0.46], [784, 0.48, 0.78]] },
    marimba: { label: 'মারিম্বা',       cycle: 1.2,
      notes: [[392, 0, 0.12], [494, 0.14, 0.26], [587, 0.28, 0.40], [784, 0.42, 0.62]] },
    digital: { label: 'ডিজিটাল পালস',   cycle: 0.7, square: true,
      notes: [[700, 0, 0.10], [700, 0.16, 0.26], [700, 0.32, 0.42]] },
    melody:  { label: 'মিষ্টি সুর',      cycle: 2.0,
      notes: [[659, 0, 0.20], [784, 0.22, 0.42], [880, 0.44, 0.64], [784, 0.66, 0.86], [659, 0.88, 1.15]] }
  };
  let _currentRingtonePreset = localStorage.getItem('bdbb_ringtone_pref') || 'classic';

  // ✅ লগইনের পর Firestore থেকে ইউজারের সেভ করা রিংটোন প্রেফারেন্স লোড করো
  async function _loadRingtonePreference() {
    if (!currentUser) return;
    try {
      const doc = await firestore.collection('users').doc(currentUser.uid).get();
      const saved = doc.exists ? doc.data().ringtonePreference : null;
      if (saved && RINGTONE_PRESETS[saved]) {
        _currentRingtonePreset = saved;
        localStorage.setItem('bdbb_ringtone_pref', saved);
      }
    } catch (e) { /* নেটওয়ার্ক না থাকলে localStorage-এ ক্যাশ করা মানই ব্যবহার হবে */ }
  }

  // ✅ ইউজার একটা প্রিসেট বেছে নিলে Firestore + localStorage দুই জায়গাতেই সেভ করো
  async function saveRingtonePreference(presetId) {
    if (!RINGTONE_PRESETS[presetId]) return;
    _currentRingtonePreset = presetId;
    localStorage.setItem('bdbb_ringtone_pref', presetId);
    refreshRingtoneSettingsUi();
    previewRingtonePreset(presetId);
    if (currentUser) {
      try {
        await firestore.collection('users').doc(currentUser.uid).set({
          ringtonePreference: presetId,
          updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
      } catch (e) { /* অফলাইনেও localStorage-এ সেভ থেকে যাবে, পরে সিঙ্ক হবে */ }
    }
    showCartToast('রিংটোন সেভ হয়েছে ✅', 'success');
  }

  // ✅ সেটিং শিটে ▶ বাটনে চাপ দিলে প্রিসেটটা একবার বাজিয়ে দেখাও (লুপ ছাড়া)
  function previewRingtonePreset(presetId) {
    try {
      const preview = new Audio(_buildRingtoneWav(presetId));
      preview.volume = 1.0;
      preview.play().catch(() => {});
    } catch (e) {}
  }

  // ✅ বটম-শিট খোলার সময় প্রিসেট লিস্ট রেন্ডার করো, বর্তমান পছন্দে ✓ দেখাও
  function refreshRingtoneSettingsUi() {
    const list = document.getElementById('ringtone-preset-list');
    if (!list) return;
    list.innerHTML = Object.keys(RINGTONE_PRESETS).map(id => {
      const preset = RINGTONE_PRESETS[id];
      const selected = id === _currentRingtonePreset;
      return `
        <div class='flex items-center justify-between bg-white border ${selected ? 'border-orange-400 ring-1 ring-orange-200' : 'border-slate-200'} rounded-xl p-3.5'>
          <button onclick="saveRingtonePreference('${id}')" class='flex items-center gap-2.5 flex-1 text-left'>
            <span class='w-5 h-5 rounded-full border-2 ${selected ? 'border-orange-500 bg-orange-500' : 'border-slate-300'} flex items-center justify-center shrink-0'>
              ${selected ? "<i class='fas fa-check text-white text-[9px]'></i>" : ''}
            </span>
            <span class='text-xs font-black text-slate-800'>${preset.label}</span>
          </button>
          <button onclick="previewRingtonePreset('${id}')" class='w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center shrink-0 active:scale-90 transition'>
  ...
  // ✅ PCM নমুনা (Int16Array) থেকে প্লেব্যাক-যোগ্য WAV data URI বানানোর কমন হেল্পার
  function _packWav(buf, sampleRate) {
  ...
  // ✅ [NEW] ইউজারের বেছে নেওয়া প্রিসেট অনুযায়ী ইনকামিং রিংটোন WAV বানাও
  function _buildRingtoneWav(presetId) {
    const preset = RINGTONE_PRESETS[presetId] || RINGTONE_PRESETS.classic;
    const sampleRate = 22050;
    const numSamples = Math.floor(sampleRate * preset.cycle);
    const buf = new Int16Array(numSamples);
    for (let i = 0; i < numSamples; i++) {
      const t = i / sampleRate;
      let v = 0;
      for (const [freq, start, end] of preset.notes) {
        if (t >= start && t < end) {
          const localT = t - start, segDur = end - start;
          const raw = preset.square ? (Math.sin(2 * Math.PI * freq * t) >= 0 ? 1 : -1) : Math.sin(2 * Math.PI * freq * t);
          const fade = Math.min(segDur * 0.25, 0.03);
          const env = localT < fade ? localT / fade : (localT > segDur - fade ? (segDur - localT) / fade : 1);
          v = raw * env;
          break;
        }
      }
      buf[i] = Math.round(v * 26000);
    }
    return _packWav(buf, sampleRate);
  }

  function _buildBeepWav(type) {
    // Offline WAV generator — no network needed
    if (type === 'incoming') return _buildRingtoneWav(_currentRingtonePreset); // ✅ [NEW] ইউজারের পছন্দের রিংটোন
    // ringback (আউটগোয়িং কল) — ফিক্সড টোন, প্রিসেট প্রযোজ্য নয়
    const sampleRate = 22050;
    const duration   = 1.2;
