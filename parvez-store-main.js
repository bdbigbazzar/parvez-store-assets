  ...
  const VOUCHER_PERCENT = 0.05; // পণ্যের দামের ৫% ভাউচার হিসেবে জমা হবে
  ...
  async function _creditLiveVoucherIfPending() {
    if (!_liveVoucherSession || _liveVoucherSession.credited || !currentUser) { _liveVoucherSession = null; return; }
    const amount = Math.round((_liveVoucherSession.price || 0) * VOUCHER_PERCENT);
    if (amount <= 0) { _liveVoucherSession = null; return; }
    try {
      _liveVoucherSession.credited = true;
      userVoucherBalance += amount;
      await firestore.collection('users').doc(currentUser.uid).set({
        voucherBalance: userVoucherBalance
      }, { merge: true });
      await firestore.collection('voucher_history').add({
        uid: currentUser.uid,
        amount,
        productName: _liveVoucherSession.name || '',
        productId: _liveVoucherSession.productId || '',
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      });
      updateVoucherUI();
      showCartToast(`🎉 লাইভ শেষ হয়েছে! আপনার ভাউচারে ৳${amount} জমা হয়েছে।`, 'success');
    } catch (e) {
      console.error('Voucher credit error:', e);
    }
    _liveVoucherSession = null;
  }
    ...
    try {
      ...
      snap.forEach(doc => {
        (doc.data().items || []).forEach(it => {
          if (it.token && String(it.token).toUpperCase() === token && String(it.id) === String(_pendingLiveTokenProduct.id)) {
            matchedItem = it;
          }
        });
      });
      if (!matchedItem) {
        if (errEl) { errEl.innerText = '❌ টোকেনটি সঠিক নয় অথবা এই পণ্যের সাথে মিলছে না!'; errEl.classList.remove('hidden'); }
        return;
      }
      // ✅ যাচাই সফল — লাইভ শেষে ভাউচার জমা দেওয়ার জন্য সেশন সেট করো (ক্রয়কালীন দাম ধরে)
      _liveVoucherSession = { productId: matchedItem.id, name: matchedItem.name, price: matchedItem.price, token, credited: false };
      await _applyLiveProductSelection(_pendingLiveTokenProduct);
      closeLiveTokenVerifyModal();
      showCartToast('✅ টোকেন যাচাই সফল! লাইভ শেষে ভাউচার জমা হবে।', 'success');
    } catch (e) {
      console.error('Token verify error:', e);
      if (errEl) { errEl.innerText = 'যাচাই করা যায়নি। ইন্টারনেট চেক করে আবার চেষ্টা করুন।'; errEl.classList.remove('hidden'); }
    } finally {
