  ...
  function tejMediaSelected(input) {
    ...
    const reader = new FileReader();
    reader.onload = e => {
      _tejMediaBase64 = e.target.result;
  ...
  function removeTejMedia() {
    _tejMediaBase64 = null; _tejMediaMime = null;
    document.getElementById('tej-image-preview-wrap').classList.add('hidden');
    document.getElementById('tej-video-preview-wrap').classList.add('hidden');
  ...
  async function submitTejPost() {
    ...
    if (!text && !_tejMediaBase64) { showCartToast('⚠️ টেক্সট বা মিডিয়া যোগ করুন'); return; }
    ...
    try {
      ...
      tejBatch.set(newTejPostRef, {
        ...
        text: text || '',
        mediaType,
        mediaData: _tejMediaBase64 || null,
        postCategory,
        publishAt,
        likes: 0,
        likedBy: [],
      ...
      showCartToast('❌ পোস্ট করা যায়নি');
    }
    btn.disabled = false;
    ...
    _tejCommentUnsub = firestore.collection(TEJ_COLLECTION).doc(postId).collection('comments')
      ...
      .onSnapshot(snap => {
        ...
        cl.innerHTML = snap.docs.map(c => {
          ...
          return `<div class='flex gap-2 mb-3'><div class='cursor-pointer shrink-0' onclick="openUserProfileDetail('${cd.uid}')">${av}</div><div class='bg-slate-50 rounded-2xl px-3 py-2 flex-1'><p class='text-[11px] font-bold text-slate-700 cursor-pointer' onclick="openUserProfileDetail('${cd.uid}')">${escapeHtml(cd.userName)||'অজানা'}</p><p class='text-xs text-slate-600 mt-0.5'>${escapeHtml(cd.text)}</p></div></div>`;
        }).join('');
      });
  }
  ...
  async function submitProductReview() {
    if (!currentUser) { alert('রিভিউ দিতে আগে Google দিয়ে লগইন করুন!'); return; }
    if (!currentDetailProductId) return;
    if (currentReviewRating === 0) { alert('দয়া করে স্টার রেটিং দিন!'); return; }
    ...
    try {
      ...
      applyFiltersAndRender();
      alert('✅ আপনার রিভিউ জমা হয়েছে! ধন্যবাদ।');
    } catch (e) {
      alert('রিভিউ জমা হয়নি: ' + e.message);
  ...
  function showSearchHistory() {
    ...
    if (history.length === 0 || inputVal.trim().length > 0) { dropdown.classList.add('hidden'); return; }
  ...
  function toggleWishlist(productId, productName) {
    ...
    if (wishlist.find(item => item.id === productId)) {
      ...
      alert('উইশলিস্টে যোগ করা হয়েছে!');
    }
  ...
  function openWishlistView() {
    ...
    if (wishlist.length === 0) {
      alert('আপনার উইশলিস্ট খালি! পছন্দের পণ্যে ❤️ আইকনে ট্যাপ করে যুক্ত করুন।');
      return;
    }
  ...
  async function loadAnalytics(period = 'week') {
    ...
    try {
      ...
      const totalRevenue  = filteredOrders.filter(o => o.status === 'ডেলিভারড ✅').reduce((s, o) => s + (o.totalAmount || 0), 0);
      ...
      document.getElementById('analytics-revenue-change').innerText = 'ডেলিভারড অর্ডার থেকে';
  ...
  function renderAnalyticsChart(orders, daysBack) {
    ...
    orders.forEach(o => {
      ...
      if (dailyRevenue[key] !== undefined) {
        dailyRevenue[key] += o.totalAmount || 0;
        dailyOrders[key]  += 1;
      }
    });
...
চামড়ার জুতো,2500,shoe,"40,41,42,43",15,খাঁটি চামড়ার তৈরি দীর্ঘস্থায়ী জুতো,https://example.com/shoe.jpg`;
  ...
  async function enableBrowserPushNotification() {
    ...
    if (granted) showCartToast(t('notifEnabledToast'));
  }
  ...
  (function registerServiceWorker() {
    ...
    const swSource = `
      ...
      // ✅ Push notification আসলে দেখানো (ভবিষ্যতে সার্ভার-পুশ যোগ হলে কাজে লাগবে)
      self.addEventListener('push', (event) => {
        let data = {};
        try { data = event.data ? event.data.json() : {}; } catch (e) {}
        const title = data.title || 'পারভেজ স্টোর';
        event.waitUntil(self.registration.showNotification(title, {
          body: data.body || '',
          icon: data.icon || 'https://bdbigbazzar.blogspot.com/icon-192.png',
          badge: 'https://bdbigbazzar.blogspot.com/icon-192.png',
          data: { url: data.url || 'https://bdbigbazzar.blogspot.com' }
        }));
      });
